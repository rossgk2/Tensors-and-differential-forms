...
* Just enough complexity is presented in my book. There's not too little, as there is in introductory books on differential forms that don't use linear algebra, because my book explains and uses the relveant linear algebra. There's not too much, as there is in manifold books that do use linear algebra but that introduce tensors, because tensors are investigated in the simpler setting of vector spaces before tensors on manifolds are defined.
