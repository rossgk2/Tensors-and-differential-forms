...
\label{ch::diff_forms::lemma::technical_pullback_yi}
    ...
    Let $M$ and $N$ be smooth $n$-manifolds WWBOC, let $(U, \xx)$ be a smooth chart on $M$, let $(V, \yy)$ be a smooth chart on $N$, and let $\FF:U \subseteq M \rightarrow V \subseteq N$ be a diffeomorphism. Then $\Omega^k \FF^*(dy^i) = d\tF_{(V, \yy)}^i$.
\end{lemma}
