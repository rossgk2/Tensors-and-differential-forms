...
Otherwise, when analysis of an $x \in S$ with $P(x)$ is too verbose, we again allude to the syntax ``$\forall x \in S \spc P(x) \implies$'' by using a phrase such as ``Let $x \in S$ and $P(x)$'', ``Assume $x \in S$ and $P(x)$'', or ``Fix $x \in S$ and $P(x)$'':
