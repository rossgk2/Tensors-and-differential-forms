...
\begin{theorem}
    ...
    (Geometric tangent vectors are naturally isomorphic to ``directional derivative functions'').
     ...
     For $\vv \in \R^n$ and $f \in C^\infty(\R^n \rightarrow \R)$, let $\frac{\pd}{\pd \vv}\Big|_\pp$ denote the function $f \mapsto \frac{\pd}{\pd \vv}\Big|_\pp (f)$. The map $\vv_\pp \mapsto \frac{\pd}{\pd \vv}\Big|_\pp$ is a natural linear isomorphism $\R^n_\pp \rightarrow T_\pp(\R^n)$.
\end{theorem}
...
\label{ch::manifolds::defn::basis_of_TpM}
    ...
    In the above, $\tf_{(U, \xx)}$ denotes the coordinate representation of $f$ relative to the chart $(U, \xx)$. The $\frac{\pd}{\pd x^i}\Big|_\pp$ to the left of $:=$ are what we are defining in our abuse of notation, and the $\frac{\pd}{\pd x^i}\Big|_{\xx(\pp)} = \frac{\pd}{\pd \see^i}\Big|_{\xx(\pp)}$ on the right side are directional derivatives taking in smooth functions on $\R^n$ as their arguments. 
...
\begin{proof}
    ...
    \begin{enumerate}
        \item We will put an ``LS'' under $\frac{\pd}{\pd x^i}\Big|_\pp$ when we mean $\frac{\pd}{\pd x^i}\Big|_\pp$ to be from the left side of the condition in the above definition. All other occurrences of $\frac{\pd}{\pd x^i}\Big|_\pp$ in this proof are partial derivatives.
...
\label{ch::manifolds::defn::differential_smooth_function_Rn_Rm}
    ...
    (This last differential is no longer one of the usual partial derivatives; recall from Definition \ref{ch::manifolds::defn::basis_of_TpM}) that it is defined by $\frac{\pd}{\pd x^i}\Big|_\pp(g) = \frac{\pd \widetilde{g}_{(U, \xx)}}{\pd x^i}\Big|_\pp$).
...
\begin{proof}
    We need to check that $d\FF_\pp(v_\pp)$ is a derivation at $\FF(\pp) \in N$. To do so, follow the proof of Theorem \ref{ch::manifolds::defn::basis_of_TpM}, which showed $\underbrace{\frac{\pd}{\pd x^i}\Big|_\pp}_{\text{LS}}$ is a derivation at $\xx(\pp) \in \R^n$.
\end{proof}
...
\label{ch::manifolds::thm::differential_smooth_function_coords}
    \smallcite{book::SM}{281} (Differential of a smooth function $M \rightarrow \R$ in local coordinates).

    Let $M$ be a smooth $n$-manifold WWBOC or corners, and let $(U, \xx)$ be a smooth chart on $M$, with $x^i$ being the $i$th coordinate function of $\xx$. The differential of a smooth function $f:M \rightarrow \R$ obtained by identifying $T_{f(\pp)}(\R) \cong \R$ (see the previous theorem) is given by
    
    \begin{empheq}[box = \fbox]{align*}
        df_\pp &= \sum_{i = 1}^n \frac{\pd f}{\pd x^i}\Big|_\pp dx^i_\pp \text{ for all $\pp \in U$}
    \end{empheq}
    
    Note, the $d$ on the right side is the differential of the $i$th coordinate function $x^i$ of $\xx$ after identifying $T_{f(\pp)}(\R) \cong \R$ (see the previous theorem).
\end{theorem}
...
\begin{deriv}
    ...
    What set do the differentials $dx^i_\pp$ reside in? Well, $dx^i_\pp:T_\pp(M) \rightarrow \R$, so $dx^i_\pp \in T_\pp(M)^*$.
    
    Additionally, notice that since applying $dx^i_\pp$ to $v_\pp$ yields the $i$th coordinate of $v_\pp$ in the $\Big\{ \frac{\pd}{\pd x^i}\Big|_\pp \Big\}_{i = 1}^n$ basis, the set $\{dx^i_\pp\}_{i = 1}^n$ must be the dual basis for $T_\pp(M)^*$.

    Indeed, the dual basis for $T_\pp(M)^*$ is the unique set of vectors satisfying the above condition involving $v_\pp$, so it follows that $\{dx^i_\pp\}_{i = 1}^n$ is the dual basis for $T_\pp(M)^*$.
...
\begin{remark}
    ...
    Let $M$ be a smooth $n$-manifold, and consider a smooth chart $(U, \xx)$ about $\pp \in M$. You have likely noticed that the collection of symbols ``$x^i$'' appears in notation in various ways. A summary of all the ways $x^i$ is used is as follows. When $x^i$ appears in a ``numerator'' (e.g. $\frac{\pd x^i}{\pd x_j}$), as an argument of a differential (e.g. $dx^i$), or ``by itself'' (e.g. $x^i$), then $x^i$ is the $i$th coordinate function of $\xx$. When $x^i$ appears in a ``denominator'', it is because that ``denominator'' is part of the basis vector $\frac{\pd}{\pd x^i}\Big|_\pp$ of $T_\pp(M)$.
\end{remark}
...
\begin{lemma}
    ...
    When $M$ is a smooth one-dimensional manifold and $f:M \rightarrow \R$ is a smooth function, then for every $\pp \in M$ the cotangent space $T_\pp^*(M)$ is one-dimensional, so $df = \sum_{i = 1}^1 (\pd_i f)_\pp dx^i_\pp = f'(x(\pp)) dx_\pp$ can be identified with the scalar $f'(x(\pp)) \in \R$. In other words, the induced differential $\td$ under these identifications is $\td = D$, where $Df = f'$.
\end{lemma}
...
\begin{theorem}
    ...
    Let $M$ be a one-dimensional manifold, let $(U, \xx)$ be a smooth chart on $M$, and suppose we have a smooth function $f:U \rightarrow \R$. Consider the coordinate representation $\tf_{(U, \xx)} = f \circ \xx^{-1}:\R \rightarrow \R$, denoted as $\tf := \tf_{(U, \xx)}$ for brevity. Using $d$ to denote $\td = D$, we have that $df_\pp = d\tf_{x(\pp)} dx_\pp$ for every $\pp \in U$. 
    ...
    \begin{align*}
        \frac{df}{dx} = \tfPrime(x) \text{ for all $\pp \in U$}.
    \end{align*}
\end{theorem}
