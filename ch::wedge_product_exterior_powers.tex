...
[Segway...] [The following is a much cleaner version of a previous lemma (Lemma \ref{ch::lin_alg::lemma:det_as_scale_factor}).]

\begin{theorem}
\label{ch::exterior_powers::thm::det_as_scale_factor}
    (Determinant as scale factor).
    
    Let $V$ and $W$ be $n$-dimensional vector spaces. If $\ff:V \rightarrow W$ is a linear function, then $\ff(\vv_1) \wedge ... \wedge \ff(\vv_k) = \det(\ff) \vv_1 \wedge ... \wedge \vv_k$ for all $\vv_1, ..., \vv_k \in V$.
\end{theorem}

\begin{proof}
    The proof is very similar to that of Lemma \ref{ch::lin_alg::lemma:det_as_scale_factor}. Seeing it ``again'' with the new language of the wedge product $\wedge$ may help understanding.

    Because $\wedge$ looks as if it is a multilinear alternating function, $\ff(\ee_1) \wedge ... \wedge \ff(\ee_n))$ is analogous to the determinant $\det(\ff(\ee_1), ..., \ff(\ee_n))$. So, set $(a^i_j) = [\ff(E)]_F$, consider the expression $\ff(\ee_1) \wedge ... \wedge \ff(\ee_n)$, and use essentially the same argument as was made to derive the permutation formula of the determinant. We obtain
...
As a result of this relationship between the wedge product and determinant, the wedge product ``inherits'' several properties of the determinant.
...
\begin{theorem}
    ...
    show that $\wedge$ has similar properties to ``consequent properties of det'' here
\end{theorem}
