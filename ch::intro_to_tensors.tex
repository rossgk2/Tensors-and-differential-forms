...
\begin{defn}
    ...
    Let $V_1, ..., V_k, W$ be finite-dimensional vector spaces. When defining a multilinear function $V_1 \times ... \times V_k \rightarrow W$ or a linear function $V_1 \otimes ... \otimes V_k \rightarrow W$, 
\end{defn}

Now, we start investigating key properties of tensors.
...
\begin{proof}
    To construct a linear isomorphism $\FF:\LLLL(V_1 ... \times V_k \rightarrow W) \mapsto \LLLL(V_1 \otimes ... \otimes V_k \rightarrow W)$, we define $\FF(\ff) = \hh_\ff$, where $\hh_\ff$ is from Theorem \ref{ch::lin_alg::thm::universal_prop_tensor_prod}. We need to show that $\FF$ is a linear bijection. Currently, $\hh_\ff$ is defined implicitly; we will need to find a more explicit description of $\hh_\ff$ to approach this.
    
    The implicit description of $\hh_\ff$ which we already know is $\ff(\vv_1, ..., \vv_k) = \hh_\ff(\vv_1 \otimes ... \otimes \vv_k)$. Rephrasing this, we have $\ff(\vv_1, .., \vv_k) = \hh_\ff(\gg(\vv_1 \otimes ... \otimes \vv_k))$, where $\gg:V_1 \times ... \times V_k \rightarrow V_1 \otimes ... \otimes V_k$ is defined by $\gg(\vv_1, ..., \vv_k) := \vv_1 \otimes ... \otimes \vv_k$. Of course, this is equivalent to $\ff = \hh_\ff \circ \gg$. It is clear that $\gg$ has a trivial kernel. Thus $\gg$ is invertible and $\hh_\ff = \ff \circ \gg^{-1}$.
