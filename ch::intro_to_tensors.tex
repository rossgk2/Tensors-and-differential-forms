...
\label{ch::motivated_intro::rmk::outer_products}
    ...
    Let $\vv \in K^n$ and let $\ww \in K^m$. Throughout this chapter, we've seen that matrices of the form $\ww \vv^\top$ are identified with linear functions of the form ${\ww \circ \phi, \phi \in V^*}$, which are identified with tensors of the form ${\ww \otimes \phi, \phi \in V^*}$. Because elementary tensors are related to outer products in this way, then when $V$ and $W$ are vector spaces, a tensor $\ww \otimes \vv \in W \otimes V$ is unfortunately sometimes called ``the outer product of $\ww$ and $\vv$''.
    
    Additionally, some texts take advantage of the fact that the function $(\vv, \ww) \mapsto \ww \vv^\top$ is bilinear, and actually \textit{define} the tensor product of $\ww \in K^m$ and $\vv \in K^n$ to be $\ww \otimes \vv := \ww^\top \vv$.
\end{defn}
