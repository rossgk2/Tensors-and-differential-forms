...
\begin{remark}
    ...
    Because of this, we can say things such as ``we \textit{impose} that $\ff$ be linear'', or ``we \textit{extend} the definition of $\ff$ \textit{with linearity}'', to indicate that $\ff(\TT)$ is secretly $\gg(\TT)$ when $\TT$ is a nonelementary tensor.
\end{remark}
...
\label{ch::lin_alg::thm::universal_prop_tensor_prod}
    ...
    There exists a linear function $\hh_\ff:V_1 \otimes ... \otimes V_k \rightarrow W$ such that $\ff(\vv_1, ..., \vv_k) = \hh_\ff(\vv_1 \otimes ... \otimes \vv_k)$.
...
\begin{proof}
    We define $\hh_\ff(\vv_1 \otimes ... \otimes \vv_k) := \ff(\vv_1, ..., \vv_k)$, and impose that $\hh_\ff$ be linear to guarantee that $\hh_\ff(\TT)$ is indeed defined when $\TT$ is a nonelementary tensor.
\end{proof}
...
\begin{proof}
    To construct a linear isomorphism $\FF:\LLLL(V_1 ... \times V_k \rightarrow W) \mapsto \LLLL(V_1 \otimes ... \otimes V_k \rightarrow W)$, we define $\FF(\ff) = \hh_\ff$, where $\hh_\ff$ is from Theorem \ref{ch::lin_alg::thm::universal_prop_tensor_prod}. We need to show that $\FF$ is a linear bijection. 
    
    First, we find an explicit expression for $\hh_\ff$. Since $\ff = \hh_\ff \circ \gg$ and since $\gg$ has a trivial kernel, then $\gg$ is invertible and $\hh_\ff = \ff \circ \gg^{-1}$.
