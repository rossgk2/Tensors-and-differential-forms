...
\label{ch::motivated_intro::defn::tensor_product_space}
    ...
    for all $\vv_1, ..., \vv_k \in V$ and $c \in K$, so that $\otimes$ looks like it is a multilinear function.
...
\begin{proof}
    ...
    We do so by defining an isomorphism in the ``reverse'' direction. We define this isomorphism on elementary tensors and extend linearly. Given $(\phi \otimes \psi) \in V^* \otimes W^*$, we produce the linear function $\ff_{\phi \otimes \psi} \in (V \otimes W)^*$, where $\ff_{\phi \otimes \psi}:V \otimes W \rightarrow K$ is defined by $\ff_{\phi \otimes \psi}(\vv \otimes \ww) = \phi(\vv) \psi(\ww)$. The explicit check that this is a linear isomorphism is left to the reader.
\end{proof}
