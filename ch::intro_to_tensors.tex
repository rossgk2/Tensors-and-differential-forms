...
\label{ch::motivated_intro::rmk::outer_products}
    ...
    Let $K$ be a field. We showed in the first derivation of this chapter (Derivation \ref{ch::motivated_intro::deriv::decomposition_of_a_linear_function}) that every matrix $\AA \in K^{m \times n}$ can be decomposed as $\AA = \sum_{\vv \in E, \ww \in F} \ww \vv^\top$, where $E \subseteq K^n$ and $F \subseteq K^m$. Because, in the expression $\ww \vv^\top$, the transpose $\top$ is on the ``outside'' of $\ww$ and $\vv$, then $\ww \vv^\top$ is sometimes called the ``outer product'' of $\ww$ and $\vv$. This is the main point. Here are some additional details:

    \begin{itemize}
        \item We've seen that matrices of the form $\ww \vv^\top$ (where $\ww \in K^m$ and $\vv \in K^n$) are identified with linear functions of the form $\ww \circ \phi$ (where $\ww \in K^m$ and $\phi \in (K^n)^*$), which are identified with tensors of the form $\ww \otimes \phi$, (where $\ww \in K^m$ and $\phi \in (K^n)^*$). Because elementary tensors are related to matrix outer products in this way, then when $V$ and $W$ are vector spaces, a tensor $\ww \otimes \vv \in W \otimes V$ is unfortunately sometimes called the ``outer product of $\ww$ and $\vv$''.
        \item Some texts take advantage of the fact that the function $(\vv, \ww) \mapsto \ww \vv^\top$ is bilinear, and define the tensor product of $\ww \in K^m$ and $\vv \in K^n$ to be $\ww \otimes \vv := \ww^\top \vv$.
    \end{itemize}
\end{remark}
