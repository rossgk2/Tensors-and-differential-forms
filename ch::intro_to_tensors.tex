...
\begin{theorem}
    ...
    \textbf{this theorem should not need to involve a metric}
\end{theorem}
...
\begin{proof}
    ...
    Using that $\phi^{\ee_{j_\ell}}(\ee_{i_k}) = \ee_{j_\ell}^\flat(\ee_{i_k}) = g(\ee_{j_\ell}, \ee_{i_k}) = g_{j_\ell i_k} = g_{i_k j_\ell}$, \textbf{here we have assumed that $\phi^{\ee_{j_\ell}} = \ee_{j_\ell}^\flat$, which is equivalent to assuming that $E$ is orthonormal.} we see that the $(k, \ell)$ contraction of $\TT$ is
