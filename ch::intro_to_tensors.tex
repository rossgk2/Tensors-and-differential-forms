...
\begin{defn}
    ...
    Note that a $1$-linear function is simply a linear function. A $2$-linear function is called a \textit{bilinear function}. When $k$ is not known, $k$-linear functions are usually referred to as \textit{multilinear functions}.
\end{defn}
...
We know from our studies in linear algebra that elements of a vector space are sometimes called ``linear elements'' because their decompositions are respected by linear functions (see Definition \ref{ch::lin_alg::defn::linear_function_intuitive}). We have just been introduced to the notion of a multilinear function. A natural question is then, ``what is a reasonable definition of `multilinear element'?'' 
...
\label{ch::lin_alg::thm::universal_prop_tensor_prod}
    ...
    Then there exists a linear function $\hh_\ff:V_1 \otimes ... \otimes V_k \rightarrow W$ such that $\ff(\vv_1, ..., \vv_k) = \hh_\ff(\vv_1 \otimes ... \otimes \vv_k)$.

    Equivalently, there are linear functions $\hh_\ff:V_1 \otimes ... \otimes V_k \rightarrow W$ and $\gg:V_1 \times ... \times V_k \rightarrow V_1 \otimes ... \otimes V_k$, with $\gg$ defined on elementary tuples as $\gg(\vv_1, ..., \vv_k) := \vv_1 \otimes ... \otimes \vv_k$, such that $\ff = \hh_\ff \circ \gg$.
\end{theorem}

\begin{proof}
    We define $\hh_\ff(\vv_1 \otimes ... \otimes \vv_k) := \ff(\vv_1, ..., \vv_k)$, and impose that $\hh_\ff$ be linear to guarantee that $\hh_\ff(\TT)$ is indeed defined when $\TT$ is a nonelementary tensor. This works because defining how a linear function acts on elementary tensors (i.e. tensors of the form $\vv_1 \otimes ... \otimes \vv_k$) determines how it acts on any tensor.
\end{proof}

This result shows us that the decompositions of multilinear elements truly are respected by multilinear functions. 

\begin{remark}
    (The sense in which multilinear functions respect multilinear elements).
        
    Consider a linear combination of tuples subject to the equivalence relation given in the definition of tensor product spaces (recall Definition \ref{ch::motivated_intro::defn::tensor_product_space}),

    \begin{align*}
        \sum_{i_1, ..., i_k \in \{1, ..., n\}} T_{i_1 ... i_k} (\ww_{i_1}, ..., \ww_{i_k}),
    \end{align*}

    where $\ww_{i_1} \in V_1, ..., \ww_{i_k} \in V_k$ and $T_{i_1, ..., i_k} \in K$ for $i_1, ..., i_k \in \{1, ..., n\}$.

    Now, let $W$ be another finite-dimensional vector space and suppose $\ff:V_1 \times ... \times V_k \rightarrow W$ is a multilinear function. The above theorem says that there are linear functions $\hh_\ff:V_1 \otimes ... \otimes V_k \rightarrow W$ and $\gg:V_1 \times ... \times V_k \rightarrow V_1 \otimes ... \otimes V_k$, with $\gg$ defined on elementary tuples as $\gg(\vv_1, ..., \vv_k) := \vv_1 \otimes ... \otimes \vv_k$, such that $\ff = \hh_\ff \circ \gg$. Therefore

    \begin{align*}
        \ff\Big(\sum_{i_1, ..., i_k \in \{1, ..., n\}} T_{i_1 ... i_k} (\ww_{i_1}, ..., \ww_{i_k})\Big) &= \hh_\ff\Big(\gg\Big(\sum_{i_1, ..., i_k \in \{1, ..., n\}} T_{i_1 ... i_k} (\ww_{i_1}, ..., \ww_{i_k})\Big)\Big) \\
        &= \hh_\ff\Big(\sum_{i_1, ..., i_k \in \{1, ..., n\}} T_{i_1 ... i_k} \ww_{i_1} \otimes ... \otimes \ww_{i_k}\Big) \\
        &= \sum_{i_1, ..., i_k \in \{1, ..., n\}} T_{i_1 ... i_k} \hh_\ff(\ww_{i_1} \otimes ... \otimes \ww_{i_k}) \text{ since $\hh_\ff$ is linear} \\
        &= \sum_{i_1, ..., i_k \in \{1, ..., n\}} T_{i_1 ... i_k} \ff(\ww_{i_1}, ..., \ww_{i_k}) \text{ since $\hh_\ff(\vv_1 \otimes ... \otimes \vv_k) = \ff(\vv_1, ..., \vv_k)$}
    \end{align*}

    Overall, we have

    \begin{align*}
        \ff\Big(\sum_{i_1, ..., i_k \in \{1, ..., n\}} T_{i_1 ... i_k} (\ww_{i_1}, ..., \ww_{i_k})\Big) = \sum_{i_1, ..., i_k \in \{1, ..., n\}} T_{i_1 ... i_k} \ff(\ww_{i_1}, ..., \ww_{i_k}).
    \end{align*}

     Thus, we see that if the tuple input of $\ff$ is decomposed in accordance to the tensor equivalence relation, then $\ff$ preserves this decomposition.
\end{remark}
...
\begin{proof}
    ...
    To construct a linear isomorphism $\FF:\LLLL(V_1 \times V_2 \rightarrow W) \mapsto \LLLL(V_1 \otimes V_2 \rightarrow W)$, we define $\FF(\ff) = \hh_\ff$, where $\hh_\ff$ is from the case $k = 2$ of Theorem \ref{ch::lin_alg::thm::universal_prop_tensor_prod}. We need to show that $\FF$ is a linear bijection. Currently, $\hh_\ff$ is defined implicitly; we will need to find a more explicit description of $\hh_\ff$ to approach this.
    
    The implicit description of $\hh_\ff$ which we already know is $\ff(\vv_1, \vv_2) = \hh_\ff(\vv_1 \otimes \vv_2)$. Rephrasing this, we have $\ff(\vv_1, \vv_2) = \hh_\ff(\gg(\vv_1, \vv_2))$, where $\gg:V_1 \times V_2 \rightarrow V_1 \otimes V_2$ is defined by $\gg(\vv_1, \vv_2) := \vv_1 \otimes \vv_2$. Of course, this is equivalent to $\ff = \hh_\ff \circ \gg$. It is clear that $\gg$ has a trivial kernel. Thus $\gg$ is invertible and $\hh_\ff = \ff \circ \gg^{-1}$.

    Now, having an explicit expression for $\hh_\ff$, $\hh_\ff = \ff \circ \gg^{-1}$, we can show that $\FF$ is linear. The map is linear because, given vector spaces $Y, Z, W$, the function $\circ$ which composes linear functions, ${\circ:\LLLL(Y \rightarrow Z) \times \LLLL(Z \rightarrow W) \rightarrow \LLLL(Y \rightarrow W)}$, is a bilinear map. (Check this fact for yourself. Other consequences of this are explored in Derivation \ref{ch::bilinear_forms_metric_tensors::deriv::compos_linear_map_with_contract}). 
    
    To show $\FF$ is bijective, since it is a map between vector spaces of the same dimension, it suffices to show that it is one-to-one, i.e., that it has a trivial kernel (recall Theorem \ref{ch::lin_alg::thm::linear_fn_1-1_iff_onto}). This is clearly the case because if $\FF(\ff) = \hh_\ff = \mathbf{0}$, then $\ff = \hh_\ff \circ \gg = \mathbf{0} \circ \gg = \mathbf{0}$.
\end{proof}
...
\begin{proof}
      It suffices to show that if $V$ and $W$ are finite-dimensional vector spaces with bases $E = \{\ee_1, ..., \ee_n\}$ and $F = \{\ff_1, ..., \ff_m\}$, then $\{\ee_i \otimes \ff_j \mid i \in \{1, ..., n\}, j \in \{1, ..., m\} \}$ is a basis of $V \otimes W$. The general result follows by induction.
      
      To show that this set spans $V \otimes W$, it suffices to show it spans the set of elementary tensors in $V \otimes W$, since any tensor in $V \otimes W$ is a linear combination of elementary tensors. For an elementary tensor $\vv \otimes \ww \in V \otimes W$, we have $\vv \otimes \ww = \sum_{ij} ([\vv]_E)^i ([\ww]_F)^j \ee_i \otimes \ff_j$ by the seeming-bilinearity of $\otimes$.
      
      To show linear independence, assume that $\sum_{ij} T_{ij} \ee_i \otimes \ff_j = \mathbf{0}$. We must show that $T_{ij} = 0$ for all $i, j$. Note that a tensor in $V \otimes W$ is the zero tensor iff\footnote{($\implies$). We have $\vv \otimes \mathbf{0} = 0 \cdot (\vv \otimes \mathbf{0}) = \mathbf{0}$. ($\impliedby$). If $\sum_{ij} T_{ij} \ee_i \otimes \ff_j = \mathbf{0}$, then $\sum_{ij} T_{ij} \ee_i \otimes \ff_j = \mathbf{0} = \vv \otimes \mathbf{0}$ for any $\vv \in V$.} it is an elementary tensor of the form $\vv \otimes \mathbf{0}$, where $\vv \in V$, or $\mathbf{0} \otimes \ww$, where $\ww \in W$. Due to the linear independence of the bases $E$ and $F$, it is impossible to obtain a $\mathbf{0}$ in either position unless all $T_{ij}$ are $0$.
\end{proof}
