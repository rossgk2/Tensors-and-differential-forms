...
\begin{itemize}
    \item (Subsection intro). You may be familiar with how the \textit{angle} between two vectors $\vv, \ww \in \R^2$ of the same length $r$ is defined by placing the tails of the vectors at the center of a circle of radius $r$, and then computing the ratio of arclength on the circle between the tips to the radius of the circle; $\text{angle} = \frac{\text{arclength}}{\text{radius}}$. Less commonly discussed, though still crucial for good understanding, is the distinction between the two types of angle, which are \textit{signed} and \textit{unsigned}. We will see that the \textit{signed counterclockwise angle} $\theta_s(\vv, \ww)$ between vectors $\vv, \ww \in \R^2$ accounts for whether the shorter of the two arcs between the vectors is achieved by counterclockwise motion (in this case, the \textit{sign} is positive) or by clockwise motion (in this case, the \textit{sign} is negative). (There is a similar definition for \textit{signed clockwise angle} $\widetilde{\theta}_s(\vv, \ww)$ that uses the opposite signed convention, in which clockwise motion is positive and counterclockwise motion is negative). The \textit{unsigned angle} $\theta_u(\vv, \ww)$ between vectors $\vv, \ww \in \R^2$ is simply the absolute value of the signed counterclockwise angle.

    Signed angle is useful for specifying positions on circles, since for every angle $t$ there is a unique vector that makes a counterclockwise or clockwise signed angle of $t$ with $\see_1$. (Unsigned angle does not work for this purpose, since for every angle $t$, there are \textit{two} vectors that make an angle of $t$ with $\see_1$).
    
    Unsigned angle is useful for computing the angle between two vectors, since if ``the angle $\theta$ between $\vv \in \R^2$ and $\ww \in \R^2$'' is a sensible notion, it must satisfy $\theta(\vv, \ww) = \theta(\ww, \vv)$, and we indeed have $\theta_u(\vv, \ww) = \theta_u(\ww, \vv)$. (Signed angle does not work for this purpose, since $\theta_s(\vv, \ww) \neq \theta_s(\ww, \vv)$).
    
    \item definition: signed CCW and CW angles
    \begin{itemize}
        \item Use $\widetilde{\theta}_s$ for signed CW angle
    \end{itemize}
    \item theorem: $\theta_s(\vv, \ww) + \widetilde{\theta}_s(\vv, \ww) = 2\pi$ for all $\vv, \ww$. Since $-\theta \bmod 2\pi = 2\pi - \theta \bmod 2\pi$ [this is evident from distributivity of mod over addition- might not even need fact in appendix. would need to give a footnote about distributivity though] it follows that $-\theta_s(\vv, \ww) \bmod 2\pi = \widetilde{\theta}_s(\vv, \ww) \bmod 2\pi$.
    \item remark: ``It is convention to use counterclockwise angle''
     \item definition: $\cos(t)$ and $\sin(t)$ as coordinates of vector that makes signed counterclockwise angle of $t$ with $\see_1$
    \item definition: $\arccos$ and $\arcsin$ as inverses of one-to-one restrictions of $\cos$ to $[0, \pi]$ and $\sin$ to $[-\frac{\pi}{2}, \frac{\pi}{2}]$.
    \item definition: unsigned angle
    \item ...
    \item definition: $\vv \cdot \ww := ||\ww|| \sproj(\vv \rightarrow \ww)$
    \item lemma: $\sproj(v -> w) = ||\vv|| \cos(\theta_s(\vv, \ww))$
...
\begin{enumerate}
    \item prove that rotation matrix of CCW rotation by $t$ is $\begin{pmatrix} \cos(t) & -\sin(t) \\ \sin(t) & \cos(t) \end{pmatrix}$. have this theorem properly backed up by a correct notion of angle in which $-\pi/2 \sim 3\pi/2$. we need this for the orientation section; we use $t = \pi/2$ to prove the antisymmetry of ordered bases. 
