...
\begin{itemize}
    \item overall goal: prove that rotation matrix of ccw rotation by t is the expected thing with cos(t) and sin(t). have this theorem properly backed up by a correct notion of angle in which $-\pi/2 \sim 3\pi/2$
    \item figure out what the angle is in the dot product formula. when written with arccos, it must be unsigned; when written with cos, it can probably be signed or unsigned
    \item need to know that rotations are isometries, i.e., orthogonal linear functions (this is done since rotations are defined to be certain isometries), and that orthogonal linear functions preserve algebraic dot product so can prove algebraic => geometric. this is mostly outlined.
    \item need to prove facts about projections for geometric => algebraic, and for the very last part of algebraic => geometric I think. this seems mostly done but go over it to check.
\end{itemize}
...
\begin{itemize}
    ...
    \item New: the metric tensor can have its coordinates changed
