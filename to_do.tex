...
\begin{itemize}
    ...
    Unsigned angle is useful when we desire to speak of an angle between two vectors without privileging one of the vectors as being considered the ``start'', since $\theta_u(\vv, \ww) = \theta_u(\ww, \vv)$. (Signed angle does not work for this purpose, since $\theta_s(\vv, \ww) \neq \theta_s(\ww, \vv)$).
