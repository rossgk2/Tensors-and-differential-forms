...
\begin{enumerate}
	\item[4.] (Implicit equation in $n$ variables characterization). A plane in $\R^n$ is a solution set to an equation of the form $(c_1 x_1 + ... + c_n x_n = 0, \text{ where $x_1, ..., x_n \in \R$})$.
\end{enumerate}

It's easy to prove the calculus characterization implies the implicit equation characterization. And obviously the implicit equation characterization is equivalent to the orthogonal complement characterization. A little harder is proving that the implicit equation characterization implies the subspace characterization (CTRL-F ``Implicit to span'' in the linear algebra chapter). The most opaque proof of all is required for proving that the subspace characterization implies the orthogonal complement characterization (CTRL-F ``Span to implicit'' in the linear algebra chapter).
