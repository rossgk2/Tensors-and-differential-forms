...
\begin{itemize}
    \item figure out whether signed or unsigned angle (or both) should appear in the theorem about preserved quantities
    \item figure out definition for perpendicular. don't want it to be circular
\end{itemize}
...
This section enables us to talk about hyperplanes in the systems of linear equations section.
...
Implement alternative $1$. First step is to add ``Thm 1'' and ``Defn 2'' described below to \\section\{Orientation*\} in linear\_algebra.tex. Of course, first take a moment to refamiliarize and double check that the definition of orientation from ``Defn 2'' actually works.

Then add alternative $2$ to the appendix and finish the footnote directing readers to the appendix.
...
\begin{itemize}
    ...
    \item redo proof that $\det$ tracks orientation (more specifically, $\sgn(\det(\FF))$ is orientation of $F$)
    \item add defn of $\text{orn}$ function that returns $-1$ or $1$
\end{itemize}

Then of course:

\begin{itemize}
    \item improve bullet point in GitHub readme about how this book does orientation and rotation so much better 
    \begin{itemize}
        \item we use signed angle to define orientation in $2$ dimensions, find an equivalent definition that's generalizable, and then prove theorems about this definition, instead of just defining that orientation coorresp. to the determinant out of thin air 
        \item for the comment on rotation, see the below ``Rotation'' section
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Defn 1. Current definition of orientation in $\R^2$. The \textit{orientation} of an ordered basis $(\ee_1, \ee_2)$ of $\R^2$ is said to be \textit{counterclockwise}, or \textit{positive} iff $|\theta_s(\ee_1, \ee_2)| < |\tttheta_s(\ee_1, \ee_2)|$ and \textit{clockwise}, or \textit{negative}, iff $|\tttheta_s(\ee_1, \ee_2)| < |\theta_s(\ee_1, \ee_2)|$.
    \item Thm 1. $(\ee_1, \ee_2)$ is positively oriented iff $\sproj(\ee_2 \rightarrow \see_2) > 0$, i.e., iff $\ee_2$ is above $\spann(\{\see_2\})$.
    \item Defn 2. $(\ee_1, ..., \ee_{n - 1}, \ee_n)$ is positively oriented iff $(\ee_1, ..., \ee_{n - 1})$ is positively oriented and $\sproj(\ee_n \rightarrow \see_n) > 0$
    ...
    \item Thm. Orientation is the sign of a multilinear function.
    \begin{proof}
...
Define orientation in $\R^n$ by making use of $n$-dimensional spherical coordinates. As with the other alternative, this approach can be extended to finite-dimensional inner product spaces with a given orthonormal basis.

\begin{itemize}
    \item Defn. Current definition of orientation in $\R^2$. The \textit{orientation} of an ordered basis $(\ee_1, \ee_2)$ of $\R^2$ is said to be \textit{counterclockwise}, or \textit{positive} iff $|\theta_s(\ee_1, \ee_2)| < |\tttheta_s(\ee_1, \ee_2)|$ and \textit{clockwise}, or \textit{negative}, iff $|\tttheta_s(\ee_1, \ee_2)| < |\theta_s(\ee_1, \ee_2)|$.
    \item Thm. $(\ee_1, \ee_2)$ is positively oriented iff $\theta_s(\ee_1, \ee_2) < \pi$.
    \item Defn. Define a parameterization of the $n$-sphere, $(r, \theta_2, ..., \theta_n) \mapsto \rr_n(r, \theta_2, ..., \theta_n)$, where $\theta_2 \in [0, 2\pi)$ and $\theta_3, ..., \theta_n \in (0, \pi)$.
    \item Defn. For $\vv, \ww \in \R^n$ define signed counterclockwise angle $\theta_s(\vv, \ww)$ to be the tuple $(\theta_s(\vv, \ww), \Delta \theta_2, ..., \Delta \theta_n)$, where $\Delta \theta_i$ is the difference between the $\theta_i$ of $\ww$ and the $\theta_i$ of $\vv$. Define $\tttheta_s(\vv, \ww)$ to be $-\theta_s(\vv, \ww) \bmod 2\pi$.
    \item Defn. Define orientation of an ordered basis $(\ee_1, ..., \ee_n)$ of $\R^n$ to be an ordered tuple of signed angles, i.e., $(n - 1) \times (n - 1)$ matrix of signs. The $ij$th sign is $1$ iff $|\theta_s(\ee_i, \ee_{i + 1})_j| < |\tttheta_s(\ee_i, \ee_{i + 1})_j|$ and is $-1$ otherwise. 
\end{itemize}
