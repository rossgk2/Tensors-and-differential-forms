...
\begin{itemize}
    \item overall goal: figure out what the angle $\theta$ is in the dot product formula. is it signed or unsigned? is it somehow both, because it's in a domain that, $\bmod 2\pi$, is both? or is it only one? how does the one-to-one restriction of cosine come into play?
\end{itemize}
