...
\textbf{CCW and CW angle}
\begin{itemize}
    \item definition: signed CCW angle $\theta_s$
    \item signedness of CCW signed angle
    \begin{itemize}
        \item definition: $\bmod n$ for $n \in \Z \cap [0, \infty)$, $\bmod T$ for $T \in [0, \infty)$
        \begin{itemize}
            \item idea for $\bmod T$. when $a, n \in \Z$ can write $a/n = q + r/n$ for integers $q$ and $r$, where $r \in [0, n)$. sim., when $a, T \in \R$ can write $a/T = q + r/T$ for integer $q$ and real $r \in [0, T)$.
        \end{itemize}
        \item remark: moduli of negative integers
        \item theorem: $\theta_s(\vv, \ww) = -\theta_s(\ww, \vv)$
    \end{itemize}
    \item definition: signed CW angle $\widetilde{\theta}_s$
...
\textbf{trigonometry}
\begin{itemize}
    \item definition: $(\cos(t), \sin(t))^\top := \cc(t)$
    \item theorem: $\theta_s(\see_1, \cc(t)) = \theta_s(\see_1, (\cos(t), \sin(t))^\top) = t$
    \item theorem: $\cos$ and $\sin$ as ratios of lengths sides of triangles
    \item definition: $\arccos$ and $\arcsin$ as inverses of one-to-one restrictions of $\cos$ to $[0, \pi]$ and $\sin$ to $[-\frac{\pi}{2}, \frac{\pi}{2}]$.
\end{itemize}

\textbf{unsigned angle}
\begin{itemize}
    \item theorem: signed CCW angle as a minimum
    \item definition: unsigned angle
    \item $\theta_u(\vv, \ww) = \theta_u(\ww, \vv)$; reminder of how this makes $\theta_u$ the natural notion of angle between vectors
\end{itemize}

\textbf{make sure this discussion on angle is given in dot product section}
\begin{itemize}
