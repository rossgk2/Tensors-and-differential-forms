...
\subsubsection*{Angle and rotations}

Angle

\begin{itemize}
    \item overall goal: figure out what the angle is in the dot product formula.
    \begin{itemize}
        \item seems to go like this. first, prove that $\sproj(\vv \rightarrow \ww) = ||\vv||\cos(\theta_s(\vv, \ww)) = ||\vv||\cos(\theta_u(\vv, \ww))$. since $\cos$ is even and $\theta_u(\vv, \ww) = \pm \theta_s(\vv, \ww)$ then $\cos(\theta_u(\vv, \ww)) = \cos(\pm \theta_s(\vv, \ww)) = \cos(\theta_s(\vv, \ww))$.

        once we have defined the dot product, we have $\hvv \cdot \hww = \cos(\theta_s(\vv, \ww) = \cos(\theta_u(\vv, \ww))$. obviously then $\theta_s(\vv, \ww) = \arccos(\hvv \cdot \hww) = \theta_u(\hvv, \hww)$. how can we have $\theta_s(\vv, \ww) = \theta_u(\vv, \ww)$ here? [has something to do with identifications mod $2\pi$]
    \end{itemize}
\end{itemize}

Rotations- three overall goals-
