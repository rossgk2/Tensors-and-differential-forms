...
\begin{theorem}
    ...
    Let $V$ be an $n$-dimensional vector space with basis $E = \{\ee_1, ..., \ee_n\}$. When $k \leq n$, then $\Lambda^k(V)$ is a $\binom{n}{k}$ dimensional vector space with basis
...
\begin{proof}
     Use the seeming-multilinearity of $\wedge$, as was done for $\otimes$ in the proof of Theorem \ref{ch::motivated_intro::thm::basis_dim_tensor_product_space}, to show the proposed basis spans $\Lambda^k(V)$ for all $k$.
   
    To show the set is linearly independent, consider the following cases.

    (Case: $k > n$). When $k > n$, then every basis element is a wedge product in which at least one vector is repeated. Since a wedge product with a repeated vector is the zero wedge product, then when $k > n$ all vectors in the proposed basis are the zero wedge product, and the proposed basis is $\{\mathbf{0}\}$, which is not linearly independent. So the proposed basis is not a basis when $k < n$. The proposed basis still spans $\Lambda^k(V)$, though, so we can conclude that $\Lambda^k(V) = \{\mathbf{0}\}$.

    (Case: $k = n$). If $i_1 < ... < i_n$ and $i_1, ..., i_n \in \{1, ..., n\}$, then we must have $i_1 = 1, ..., i_n = n$. Therefore, the proposed basis is $\{\ee_1 \wedge ... \wedge \ee_n\}$. Since a singleton set is linearly independent iff it is not $\{\mathbf{0}\}$, then the proposed basis is linearly independent.

    (Case: $k \leq n$). Suppose that 

    \begin{align*}
        \sum_{i_1 < ... < i_k} T_{i_1 ... i_k} \ee_{i_1} \wedge ... \wedge \ee_{i_k} = \mathbf{0}.
    \end{align*}

    We must show that $T_{i_1 ... i_k} = 0$ for all $i_1, ..., i_k \in \{1, ..., n\}$.

    To do so, note that in general, if $\TT_1, \TT_2 \in \Lambda^k(V)$ and $\ww \in V - \{\mathbf{0}\}$, then we have ${\TT_1 = \TT_2 \iff \TT_1 \wedge \ww = \TT_2 \wedge \ww}$. Therefore, if we select some $\ee_{i_j}$ from one of the wedge products appearing in the sum, then we can obtain an equivalent equation by applying the map $\TT \mapsto \TT \wedge \ee_{i_j}$ to both sides of the above:

    \begin{align*}
        \sum_{i_1 < ... < i_k} T_{i_1 ... i_k} (\ee_{i_1} \wedge ... \wedge \ee_{i_k}) \wedge \ee_{i_j} = \mathbf{0}.
    \end{align*}

    Since there are terms in the above sum containing $\ee_{i_j}$ in their wedge products

    

    
    There is some $i_j$ such that some wedge product in the sum does not contain $\ee_{i_j}$ and some other wedge product in the sum does. 
    
    Take the exterior product of the sum with $\ee_{i_j}$ to obtain a linear dependence relation among a smaller number of elementary wedge products in $\Lambda^{k + 1}(V)$. By repeating this argument over and over we obtain a linear dependence relation between elementary wedge products in $\Lambda^n(V)$.
\end{proof}
