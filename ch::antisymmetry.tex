...
\label{ch::antisymmetry::defn::determinant}
    ...
    \begin{enumerate}
        ...
        \end{itemize}
        \item $\det(\cc_1, ..., \cc, ..., \cc, ..., \cc_n) = 0$ for all $\cc \in K^n$. This should hold because when two sides of a parallelapiped coincide, its \textit{$n$-dimensional} volume is zero.
    \end{enumerate}
\end{defn}
...
\begin{deriv}
    ...
    Let $\cc, \dd \in K^n$. By the third axiom of the determinant, we have $\det(\cc_1, ..., \cc, ..., \dd, ..., \cc_n) + \det(\cc_1, ..., \dd, ..., \cc, ..., \cc_n) = \det(\cc_1, ..., \cc + \dd, ..., \dd + \cc, ..., \cc_n) = 0$. Thus,

    \begin{align*}
        \det(\cc_1, ..., \cc, ..., \dd, ..., \cc_n) = -\det(\cc_1, ..., \dd, ..., \cc, ..., \cc_n) \text{ for all $\cc, \dd \in K^n$}.
    \end{align*}
    ...
    Notice that the alternatingness property of the determinant implies that there exist $\cc_1, ..., \cc_n \in K^n$ with $\det(\cc_1, ..., \cc_n) < 0$. So, we see our intuitive assumptions about volume require that volume be \textit{signed}. In Theorem [...] we will see how signed volume corresponds to the concept of orientation.
\end{deriv}
...
\begin{theorem}
    ...
    The determinant is the unique\footnote{We have not yet explained why a function satisfying the determinant axioms is unique. We do so in Remark [...].} alternating linear $k$-form sending $\sE \mapsto 1$. (Recall Definition \ref{ch::bilinear_forms_metric_tensors::defn::linear_k_form}).
\end{theorem}
...
\begin{proof}
    ...
    \setcounter{enumi}{4}
        ...
        Using the axiom ``$\det(\cc_1, ..., \cc, ..., \cc, ..., \cc_n) = 0$ for all $\cc \in K^n$'' together with the multilinearity of the determinant, we have
        
        \begin{align*}
            \det(\cc_1, ..., \cc_i, ..., \cc_n)
            &= \det(\cc_1, ..., \cc_i, ..., \cc_j, ..., \cc_n) + \sum_{j = 1, j \neq i}^n d_j \det(\cc_1, ..., \cc_{i - 1}, \cc_j, \cc_{i + 1}, ..., \cc_j, ..., \cc_n) \\
...
\begin{deriv}
    ...
    We now derive \textit{permutation formula} for the determinant. The formula we obtain shows that the function $\det$ specified in Definition \ref{ch::antisymmetry::defn::determinant} exists; since this formula is derived from the axioms of the determinant, it is also a unique formula for the determinant.
...
\begin{remark}
    ...
    Since the boxed formula was derived by assuming the axioms of the determinant, and since the right side of the boxed formula is a well-defined function of every variable it involves, then the determinant is the unique function satisfying the axioms. Thus, the determinant is the unique alternating linear $k$-form sending $\sE \mapsto 1$.
\end{remark}
...
\label{ch::antisymmetry::lemma:det_as_scale_factor}
    ...
    Let $V$ and $W$ be vector spaces over the same field. Let $\hh$ be an alternating linear $k$-form. If $\ff:V \rightarrow W$ is a linear function, then $h(\vv_1, ..., \vv_k) = \det(\ff) h(\ff(\vv_1), ..., \ff(\vv_k))$.
\end{lemma}
...
\begin{proof}
   ...
   Using the seeming-multilinearity of $\wedge$, we can extend this fact to apply to any list of vectors in $V$ and obtain the lemma.
\end{proof}
...
We restate an earlier definition for convenience.
...
\begin{deriv}
    ...
    We have a way to represent multilinear functions as vectors. (The way: tensors). Given that we've seen multilinear alternating functions are also highly relevant [determinant], we now seek a way to represent such functions as vectors.

    Just as multilinear functions correspond to symbols of the form $\vv_1 \otimes ... \otimes \vv_k$, where $\otimes$ is multilinear, we will have multilinear alternating functions correspond to symbols of the form $\vv_1 \wedge ... \wedge \vv_k$ such that
    
    \begin{enumerate}
        \item $\wedge$ looks as if it is bilinear, just as was the case with $\otimes$. That is, [...]
        
        \item $\wedge$ is associative, just as was the case with $\otimes$: [...]
        
        \item $\wedge$ looks as it it is an antisymmetric map: [...]
        ...
        \item $\vv_1 \wedge ... \vv \wedge ... \wedge \vv \wedge ... \wedge \vv_k = \mathbf{0}$ for all $\vv \in V_i$ and all $i$
    \end{enumerate}

    We define $V_1 \wedge ... \wedge V_k$ to be the vector space spanned by elements of the form $\vv_1 \wedge ... \wedge \vv_k$, where $\vv_1 \in V_1, ..., \vv_k \in V_k$.
\end{deriv}
...
\begin{theorem}
    (Wedge product spaces are spaces of antisymmetric tensors).
\end{theorem}
...
\begin{theorem}
    ...
    as before, $V_1 \wedge ... \wedge V_k$ is defined to be the vector space of elements spanned by elements of the form $\vv_1 \wedge ... \wedge \vv_k$, where $\vv_1 \in V_1, ..., \vv_k \in V_k$.
\end{theorem}
...
\label{ch::antisymmetry::rmk::top_pushforward_det}
    ...
    Now consider the pushforward $\Lambda^n \ff: \Lambda^n(V) \rightarrow \Lambda^n(W)$ on the top exterior power, which (recall Definition \ref{ch::antisymmetry::defn::pushforward_on_exterior_pwr}) is defined on elementary tensors by $\vv_1 \wedge ... \wedge \vv_n \mapsto \ff(\vv_1) \wedge ... \wedge \ff(\vv_n)$ and is extended with multilinearity. Defined this way, the pushforward $\Lambda^n \ff$ is a multilinear alternating map. This pushforward is also a map of 1-dimensional vector spaces, so it must be multiplication by a constant. We will determine what this constant is.
    ...
    \textbf{had to introduce this fact in a different form earlier to prove det product rule. so, maybe use this lemma to prove this theorem: Lemma \ref{ch::antisymmetry::lemma:det_as_scale_factor}.}
...
\label{ch::antisymmetry::deriv::pullback_on_exterior_pwr_of_actual_fns}
    ...
    In appearance, it seems that $\tLambda^k \ff^*$ acts on $\vv_1, ..., \vv_k$ exactly as does $\totimes_{0,k} \ff^*$, since $\totimes_{0,k} \ff^*(\vv_1, ..., \vv_k) = \TT(\ff(\vv_1), ..., \ff(\vv_k))$. The distinction between the two definitions is that $\tLambda^k \ff^*$ acts on multilinear alternating functions $\TT$, while $\totimes_{0,k} \ff^*$ acts on multilinear (not necessarily alternating) functions $\TT$.
\end{deriv}
