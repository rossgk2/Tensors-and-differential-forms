...
\label{ch::lin_alg::defn::length_in_R2}
    ...
    Let $\vv \in \R^2$. If we interpret $([\vv]_\sE)_1$ and $([\vv]_\sE)_2$ as the lengths of the base and height, respectively, of a right triangle, then the length of the hypotenuse of this right triangle is $\sqrt{([\vv]_\sE)_1^2 + ([\vv]_\sE)_2^2}$.

    Since $\vv = ([\vv]_\sE)_1 \see_1 + ([\vv]_\sE)_2 \see_2$, it makes sense to think of the hypotenuse as being $\vv$, and thus, to define the \textit{length} (or \textit{magnitude}) $||\vv||$ of $\vv$ to be the length of the hypotenuse: $||\vv|| := \sqrt{([\vv]_\sE)_1^2 + ([\vv]_\sE)_2^2}$.
\end{defn}
...
\begin{defn}
    (Nonnegative angle between vectors in $\R^2$).
    
    Let vectors $\vv, \ww \in \R^2$ have the same length $r = ||\vv|| = ||\ww||$, suppose the initial points of $\vv$ and $\ww$ coincide, and the circle of radius $r$ that has the coinciding initial points of $\vv$ and $\ww$ as its center. Let $s(\vv, \ww)$ be the length of the shortest path on the circle from $\vv$ to $\ww$:

    \begin{align*}
        s(\vv, \ww) = \int_{t_1}^{t_2} \Big|\Big|\frac{d\xx(t)}{dt}\Big|\Big| dt, &\text{ where $\xx(t) = r\begin{pmatrix} \cos(t) \\ \sin(t) \end{pmatrix}$,} \\ &\text{ $\xx(t_1) = \vv$, $\xx(t_2) = \ww$}, t_2 \geq t_1.
    \end{align*}
    
    \textbf{find and replace ``unsigned''.}
    \textbf{$t \in [0, 2\pi)$ should probably be $t \in [0, \pi]$}

    We define the \textit{nonnegative\footnote{Notice that because the integrand for $s$ is nonnegative and $t_2 \geq t_1$, we have $s(\vv, \ww) \geq 0$. Since $s(\vv, \ww) \geq 0$ and $r \geq 0$, we have $\theta(\vv, \ww) \geq 0$.} angle $\theta(\vv, \ww)$ between $\vv$ and $\ww$} to be the ratio $\theta(\vv, \ww) := \frac{s(\vv, \ww)}{r}$. When $\vv$ and $\ww$ don't have the same length, we define $\theta(\vv, \ww) := \theta(r_0\hvv, r_0\hww)$, where $r_0 > 0$ is any positive real number.
\end{defn}
...
\label{ch::lin_alg::thm::unsigned_angle_Rn}
    ...
    Let $\vv, \ww \in \R^2$. Then
    
    \begin{align*}
        \theta(\vv, \ww) = \text{arccos}\Big( \sum_{i = 1}^2 ([\hvv]_\sE)_i  ([\hww]_\sE)_i \Big).
    \end{align*}

    Note, since $\arccos:[-1, 1] \rightarrow [0, \pi]$, we have $\theta(\vv, \ww) \in [0, \pi]$.
\end{theorem}

\begin{proof}
    We prove the theorem in the case when $||\vv|| = ||\ww|| = r$; the general case follows from this easily. We have
    
    \begin{align*}
       \theta(\vv, \ww) = \frac{s}{r} = \frac{1}{r} \int_{t_1}^{t_2} \Big|\Big| \frac{d\xx(t)}{dt}\Big|\Big| dt, &\text{ where $\xx(t) = r\begin{pmatrix} \cos(t) \\ \sin(t) \end{pmatrix}$,} \\ &\text{ $\xx(t_1) = \vv$, $\xx(t_2) = \ww$, $t_2 \geq t_1$}.
    \end{align*}

    Since $||\frac{d\xx(t)}{dt}|| = r$, then $\theta(\vv, \ww) = t_2 - t_1$. Further, since $\xx(t_1) = r \hvv$ and $\xx(t_2) = r \hww$, we have ${r \cos(t_1) &= (r[\hvv]_\sE)_1}$ and ${r \cos(t_2) &= (r[\hww]_\sE)_1}$, giving us
   
   \begin{align*}
       \theta(\vv, \ww) = t_2 - t_1 
       = \text{arccos}\Big(([\hww]_\sE)_1\Big) - \text{arccos}\Big(([\hvv]_\sE)_1\Big).
   \end{align*}
   
   The cosine angle addition identity $\cos(\alpha + \beta) = \cos(\alpha) \cos(\beta) - \sin(\alpha) \sin(\beta)$ implies\footnote{Solve the first identity for $\gamma = \cos(\alpha)$ and $\delta = \cos(\beta)$ to obtain the second identity in terms of $\gamma$ and $\delta$.} the identity \\ ${\text{arccos}(\alpha) + \text{arccos}(\beta) = \text{arccos}(\alpha \beta - \sqrt{1 - \alpha^2}\sqrt{1 - \beta^2})}$. Using this identity, we have

    \begin{align*}
       \theta(\vv, \ww) &= \text{arccos}\Bigg(([\hvv]_\sE)_1 ([\hww]_\sE)_1 + \sqrt{1 -([\hvv]_\sE)_1^2} \sqrt{1 - ([\hww]_\sE)_1^2} \Bigg) \\
       &= \text{arccos}\Bigg(([\hvv]_\sE)_1 ([\hww]_\sE)_1 + ([\hww]_\sE)_2 ([\hww]_\sE)_2 \Bigg).
   \end{align*}
\end{proof}
...
\subsubsection{Rotation in $\R^2$}

\begin{lemma}
    (Rotation in $\R^2$).

    Let $\theta \in [0, \pi]$. The counterclockwise rotation $\RR_t:\R^2 \rightarrow \R^2$ by $\theta$ is given by
...
\begin{proof}
    To obtain the formula for the standard matrix of $\RR_t$, use trigonometry to show that $\RR(\see_1) = (\cos(\theta), \sin(\theta))^\top$ and $\RR(\see_2) = (-\sin(\theta), \cos(\theta))^\top$.
\end{proof}
...
\begin{proof}
    Compute the left side as a matrix-vector product and then use the trigonometric angle-sum identities.
\end{proof}

\begin{lemma}
    The previous lemma implies that $\RR_\theta$ is length-preserving. Because $\RR_\theta$ is length-preserving, we have $\widehat{\RR_\theta(\vv)} = \RR_\theta(\hvv)$.
\end{lemma}

\begin{proof}
    We have $\widehat{\RR_\theta(\vv)} = \frac{\RR_\theta(\vv)}{||\RR_\theta(\vv)||} = \frac{\RR_\theta(\vv)}{||\vv||} = \RR_\theta\Big(\frac{\vv}{||\vv||}\Big) = \RR_\theta(\hvv)$.
\end{proof}
...
\begin{lemma}
    ...
    \begin{align*}
        \{ \ww \mid \theta(\hvv, \hww) = t\} = \{\RR_{-t}(\hvv), \RR_t(\hvv)\}.
    \end{align*}
\end{lemma}

\begin{proof}
    First, we show $\theta(\hvv, \widehat{\RR_t(\vv)}) = \theta(\hvv, \RR_t(\hvv)) = t$ for all $t \in [0, \pi]$. We have $\hvv = (\cos(s), \sin(s))^\top$ for some $s \in [0, 2\pi)$, and have $\RR_t(\hvv) = (\cos(s + t), \sin(s + t))^\top$ by Lemma [...]. The proof of Theorem \ref{ch::lin_alg::thm::unsigned_angle_Rn} showed that in general, 

    \begin{align*}
        \theta(\huu_1, \huu_2) = t_2 - t_1, \text{ where $\xx(t) = \begin{pmatrix} \cos(t) \\ \sin(t) \end{pmatrix}, \xx(t_1) = \huu_1, \xx(t_2) = \huu_2$}.
    \end{align*}
    
    Using $\huu_1 = \hvv$, $\huu_2 = \RR_t(\hvv)$, $t_1 = s$, $t_2 = s + t$, we have $\theta(\hvv, \RR_t(\hvv)) = (s + t) - s = t$.

    Now we show uniqueness. Suppose $\theta(\hvv, \hww) = t$; we need to show that $\hww = \RR_t(\hvv)$. It suffices to show that $\theta(\hvv, \cdot):\R^2 \rightarrow [0, \pi]$ is one-to-one. Well, 
...
\begin{proof}
    The previous lemma allows us to represent $\vv_\perp$
\end{proof}

\newpage

\subsubsection*{The dot product in $\R^2$}

Now that we have a notion of perpendicularity between vectors in $\R^2$, we can define the notion of vector projection, which is the main idea involved in the dot product.
...
\begin{defn}
    ...
    The \textit{dot product in $\R^2$} is the function $\cdot:\R^2 \times \R^2 \rightarrow \R$ defined by
    
    \begin{align*}
        \vv \cdot \ww := \sproj(\vv \rightarrow \ww) \spc ||\ww||.
    \end{align*}
    
    Why do we care about the dot product? The primary reason is that when $||\ww|| = 1$, we have ${\vv \cdot \ww = \sproj(\vv \rightarrow \ww)}$, so investigating the dot product can tell us more about vector projections\footnote{We will indeed find that the dot product tells us something about projections in Theorem \ref{ch::lin_alg::thm::vector_proj_dot_product}!}. Another reason is that in physics, the \textit{work} done by a force $\FF$ along a displacement $\Delta \xx$ is defined to be the product of the magnitude of the force that ``aligns'' with the displacement with the magnitude of the displacement\footnote{A negative sign occurs when the force opposes the displacement.}; i.e., work is defined to be $\FF \cdot \Delta \xx$.
\end{defn}
...
\label{ch::lin_alg::thm::length_in_Rn}
    (Length in $\R^2$ in terms of dot product).
    
    For any $\vv \in \R^2$, we have $||\vv|| = \sqrt{\vv \cdot \vv}$.
\end{theorem}

\begin{proof}
   $\vv \cdot \vv = \sproj(\vv \rightarrow \vv) \spc ||\vv|| = ||\vv||^2$, so $\sqrt{\vv \cdot \vv} = ||\vv||$.
\end{proof}
...
We could have proved the following theorem immediately after defining $\vv \cdot \ww := \sproj(\vv \rightarrow \ww) \spc ||\ww||$. The usefulness of this next theorem, however, wouldn't have been apparent until knowing that $\vv \cdot \ww = \sum_{i = 1}^n ([\vv]_\sE)_i ([\ww]_\sE)_i$.

\begin{theorem}
\label{ch::lin_alg::thm::vector_proj_dot_product}
    (Vector projection  in $\R^2$ in terms of dot product in $\R^2$).
    
    If $\vv, \ww \in \R^2$, then
    
    \begin{align*}
        \boxed
        {
            \proj(\vv \rightarrow \ww) = \frac{\vv \cdot \ww}{||\ww||} \hat{\vv}_2 = \frac{\vv \cdot \ww}{\ww \cdot \ww} \ww
        }
    \end{align*}
    
    If we didn't know the algebraic formula for the dot product, this theorem would be a bit of a tautology. However, knowing the algebraic dot product allows for easy computation of the expressions $\vv \cdot \ww$ and $\ww \cdot \ww$ whenever we know the coordinates of $\vv$ and $\ww$ relative to the standard basis\footnote{Actually, we only need to know the coordinates of $\vv$ and $\ww$ relative to some \textit{orthonormal} basis! Orthonormal bases, also known as \textit{self-dual bases}, are discussed in Section \ref{ch::bilinear_forms_metric_tensors::section::self-duality}.} for $\R^2$.
\end{theorem}
...
\label{ch::lin_alg::thm::dot_prod_and_angle}
    (Unsigned angle between vectors in $\R^2$ and dot product in $\R^2$).
    
    Before we discovered the algebraic dot product, Theorem \ref{ch::lin_alg::thm::unsigned_angle_Rn} showed 
    
    \begin{align*}
        \theta = \text{arccos}\Big(\sum_{i = 1}^2 ([\vv]_\sE)_i ([\ww]_\sE)_i\Big),
    \end{align*}
    
    as a consequence of the definition $\theta := \frac{s}{r}$. Now that we know about the algebraic dot product, we can say that the signed angle $\theta$ between $\vv, \ww \in \R^n$ is
    ...
    \begin{align*}
        \vv \cdot \ww = ||\vv|| \spc ||\ww|| \cos(\theta).
    \end{align*}
\end{theorem}

Now that we have used the geometric definition of the dot product in $\R^2$ to derive an ``algebraic'' formula for the dot product in $\R^2$, we will show that one can start with the ``algebraic'' formula in $\R^2$ as a definition and discover the geometric formula in $\R^2$. We need the following definition and two lemmas before we can do this.

\begin{defn}
\label{ch::lin_alg::defn::orthogonal_linear_fn_Rn}
    (Orthogonal linear function on $\R^2$).
...
\label{ch::lin_alg::lemma::orthogonal_linear_fns_preserve_alg_dot_product}
    ...
    Let $\ff:\R^n \rightarrow \R^n$ be an orthogonal linear function. Then $\ff$ preserves the algebraic dot product in $\R^n$: $\vv_1 \cdot \vv_2 = \ff(\vv_1) \cdot \ff(\vv_2)$.
\end{lemma}
...
\begin{proof}
    ...
    where the last equality is by Lemma \ref{ch::lin_alg::lemma::rotated_projection_length}. \\ Therefore $\vv_1 \cdot \vv_2 = ||\proj(\vv_1 \rightarrow \vv_2)|| \spc ||\vv_2||$, which is the definition of the geometric dot product in $\R^n$.
\end{proof}
