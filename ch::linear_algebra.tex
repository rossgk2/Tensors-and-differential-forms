...
The first theorem in this chapter recursively characterizes the problem of solving linear equations, telling us that in order to solve $(\ff(\vv) = \ww \text{ for all $\vv \in V$})$, we have to solve another linear equation, $(\ff(\vv) = \mathbf{0} \text{ for all $\vv \in V$})$. Thus, the problem of solving linear equations remains unsolved at this point.

We address this problem now. In order to solve a linear equation, we will transform it into an equivalent system of linear equations, and solve that.
...
\begin{deriv}
    ...
    Furthermore, since for $\ff:K^n \rightarrow K^m$ we have $\ff(\vv) = \ff(\sE) \vv$ for all $\vv \in K^n$, and since the columns of $\ff(\sE)$ vary over $K^m$ (recall Theorem \ref{ch::lin_alg::thm::basis_sent_to_any_ordered_list}), we can study equations of the above form by studying equtions of the form

    \begin{align*}
        \AA \vv = \ww \text{ for all $\vv \in K^n$, where $\AA \in K^{m \times n}$ and $\ww \in K^m$}.
    \end{align*}
    
    Equations of the above form are called \textit{(linear) matrix equations}\footnote{These equations are most commonly called \textit{matrix equations}.}. Traditionally, they are typically written with $\xx \in K^n$ instead of $\vv \in K^n$ and $\bb \in K^m$ instead of $\ww \in K^m$:

    \begin{align*}
        \AA \xx = \bb \text{ for all $\xx \in K^n$, where $\AA \in K^{m \times n}$ and $\bb \in K^m$}.
    \end{align*}
    ...
    which is equivalent to the system of linear\footnote{Each equation in this system is itself a linear equation because the function $(x_1, ..., x_n) \mapsto x_1 a_{i1} + ... + x_n a_{in}$ is linear for all $i$.} equations
...
\begin{deriv}
    ...
    Consider $\rref(\AA) = (b_{ij}) := \rref(\AA)$.

    
    Let $x_{j_1}, ..., x_{j_p}$ be the pivot variables, $y_{k_1}, ..., y_{k_q}$ be the free variables, and consider the system of equations $\rref(\AA) \xx = \mathbf{0}$, where $\xx = (x_{j_1}, ..., x_{j_p}, y_{k_1}, ..., y_{k_q})^\top$ 

    [same as considering $\rref(\AA) (x_1, ..., x_n)^\top = \mathbf{0}$]
    
    [???]
    
    \begin{align*}
        \begin{cases}
            x_{j_1} + \sum_{\alpha = 1}^q b_{j_1 k_\alpha} y_{k_\alpha} = 0 \\
            \vdots \\
            x_{j_p} + \sum_{\alpha = 1}^q b_{j_p k_\alpha} y_{k_\alpha} = 0  \\
            y_{k_1} = y_{k_1} \\
            \vdots \\
            y_{k_q} = y_{k_q} \\
        \end{cases}.
    ...
    [better notation than ``$y_{k_\alpha} = y_{k_\alpha}$''?] This system is equivalent to

    \begin{align*}
        \begin{cases}
            x_{j_1} = -\sum_{\alpha = 1}^q b_{j_1 k_\alpha} y_{k_\alpha} \\
            \vdots \\
            x_{j_p} = -\sum_{\alpha = 1}^q b_{j_p k_\alpha} y_{k_\alpha}  \\
            y_{k_1} = y_{k_1} \\
            \vdots \\
            y_{k_q} = y_{k_q} \\
        \end{cases},
    ...
    \begin{align*}
        ...
        =
        \begin{pmatrix}
            -\sum_{\alpha = 1}^q b_{j_1 k_\alpha} y_{k_\alpha} \\
            -\sum_{\alpha = 1}^q b_{j_2 k_\alpha} y_{k_\alpha} \\
            \vdots \\
            -\sum_{\alpha = 1}^q b_{j_p k_\alpha} y_{k_\alpha} \\
            t_1 \\
            t_2 \\
            \vdots \\
            t_q
    ...
    \begin{align*}
        \left\{
        \begin{pmatrix} \cc_1 \\ \see_1 \end{pmatrix},
        \begin{pmatrix} \cc_2 \\ \see_2 \end{pmatrix},
        ...
        \begin{pmatrix} \cc_q \\ \see_q \end{pmatrix}
        \right\},
    \end{align*}

    where $\cc_\alpha \in K^p$, $\see_\alpha \in K^q$ and where $\begin{pmatrix} \cc_\alpha \\ \see_\alpha \end{pmatrix}$ denotes the vector in $K^{p + q}$ formed by ``stacking'' $\cc_\alpha$ above $\see_\alpha$.

    Since $\see_1, ..., \see_q$ are linearly independent, it follows that this set of vectors is linearly independent, too. Since the dimension of $\ker(\AA)$ is equal to the number of free variables, $\dim(\ker(\AA)) = q$, then this set is a linearly independent set of $\dim(\ker(\AA))$ many vectors, and is thus a basis for $\ker(\AA)$.
\end{deriv}
...
\begin{enumerate}    
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            (Case: $\rref(\AA)$ has pivot columns). We have $\rref(\AA) = (\see_1, ..., \see_k, \bb_{k + 1}, ..., \bb_n)$ for some integer $k > 0$. Since row-reduction is an invertible linear operation, there is an invertible matrix $\RR$ for which $\rref(\AA) = \RR \AA$. We thus have $\AA = \RR^{-1} \rref(\AA) = (\RR^{-1}\see_1, ..., \RR^{-1}\see_k, \RR^{-1}\bb_{k + 1}, ..., \RR^{-1}\bb_n)$. Recall from Theorem \ref{ch::lin_alg::lemma::only_inv_fns_preserve_lin_indep} that because $\RR^{-1}$ is invertible it preserves linear independence. Thus, since $\see_1, ..., \see_k$ are linearly independent, $\aa_1 = \RR^{-1} \see_1, ..., \aa_k = \RR^{-1} \see_k$ are linearly independent.
