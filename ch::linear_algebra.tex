...
\begin{remark}
    ...
    \begin{itemize}
        \item Suppose that we use the above definition of \textit{point} and attempt to use it to give a definition of \textit{vector} that is equivalent to the above definition of \textit{vector}. The attempted definition is: a \textit{vector} $\vv_2 - \vv_1$ is the difference of the vectors $\vv_2 - \pp$ and $\vv_1 - \pp$, where $\pp$ is any point. This definition is circular because it involves \textit{vector}.
        
        \item Suppose that we use the above definition of \textit{vector} and attempt to use it to give a definition of \textit{point} that is equivalent to the above definition of \textit{point}. The attempted definition is: a \textit{point} $\pp$ is a sum of the form $(\qq + \vv_1) + \vv_2) + ...) + \vv_n$, where $\qq$ is any point, each $\vv_i$ is a vector, and where the sum of a point and a vector is point obtained by starting on the point and ``traveling'' according to the vector. This definition is circular because it involves \textit{point}.
    \end{itemize}
    
    Fortunately, we can escape this circular logic by defining each of \textit{point} and \textit{vector} in terms of a more primitive mathematical object. Unfortunately, the above detailed logical circularities will always persist on the intuitive level.
\end{remark}
