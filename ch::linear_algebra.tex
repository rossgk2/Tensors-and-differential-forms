...
We can provide a sensible way to realize this intertwinedness and escape circular logic by defining each of \textit{point} and \textit{vector} in terms of a more primitive mathematical object. The above detailed intertwinedness and logical circularities will always persist on the intuitive level, though. Whether this is fortunate or unfortunate is a matter of opinion!
...
\begin{proof}
    \mbox{} \\
    \begin{enumerate}
        \item The claims about the additive identity and additive inverses follow from the fact that $0$ is the additive identity in $\R$ and that the additive inverse of $x \in \R$ is denoted $-x$. The associativity and commutativity of $+$ follow from the associativity and commutativity of addition of real numbers.
...
\begin{remark}
    ...
    The above definition easily implements the intertwinedness of points and vectors; since every point in $\R^n$ can also be interpreted as a vector in $\R^n$, the identity function associates points with vectors.
\end{remark}
...
\begin{remark}
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item ($\mathbf{0}$ as a point). When $\mathbf{0}$ is considered to be a point, we interpret it to be the origin, i.e., the ``central reference point''.
