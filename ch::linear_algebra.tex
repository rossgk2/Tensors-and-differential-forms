...
The above addition operation $+$ and scalar multiplication operation $\cdot$ are algebraically sensible because they obey properties one would expect of an ``addition operation'' and a ``multiplication operation''.
...
\begin{comment} % Unnecessary for proof of next theorem since we can just use main dimension theorem
    \begin{lemma}
    \label{ch::lin_alg::lemma::linear_isos_provide_bases}
       (Linear isomorphisms provide bases).
        
       Let $V$ and $W$ be finite-dimensional vector spaces, and suppose $\ff:V \rightarrow W$ is a linear isomorphism. If $E = \{\ee_1, ..., \ee_n\}$ is a basis for $V$, then $\ff(E) = \{\ff(\ee_1), ..., \ff(\ee_n)\}$ is a basis for $W$. 
        
       As a corollary, we see that finite-dimensional vector spaces that are isomorphic to each other must have the same dimension.
    \end{lemma}
    
    \begin{proof}
      Since $\ff$ is onto, then for all $\ww \in W$ there exists a $\vv \in V$ for which $\ff(\vv) = \ww$. Let $E = \{\ee_1, ..., \ee_n\}$ be a basis for $V$. Then for all $\ww \in W$ we have $\ww = \ff(\sum_{i = 1}^n ([\vv]_E)_i \ee_i) = \sum_{i = 1}^n ([\vv]_E)_i \ff(\ee_i)$. Thus, the $\ff(\ee_i)$ span $W$. Since $E$ is a basis and $\ff$ has a trivial kernel (it is one-to-one), then the $\ff(\ee_i)$ are linearly independent because one-to-one linear functions preserve the linear independence of vectors (see the previous lemma). Thus, $\{\ff(\ee_1), ..., \ff(\ee_n)\}$ is a basis for $W$.
    \end{proof}
\end{comment}
...
\begin{proof}
   ...
   Since $[\ee_i]_E = \see_i$, we have $[\cdot]^{-1}_E(\see_i) = \ee_i$ and\footnote{In Derivation \ref{ch::lin_alg::deriv::matrix_relative_to_bases} we used the facts from the sentence this footnote appears in to conclude $[\cdot]_E^{-1}(\sE) = E$ rather than $[\cdot]_E^{-1}(\sE) = \EE$. We are able to use the latter matrix notation $\EE$ now because we are in the special situation where $E$ is a basis of $K^n$, so the list $E$ of vectors is a list of \textit{column} vectors, i.e. a matrix.} thus $[\cdot]_E^{-1}(\sE) = \EE$. Also, since $[\cdot]_E$ is the inverse of $[\cdot]_E^{-1}$, we have $[\cdot]_E(\sE) = ([\cdot]_E^{-1}(\sE))^{-1} = \EE^{-1}$. Using the two facts $[\cdot]_E(\sE) = \EE^{-1}$ and $[\cdot]_E^{-1}(\sE) = \EE$, we conclude that $([\cdot]_E \circ \ff \circ [\cdot]_E^{-1})(\sE) = [\cdot]_E(\sE) \spc  \ff(\sE) \spc [\cdot]_E^{-1}(\sE) = \EE^{-1} \ff(\sE) \EE$.
\end{proof}

\begin{comment} % Unnecessary; doesn't add much
    \begin{theorem}
    \label{ch::lin_alg::thm::action_linear_fn_arbitrary_vector_via_matrix_relative_to_bases}
    
        (Action of linear function on arbitrary vector via matrix relative to bases).
    
        Let $V$ and $W$ be finite-dimensional vector spaces over a field with bases $E = \{\ee_1, ..., \ee_n\}$ and $F = \{\ff_1, ..., \ff_n\}$. If $\ff:V \rightarrow W$ is a linear function, then 
    
        \begin{align*}
            \ff(\vv) = \sum_{ij} ([\ff(E)]_F)_{ij} ([\vv]_E)_j \ff_i.
        \end{align*}
        
        As a corollary, we have $\ff(\ee_i) = \sum_{j = 1}^n ([\ff(E)]_F)_{ji} \ff_i$.
    \end{theorem}
    
    \begin{proof}
      To prove the main statement, start with $\ff(\vv) = \sum_{i = 1}^n ([\ff(\vv)]_F)_i \ff_i$. Since $[\ff(\vv)]_F = [\ff(E)]_F [\vv]_E$, then $([\ff(\vv)]_F)_i = ([[\ff(E)]_F [\vv]_E]_\sE)_i = (\text{$i$th row of $[\ff(E)]_F$}) \cdot [\vv]_E = \sum_{j = 1}^n ([\ff(E)]_F)_{ij} ([\vv]_E)_j$. Thus $\ff(\vv) = \sum_{ij} ([\ff(E)]_F)_{ij} ([\vv]_E)_j \ff_i$.
       
      For the corollary, use the main statement to obtain the following: $\ff(\ee_k) = \sum_{ij} ([\ff(E)]_F)_{ij} ([\ee_k]_E)_j \ff_i = \sum_{ij} ([\ff(E)]_F)_{ij} \delta_{kj} \ff_i = \sum_{i = 1}^n ([\ff(E)]_F)_{ik} \ff_i$. Rename $i$ to $j$ and $k$ to $i$ to obtain the result.
    \end{proof}
\end{comment}
...
\begin{theorem}
    ...
    % Old motivation from investigating a special case; feel that general way is elegant enough that investigating special case just adds verbosity
    
    % To start, consider the special case $V = K^n$. Then we can let $\EE$ and $\FF$ be the matrices with $i$th columns $\ee_i$ and $\ff_i$, respectively. For $\vv \in K^n$, we have $\FF [\vv]_F = \vv = \EE [\vv]_E$, so $[\vv]_F = \FF^{-1} \EE [\vv]_E$.
    
    % Now, we investigate the matrix $\FF^{-1} \EE$ of the previous expression.
    
    % By the definition of matrix-matrix multiplication (Definition \ref{ch::lin_alg::thm::matrix_matrix_product_relative_to_bases_standard}), we have
    ...
    % The matrix $[\EE]_F$ was the centerpiece of the above argument. To generalize, we notice that the matrix of $[\cdot]_F$ relative to $E$ and $\sE$ is $[\cdot]_F(E) = [\EE]_F$.
    
    % The characterizing property of matrices relative to bases then implies that for any $\vv \in V$, we have
    ...
    where $\EE$ is the list whose $i$th entry is $\ee_i$.
    
    \textbf{make consistent the convention where we say $\EE$ is the matrix whose $i$th entry is $\ee_i$, or the list whose $i$th entry is $\ee_i$, or where we don't say anything at all}
...
\begin{comment} %Unnecessary/impractical
    \begin{theorem}
...
\begin{deriv}
    ...
    In all, we see that $\vv \in V$ iff $\exists c \in K \st \vv \in \ker(\ff - c \II)$. Thus the set of eigenvectors of $\ff$ is equal to $\bigcup_{c \in K} \ker(\ff - c \II)$.
