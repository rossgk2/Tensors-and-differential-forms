...
\begin{theorem}
    ...
    Let $K$ be a field, consider the vector space $K^n$, and let $E$ be a basis of $K^n$. By Lemma \ref{ch::lin_alg::lemma::basis_inverse}, the standard matrix $[\cdot]_E^{-1}(\sE)$ of $[\cdot]_E^{-1}$ is $[\cdot]_E^{-1}(\sE) = \EE$ and the standard matrix $[\cdot]_E(\sE)$ of $[\cdot]_E$ is $[\cdot]_E(\sE) = \EE^{-1}$. Thus,
...
\begin{proof}
   The matrix $[\ff(E)]_E$ of $\ff$ relative to $E$ and $E$ is the standard matrix $\ff_{E,E}(\sE)$ of $\ff_{E,E} = [\cdot]_E \circ \ff \circ [\cdot]_E^{-1}$, which is $\ff_{E,E}(\sE) = [\cdot]_E(\sE) \spc \ff(\sE) \spc [\cdot]_E^{-1}(\sE)$. We know that $[\cdot]_E(\sE) = \EE^{-1}$ and $[\cdot]_E^{-1}(\sE) = \EE$ from Theorem \ref{ch::lin_alg::thm::basis_change_from_standard_vectors}, so the most recent expression is equal to $\EE^{-1} \ff(\sE) \EE$, as claimed.
\end{proof}

\begin{remark}
    (Similar matrices).

    Most linear algebra texts do not define the matrix of a linear function $V \rightarrow W$ relative to bases $E$ and $F$, and only define ``the matrix of a linear function $\ff:V \rightarrow V$ relative to the basis $E$'' to be $\EE^{-1} \ff(\sE) \EE$.

    In such texts, matrices $\AA$ and $\BB$ are called \textit{similar} iff there exists a matrix $\SS$ such that $\BB = \SS^{-1} \AA \SS$. This is an entirely reasonable definition. Unfortunately, though, many of these texts don't actually use that definition of similarity, and instead say: ``$\AA$ and $\BB$ are called \textit{similar} iff there exists a matrix $\SS$ such that $\BB = \SS \AA \SS^{-1}$''. Of course, since all matrices of the form $\SS^{-1} \AA \SS$ are also of the form $\SS \AA \SS^{-1}$, this later definition is equivalent to the former one. But it makes things more confusing, since the placement of the matrix inverse in the later definition is different than it is in the formula $[\ff(E)]_E = \EE^{-1} \ff(\sE) \EE$. This could lead one to think that ``$[\ff(E)]_E = \EE \ff(E) \EE^{-1}$'', which is not true!
\end{remark}
...
\begin{remark}
    ...
    Some authors explicitly disallow $\mathbf{0}$ from being an eigenvector of any linear function (and thus disallow $0$ from being an eigenvalue of any linear function) in their definition of ``eigenvector'' so that the condition ``$\mathbf{0}$ is the only eigenvector of $\ff$ whose eigenvalue is $0$'' is equivalent to the condition ``$0$ is not an eigenvalue of $\ff$''. The later is easier to say than the former. This convention is a bit too contrived, though, so we will not use it.
\end{remark}
