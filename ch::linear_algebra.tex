...
\label{ch::lin_alg::rmk::det_well_defined}
    The determinant of a linear function is well-defined.
\end{theorem}
...
\begin{proof}
    ...
    \fullindent
    {
        (Special case: $V = W$, $E = F$, $G = H$). In this case, the previous equation can be written as $[\ff(G)]_G = [\cdot]_G(\sE) [\cdot]_E^{-1}(\sE) \spc [\ff(E)]_E \spc [\cdot]_E(\sE) [\cdot]_G^{-1}(\sE)$. Taking the determinant of both sides and using the determinant product rule produces the factors $\det([\cdot]_G(\sE))$, $\det([\cdot]_G(\sE))^{-1} = 1/(\det([\cdot]_G(\sE))$ and $\det([\cdot]_E(\sE))$, $\det([\cdot]_E(\sE))^{-1} = 1/(\det([\cdot]_E(\sE))$, which cancel. This yields $[\ff(G)]_G = [\ff(E)]_E$, as desired. 
    }
