...
\begin{lemma}
    If $||\vv|| = 0$ then $\vv = \mathbf{0}$.
\end{lemma}

\subsection*{Isometries}

\begin{defn}
    (Isometry). A function $\ff:\R^n \rightarrow \R^n$ is called an \textit{isometry (on $\R^n$)} iff it preserves length.
\end{defn}

\begin{theorem}
    The kernel of every isometry is trivial.
\end{theorem}

\begin{proof}
    Suppose for contradiction that $\ff$ is an isometry with a nontrivial kernel, so there exists a nonzero $\vv \in \R^n$ such that $\ff(\vv) = \mathbf{0}$. We have $||\vv|| \neq 0$ (by the lemma) and $||\ff(\vv)|| = ||\mathbf{0}|| = 0$, so $||\vv|| \neq ||\ff(\vv)||$, contradicting the fact that $\ff$ is an isometry.
\end{proof}
        
\begin{lemma}
    Let $\ff:\R^n \rightarrow \R^n$ be an isometry.
        
    \begin{enumerate}
        \item If $\ff$ fixes the origin then $\vv \in \R^n$ then $\ff(\spann(\{\vv\})) = \spann(\{\ff(\vv)\})$.
...
\begin{theorem}
    Every isometry on $\R^n$ that fixes the origin is linear\footnote{From \url{https://www.sfu.ca/~mdevos/notes/geom-sym/18_isometries.pdf}.}.
\end{theorem}

\begin{proof}
    We need to show that $\ff(\vv + \ww) = \ff(\vv) + \ff(\ww)$ for all $\vv, \ww \in \R^n$ and $\ff(c\vv) = c\ff(\vv)$ for all $c \in \R$ and $\vv \in \R^n$.
...
You may be familiar with how the \textit{angle} between two vectors $\vv, \ww \in \R^2$ of the same length $r$ is defined by placing the tails of the vectors at the center of a circle of radius $r$, and then computing the ratio of arclength on the circle between the tips to the radius of the circle; $\text{angle} = \frac{\text{arclength}}{\text{radius}}$. Less commonly discussed, though still crucial for good understanding, is the distinction between the two types of angle, which are \textit{signed} and \textit{unsigned}. We will see that the \textit{signed counterclockwise angle} $\theta_s(\vv, \ww)$ between vectors $\vv, \ww \in \R^2$ accounts for whether the shorter of the two arcs between the vectors is achieved by counterclockwise motion (in this case, the \textit{sign} is positive) or by clockwise motion (in this case, the \textit{sign} is negative). (There is a similar definition for \textit{signed clockwise angle} $\widetilde{\theta}_s(\vv, \ww)$ that uses the opposite signed convention, in which clockwise motion is positive and counterclockwise motion is negative). The \textit{unsigned angle} $\theta_u(\vv, \ww)$ between vectors $\vv, \ww \in \R^2$ is simply the absolute value of the signed counterclockwise angle.
...
Unsigned angle is the natural notion of ``the angle between two vectors'', since $\theta_u(\vv, \ww) = \theta(\ww, \vv)$, which is in accordance to the fact that ``the angle between two vectors'' shouldn't have any antisymmetry that distinguishes the two vectors from each other. (Signed angle does not work for this purpose, since $\theta_s(\vv, \ww) \neq \theta_s(\ww, \vv)$).
...
\begin{defn}
    ...
    One example of a counterclockwise parameterization is given by $\cc(t) := (t, \sqrt{1 - t^2})$ when $t \in [-1, 1]$ and $\cc(t) := (t - 1, -\sqrt{1 - (t - 1)^2})$ when $t \in ([-1, 1] + 2) = [1, 3]$. For any counterclockwise parameterization with domain $[t_1, t_2]$, a clockwise parameterization is given by $\widetilde{\cc}(t) := \cc(t_1 + t_2 - t)$.
\end{defn}

\begin{defn}
\label{ch::lin_alg::defn::angle_in_R2}
    (Signed counterclockwise angle between vectors in $\R^2$).
    ...
    We define the \textit{signed counterclockwise angle $\theta_s(\vv, \ww)$ between $\vv$ and $\ww$} to be $\theta_s(\vv, \ww) := \frac{s(\cc, \vv, \ww)}{r} \bmod 2\pi$. 
    
    Notice that $\theta_s(\vv, \ww) = \theta_s(\hvv, \hww)$. When $\vv$ and $\ww$ don't have the same length, we define $\theta_s(\vv, \ww) := \theta_s(\hvv, \hww) = s(\cc, \hvv, \hww) \text{ mod } 2\pi$. Therefore, in general, we have

    \begin{align*}
        &\theta_s(\vv, \ww) = s(\cc, \hvv, \hww) \bmod 2\pi.
    \end{align*}
\end{defn}

\begin{defn}
    (Signed clockwise angle between vectors in $\R^2$).

    $\widetilde{\theta}_s$
\end{defn}

We investigate counterclockwise motion first to get our bearings.

\subsubsection*{Signedness of signed angle}

[review facts about mod in appendix]

\begin{theorem}
    (Signedness of signed counterclockwise angle).
    
    We have $\theta_s(\vv, \ww) = -\theta_s(\ww, \vv)$ and $\widetilde{\theta}_s(\vv, \ww) = -\widetilde{\theta}_s(\ww, \vv)$ for all $\vv, \ww \in \R^2$.
\end{theorem}
...
As you might have suspected, signed clockwise angle is the additive inverse of signed counterclockwise angle,

\begin{theorem}
    (Relationship between signed counterclockwise and clockwise angles).
    
    $-\theta_s(\vv, \ww) \bmod 2\pi =  \widetilde{\theta}_s(\vv, \ww)$ for all $\vv, \ww \in \R^2$.
\end{theorem}

\begin{proof}
    For all $\vv, \ww \in \R^2$, the unit circle is partitioned into the counterclockwise path from $\hvv$ to $\hww$ (which is $\cc([t_1, t_2])$, where $\cc(t_1) = \hvv$ and $\cc(t_2) = \hww$) and the clockwise path from $\hvv$ to $\hww$ (which is $\widetilde{\cc}([\widetilde{t}_1, \widetilde{t}_2])$, where $\widetilde{\cc}(\widetilde{t}_1) = \hvv$ and $\widetilde{\cc}(\widetilde{t}_2) = \hww$). The arclength of the unit circle, $2\pi$, is therefore the sum of the arclengths of these parameterizations. Thus $s(\cc, \hvv, \hww) + s(\widetilde{\cc}, \hvv, \hww) = 2\pi$ for all $\vv, \ww \in \R^2$. That is, $\theta_s(\vv, \ww) + \widetilde{\theta}_s(\vv, \ww) = 2\pi$ for all $\vv, \ww \in \R^2$.

    Rearranging, we have $\widetilde{\theta}_s(\vv, \ww) = 2\pi - \theta_s(\vv, \ww)$; applying the map $\theta \mapsto \theta \bmod 2\pi$ to each side of this equation and using the general fact that modulus distributes over addition\footnote{See \ref{ch::appendix::thm::modulus_distributes_over_addition}.}, we have $\widetilde{\theta}_s(\vv, \ww) \bmod 2\pi = 2\pi \bmod 2\pi - \theta_s(\vv, \ww) \bmod 2\pi = -\theta_s(\vv, \ww) \bmod 2\pi$ for all $\vv, \ww \in \R^2$. So $\widetilde{\theta}_s(\vv, \ww) \bmod 2\pi = -\theta_s(\vv, \ww) \bmod 2\pi$ for all $\vv, \ww \in \R^2$. Since $\widetilde{\theta}_s(\vv, \ww) \in [0, 2\pi)$, then $\widetilde{\theta}_s(\vv, \ww) \bmod 2\pi = \widetilde{\theta}_s(\vv, \ww)$. Therefore $\widetilde{\theta}_s(\vv, \ww) = -\theta_s(\vv, \ww) \bmod 2\pi$, as desired. 
\end{proof}
...
\begin{deriv}
    ...
    The map $t \mapsto s(\cc, \vv, \cc(t))$ measures the arclength accumulated along the counterclockwise parameterization of the unit circle, and is therefore is strictly increasing, i.e., it is one-to-one, i.e., it is invertible. Since every $\hww \in \R^2$ is of the form $\hww = \cc(t)$ for some $t \in \R$ [come back and ensure consistency of angle intervals], then the map $\hww \mapsto s(\cc, \vv, \ww)$ defined on unit vectors is invertible as well. This means that for every $t$ there is a unique $\hww \in \R^2$ such that $\theta_s(\vv, \hww) = t$.

    Knowing this, then for every $t \in [0, 2\pi)$ we can $\cos(t)$ and $\sin(t)$ to be the coordinates of the unique unit vector $(\cos(t), \sin(t))^\top$ is the unique unit vector such that

    \begin{align*}
        \theta_s\left(\ee_1, \begin{pmatrix}
            \cos(t) \\
            \sin(t)
        \end{pmatrix}\right) = t.
    \end{align*}
...
\begin{theorem}
    ...
    Suppose $\vv, \ww \in Q_1 = \{(x, y) \mid x, y > 0\}$.
...
\begin{theorem}
    ...
    \begin{align*}
        \theta_s(\vv, \ww) = \pm \min(s(\cc, \hvv, \hww), s(\widetilde{\cc}, \hvv, \hww)) \bmod 2\pi,
    \end{align*}

    where $\pm$ is $+$ when the minimum is $s(\cc, \vv, \ww)$ and is $-$ when the minimum is $s(\widetilde{\cc}, \vv, \ww)$.
\end{theorem}
...
\begin{defn}
    ...
    The \textit{unsigned angle $\theta_u(\vv, \ww)$ between $\vv$ and $\ww$} is defined to be $\theta_u(\vv, \ww) := |\theta_s(\vv, \ww)|$. Because of the previous theorem, we have

    \begin{align*}
        \theta_u(\vv, \ww) = \min(s(\cc, \hvv, \hww), s(\widetilde{\cc}, \hvv, \hww)).
    \end{align*}
\end{defn}

\begin{proof}
    TO-DO: show how the $\bmod 2\pi$ disappears
\end{proof}

\begin{theorem}
    (Unsigned angle is symmetric).

    $\theta_u(\vv, \ww) = \theta_u(\ww, \vv)$ for all $\vv, \ww \in \R^2$.
\end{theorem}
...
\begin{defn}
    (Perpendicular vectors in $\R^2$). We say that $\vv, \ww \in \R^2$ are \textit{perpendicular}, or \textit{orthogonal}, iff $\proj(\vv \rightarrow \ww) = \mathbf{0}$.
\end{defn}
...
\begin{defn}
    ...
    An isometry $\ff:\R^2 \rightarrow \R^2$ is called the \textit{counterclockwise rotation (by $t$)} iff there exists $t \in [-2\pi, 2\pi]$ such that $\theta_s(\vv, \ff(\vv)) = t$ for all $\vv \in \R^2$.
\end{defn}

\begin{theorem}
     The standard matrix of the counterclockwise rotation by $t$ is
...
\begin{proof}
    We have $\RR_t(\hvv) =(\cos(\theta(\RR_t(\hvv), \see_1)), \sin(\theta(\RR_t(\hvv), \see_1)))^\top$ or $\RR_t(\hvv) = (\cos(-\theta(\RR_t(\hvv), \see_1)), \sin(-\theta(\RR_t(\hvv), \see_1)))^\top$ for all $\hvv \in \R^2$.
        
    When $\hvv = \see_1$, then since $\theta(\RR_t(\see_1), \see_1) = t$ by the definition of a rotation, we have $\RR_t(\see_1) = (\cos(t), \sin(t))^\top$. 
    
    When $\hvv = \see_2$, then\footnotemark since $\theta(\RR_t(\see_2), \see_1) = \theta(\RR_t(\see_2), \see_2) + \theta(\see_2, \see_1) = t + \frac{\pi}{2}$, we have\footnote{Recall that $\cos(s) = \sin(\pi/2 - s)$ and $\sin(s) = \cos(\pi/2 -s)$.} $\RR_t(\see_2) = (\cos(t + \pi/2), \sin(t + \pi/2))^\top = (\sin(-t), \cos(-t))^\top = (-\sin(t), \cos(t))^\top$.
\end{proof}
...
\begin{defn}
    ...
    ... $\sproj(\vv \rightarrow \ww) = ||\vv|| \cos(\theta(\vv, \ww))$ [think about $\theta_s$ vs. $\theta_u$], so $\vv \cdot \ww = ||\vv|| \spc ||\ww|| \sproj(\vv \rightarrow \ww)$.
\end{defn}
...
\label{ch::lin_alg::lemma::orthogonal_linear_fns_preserve_alg_dot_product}
    ...
    If $\ff:\R^2 \rightarrow \R^2$ is an isometry, then $\ff$ must also be linear, and  
...
\begin{proof}
    \mbox{} \\
    \begin{itemize}
        \item (Isometry $\implies$ (2)). Suppose $\ff$ preserves length. If we showed that $\vv \cdot \ww$ depends on $||\vv||$ and $||\ww||$ (on lengths), it would follow that $\ff$ preserves dot product. This is what we will do.
       ...
       \begin{align*}
        ...
        (Isometry $\impliedby$ (2)). If $\ff$ preserves dot product, then $||\ff(\vv)|| = \sqrt{\ff(\vv) \cdot \ff(\vv)} = \sqrt{\vv \cdot \vv} = ||\vv||$, so $\ff$ preserves length.
