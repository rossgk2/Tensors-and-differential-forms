...
\begin{lemma}
    ...
    Let $\vv, \ww \in \R^2$. For any $t \in [0, 2\pi)$ we have $\RR_t(\proj(\vv \rightarrow \ww)) = \proj(\RR_t(\vv) \rightarrow \RR_t(\ww))$.
\end{lemma}
...
\begin{proof}
    ...
    Now we compare the second and third expressions in the above. On one hand, $v_{||} \RR_t(\hww) = \RR_t(v_{||} \hww) = \RR_t(\proj(\vv \rightarrow \ww))$. On the other hand, since $v_{||}\widehat{\RR_t(\hww)}$ is the parallel term in the orthogonal decomposition of $\RR_t(\vv)$ onto $\RR_t(\ww)$, it must be $\proj(\RR_t(\vv) \rightarrow \RR_t(\ww))$. Rewriting both sides in terms of these projections, we have
...
\begin{lemma}
	(Scalar projection between rotated vectors).
	
	For any $t \in [0, 2\pi)$ we have $\sproj(\vv \rightarrow \ww) = \sproj(\RR_t(\vv) \rightarrow \RR_t(\ww))$.
\end{lemma}
...
We would like to extend this definition to $\R^n$ and to finite-dimensional inner product spaces over $\R$. Unfortunately, this is not possible as of now since we don't have a notion of $n$-dimensional signed angle $\theta_s$. It \textit{is} possible to define a notion of $n$-dimensional signed angle from first principles, but it is quite complicated\footnote{See [...] in the appendix if you're interested.}. The easiest way to generalize this definition is to search for an equivalent one. To this end, we present a promising definition in the next theorem.
