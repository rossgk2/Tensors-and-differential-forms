...
Convention dictates that we interpret motion along the counterclockwise parameterization as corresponding to positive contributions to angle and the clockwise parameterization as corresponding to negative contributions to angle.
...
[have to know about mod before going further]

\begin{defn}
    (Modulus).

    [idea for $\bmod T$ for $T \in \R$. when $a, n \in \Z$ can write $a/n = q + r/n$ for integers $q$ and $r$, where $r \in [0, n)$. sim., when $a, T \in \R$ can write $a/T = q + r/T$ for integer $q$ and real $r \in [0, T)$.]
\end{defn}
...
\begin{remark}
    ...
    [Let's think about $-a \bmod n$ for positive $a \in \Z$.] From above, we have $a/n = q + r/n$. That is, $a = nq + a \bmod n$. From this expression, we can clearly see that $nq$ is the greatest integer multiple of $n$ that is less than $a$. So, $a \bmod n = nq - a$; that is,
    ...
    It is fairly easy\footnote{See Lemma \ref{ch::appendix::lemma::modular_arithmetic_lemma} of the appendix.} to show that in the special case when $a \in \Z \cap (0, n]$ we have $-a \bmod n = n - a \bmod 2\pi$. For our situation of modulus $2\pi$, this gives us the familiar relationship

    \begin{align*}
        (-\theta) \bmod 2\pi = (2\pi - \theta) \bmod 2\pi.
    \end{align*}

    An example of this relationship is $-\frac{\pi}{2} \bmod 2\pi = \frac{3\pi}{2} \bmod 2\pi$ and $-\frac{3\pi}{2} \bmod 2\pi = \frac{\pi}{2} \bmod 2\pi$.    
\end{remark}

\begin{theorem}
    (Signedness of signed counterclockwise angle). $\theta_s(\vv, \ww) = -\theta_s(\ww, \vv)$ for all $\vv, \ww \in \R^2$.
\end{theorem}
...
\subsubsection*{Signed clockwise angle}
...
\begin{theorem}
    $\theta_s(\vv, \ww) + \widetilde{\theta}_s(\vv, \ww) = 2\pi$ for all $\vv, \ww \in \R^2$.
\end{theorem}

\begin{proof}
    The theorem follows from the fact that $(x + y) \bmod T = x \bmod T + y \bmod T$. A proof of this fact is given in the appendix. [TO-DO]
\end{proof}
...
\begin{defn}
    ...
    We define functions $\cos:\R \rightarrow \R$ and $\sin:\R \rightarrow \R$ by imposing that they be periodic functions of period $2\pi$ such that $(\cos(t), \sin(t))^\top$ is the point obtained by traveling an arclength of $t$ along the counterclockwise parameterization of the unit circle when $t \in [0, 2\pi]$. Succinctly,
    ...
    \begin{align*}
        ...
        \end{pmatrix}
        :=
        \cc(t \bmod 2\pi) \text{ for all $t \in \R$}.
    \end{align*}
\end{defn}

\begin{theorem}
    $\theta\left(\see_1, \begin{pmatrix} \cos(t) \\ \sin(t) \end{pmatrix}\right) = t$
\end{theorem}

\begin{proof}
    We have $\see_1 = \cc(t_1)$ and $(\cos(t), \sin(t))^\top = \cc(t_2)$, where $t_1 = 0$ and $t_2 = t$. The angle between these vectors is $\int_{t_1}^{t_2} ||\frac{d\cc(t)}{dt}||dt = \int_0^t dt = t$. 
\end{proof}

\begin{theorem}
    ($\cos$ and $\sin$ as ratios of lengths of sides of triangles).
\end{theorem}
