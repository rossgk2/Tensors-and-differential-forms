...
\begin{defn}
    ...
    A function $\RR:\R^2 \rightarrow \R^2$ is a \textit{rotation on $\R^2$} iff
...
\begin{lemma}
\label{ch::lin_alg::thm::rotations_on_R2_rotate_vectors_by_same_angle}
    (Rotations on $\R^2$ rotate all vectors by the samge angle).

    If $\ff$ is a length-preserving function on $\R^2$ that fixes the origin, then $\ff$ is a rotation on $\R^2$ iff there exists $t \in [0, 2\pi)$ such that $\theta_s(\vv, \ff(\vv)) = t$.  
\end{lemma}

\begin{proof}
    [TO-DO]

    \textbf{hard part: show that $\theta_s(\see_1, \ff(\see_1)) = \theta_s(\see_2, \ff(\see_2))$}

    We have $\theta_s(\see_1, \see_2) + \theta_s(\see_2, \ff(\see_2)) = \theta_s(\see_1, \ff(\see_2))$, so $\theta_s(\see_2, \ff(\see_2)) = \theta_s(\see_1, \ff(\see_2)) - \theta_s(\see_1, \see_2) = \theta_s(\see_1, \ff(\see_2)) - \pi/2$.

    It remains to determine $\theta_s(\see_1, \ff(\see_2))$. We have $\theta_s(\see_1, \ff(\see_2)) = \theta_s(\see_1, \ff(\see_1)) + \theta_s(\ff(\see_1), \ff(\see_2)) = t + \theta_s(\see_1, \see_2) = t + \pi/2$.

    So $\theta_s(\see_1, \ff(\see_2)) = (t + \frac{\pi}{2}) - \pi/2$.

    \textbf{easy part:}
    
    Take any non-zero vector $v$. Consider the triangle formed by vectors $0$, $v$, and $e_1$. Under rotation $\ff$, this maps to the triangle formed by $0$, $\ff(v)$, and $\ff(e_1)$.
    
    Since $\ff$ preserves all angles and lengths:
    - The angle at the origin between $v$ and $e_1$ is preserved: $\theta(v, e_1) = \theta(\ff(v), \ff(e_1))$
    - We know $\theta(e_1, \ff(e_1)) = t$
    
    By the angle addition property and the fact that $\ff$ preserves the angular structure completely, we must have $\theta(v, \ff(v)) = t$.
\end{proof}

\begin{remark}
    The forward implication of the previous lemma fails for more rotations on $\R^n$ (which we have not yet defined).
\end{remark}
...
\begin{theorem}
    ...
    for some $t \in [0, 2\pi)$. [We use $\RR_t$ to denote the rotation whose standard matrix is the above.]
\end{theorem}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        ||\RR(\vv)||
        \begin{pmatrix}
            \cos(\theta(\see_1, \RR(\vv)) \\
            \sin(\theta(\see_1, \RR(\vv))
        \end{pmatrix}
        =
        ||\vv||
        \begin{pmatrix}
    ...
    \begin{align*}
        ...
        \begin{pmatrix}
            ...
            \sin(t)
        \end{pmatrix},
        \text{ where $t = \theta(\see_1, \RR(\see_1))$}.
    \end{align*}

    When $\vv = \see_2$, the transitivity of signed angle (recall Theorem \ref{ch::lin_alg::thm::transitivity_of_angle}) gives us $\theta_s(\RR(\see_2), \see_1) = \theta_s(\RR(\see_2), \see_2) + \theta_s(\see_2, \see_1)$. We have $\theta_s(\RR(\see_2), \see_2) = t$ by Lemma \ref{ch::lin_alg::thm::rotations_on_R2_rotate_vectors_by_same_angle}, and $\theta_s(\see_2, \see_1) = \frac{3\pi}{2}$, so we have\footnote{Recall that $\cos(s) = \sin(\pi/2 - s)$ and $\sin(s) = \cos(\pi/2 - s)$.}

    \begin{align*}
        \RR(\see_2) =
        \begin{pmatrix}
            \cos\left(t + \frac{3\pi}{2}\right) \\
            \sin\left(t + \frac{3\pi}{2}\right)
        \end{pmatrix}
        =
        \begin{pmatrix}
            \sin(-t) \\
