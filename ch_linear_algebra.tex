...
\label{ch::lin_alg::defn::angle_in_R2}
    ...
    [the maximum arclength between $\vv$ and $\ww$ is the arclength of the entire parameterization. that arclength approaches $2\pi$ but never attains it, since if it were $2\pi$ then $\hvv = \hww$]. Therefore, we also have

    \begin{align*}
        \theta_s(\vv, \ww), \tttheta_s(\vv, \ww) \in [0, 2\pi) \text{ for all $\vv, \ww \in \R^2$}.
    \end{align*} 
\end{defn}
...
\begin{proof}
    In general, if $t \in [0, 2\pi)$ then $-t \bmod 2\pi = 2\pi - t$. (Hopefully this is intuitive; it is also explained in the appendix in Theorem \ref{ch::appendix::thm::negation_of_angle}). Therefore $-\theta_s(\vv, \ww) \bmod 2\pi = 2\pi - \theta_s(\vv, \ww)$. Of course, we have $2\pi - \theta_s(\vv, \ww) = \tttheta_s(\vv, \ww)$ by the previous theorem, so $-\theta_s(\vv, \ww) \bmod 2\pi = \tttheta_s(\vv, \ww)$, as desired.
\end{proof}
...
\begin{theorem}
    ...
    \begin{align*}
        \theta_s(\vv, \ww) = \pm \min(|\theta_s(\vv, \ww)|, |\tttheta_s(\vv, \ww)|) \bmod 2\pi,
    \end{align*}
...
\begin{proof}
    Since $\theta_c(\vv, \ww) = s(\cc, \hvv, \hww)$ and $\tttheta_s(\vv, \ww) = s(\widetilde{\cc}, \hvv, \hww)$, the claim is equivalent to
...
\begin{defn}
    ...
    \begin{align*}
        \theta_s(\vv, \ww) = \min(|\theta_s(\vv, \ww)|, |\tttheta_s(\vv, \ww)|) \bmod 2\pi,
    \end{align*}
\end{defn}
