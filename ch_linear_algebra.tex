...
\begin{proof}
    ...
    Notice that $\RR_t(\proj(\vv \rightarrow \ww)) = \RR_t(v_{||} \hww) = v_{||} \RR_t(\hww)$. Also, since $v_{||}\widehat{\RR_t(\hww)}$ is the parallel term in the orthogonal decomposition of $\RR_t(\vv)$ onto $\RR_t(\ww)$, it must be $\proj(\RR_t(\vv) \rightarrow \RR_t(\ww))$. So, we have
