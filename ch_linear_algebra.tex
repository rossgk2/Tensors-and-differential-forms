...
\begin{proof}
    ...
    First, we prove $\vv$ is equal to the first expression. We have $\theta_s(\see_1, \vv) = t$ for some $t \in [0, 2\pi)$. By the previous derivation, we have $\hvv = \vv/||\vv|| = (\cos(t), \sin(t))^\top = (\cos(\theta_s(\see_1, \vv)), \sin(\theta_s(\see_1, \vv)))^\top$. Multiply both sides of this equation by $||\vv||$ to obtain the desired result.
...
\begin{proof}
    We have $\theta_u(\vv, \ww) := \min(\theta_s(\vv, \ww), \tttheta_s(\vv, \ww)) \bmod 2\pi$. Consider the following cases.
    
    (Case: the minimum is $\theta_s(\vv, \ww)$). since $\theta_s(\ww, \vv) = -\theta_s(\vv, \ww)$

    (Case: the minimum is $\tttheta_s(\vv, \ww)$).
\end{proof}
