...
\begin{proof} \mbox{} \\ \indent
   ($\subseteq$). Let $\uu \in \ff^{-1}(\{\ww\}) \iff \ff(\uu) = \ww$. If $\vv \in \ff^{-1}(\{\ww\})$ then $\ff(\uu - \vv) = \ff(\uu) - \ff(\vv) = \ww - \ww = \mathbf{0}$, so $\uu - \vv \in \ff^{-1}(\{\mathbf{0}\})$. That is, $\uu - \vv = \vv_0$, where $\vv_0 \in \ff^{-1}(\{\mathbf{0}\})$. We conclude that $\uu = \vv + \vv_0$, where $\vv_0 \in \ff^{-1}(\{\mathbf{0}\})$, so $\uu$ is in the first set.
...
\begin{proof}
    $\ff$ is one-to-one iff for all $\ww \in W$ the preimage $\ff^{-1}(\{\ww\})$ is a singleton. It is easy to see via the previous theorem that the preimages are singletons iff $\ff^{-1}(\{\mathbf{0}\}) = \{\mathbf{0}\}$.
\end{proof}
...
\begin{proof}
    ...
    ($\implies$). Suppose that if $\vv_1, ..., \vv_k$ are linearly independent, then $\ff(\vv_1), ..., \ff(\vv_k)$ are linearly independent. We need to show $\ff$ is one-to-one; it suffices to show that $\ff$ has a minimal kernel. Let $\vv \in \ff^{-1}(\{\mathbf{0}\})$, so $\ff(\vv) = \mathbf{0}$. We want to show $\vv = \mathbf{0}$. 
...
\label{ch::lin_alg::thm::solutions_to_linear_equations}
    ...
    The set of solutions $\{\text{solns}\}$ is defined to be the preimage of $\{\ww\}$ under $\ff$. Recall from Theorem [...] that when it is nonempty, it can be described in recursively:
