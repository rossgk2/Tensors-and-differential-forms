...
\begin{proof}
    ...
    \fullindent
    {
        ...
        \fullindent
        {
            (Case: $c \neq 0$). Notice that $c\vv \in \spann(\{\vv\})$ satisfies
...
\label{ch::lin_alg::defn::angle_in_R2}
    ...
    Notice that $\theta(\vv, \ww) = \theta(\hvv, \hww)$. When $\vv$ and $\ww$ don't have the same length, we define $\theta(\vv, \ww) := \theta(\hvv, \hww) = s(\cc, \hvv, \hww) \text{ mod } 2\pi$. Therefore, in general, we have
...
Fortunately, there is a more convenient way to compute the signed counterclockwise angle that, by virtue of the fact that it produces a result in $[-2\pi, 2\pi]$, doesn't involve the $\bmod$ operator.

\begin{theorem}
    (Signed counterclockwise angle as a minimum).

    \begin{align*}
        \theta_s(\vv, \ww) = \pm \min(s(\cc, \hvv, \hww), s(\widetilde{\cc}, \hvv, \hww)),
    \end{align*}
...
\begin{defn}
    ...
    We define $(\cos(t), \sin(t))^\top := \cc(t)$. That is, $\cos(t)$ and $\sin(t)$ are such that $(\cos(t), \sin(t))^\top$ is the point obtained by traveling an arclength of $t$ along the counterclockwise parameterization of the unit circle.
\end{defn}
...
\begin{theorem}
    $\theta_s(\vv, \ww) = -\theta_s(\ww, \vv)$

    $-\theta_s(\vv, \ww) = 2\pi - \theta_s(\vv, \ww) \text{ mod $2\pi$}$
\end{theorem}

\begin{proof}
    We have $s(\xx, \vv, \ww) = s(\xx, \ww, \vv)$ for all $\vv, \ww \in \R^2$ and $\xx \in \{\cc, \widetilde{\cc}\}$ because the length of a path is equal to the length of the reverse path. 
        
    If $\theta(\vv, \ww) = \frac{s(\cc, \vv, \ww)}{r}$ then 

    FIX THIS
\end{proof}

\begin{lemma}
    $\cos(\theta_s(\vv, \ww)) = \cos(\theta_u(\vv, \ww))$ when $\theta_s \in [0, \pi]$.
\end{lemma}
...
\section{Length and angle [deprecated]}
