...
\begin{theorem}
\label{ch::lin_alg::thm::transitivity_of_signed_angle}
    (Transitivity of signed angle).

    We have $\theta_s(\vv, \uu) + \theta_s(\uu, \ww) = \theta_s(\vv, \ww)$ for all $\vv, \uu, \ww \in \R^2$.

    check: does this not work for unsigned angle too?
\end{theorem}
...
\begin{defn}
    ...
    A function $\ff:\R^2 \rightarrow \R^2$ is a \textit{rotation} iff

    \begin{enumerate}
        \item ($\ff$ preserves length). $||\vv|| = ||\ff(\vv)||$ for all $\vv \in \R^2$.
        \item ($\ff$ preserves signed angle). $\theta_s(\vv, \ww) = \theta_s(\ff(\vv), \ff(\ww))$ for all $\vv, \ww \in \R^2$.
        \item ($\ff$ fixes the origin). $\ff(\mathbf{0}) = \mathbf{0}$.
    \end{enumerate}
\end{defn}
...
\begin{theorem}
    Every length-preserving function on $\R^n$ that fixes the origin is linear\footnote{From \url{https://www.sfu.ca/~mdevos/notes/geom-sym/18_isometries.pdf}.}.
\end{theorem}
...
\begin{theorem}
    All rotations are linear functions.
\end{theorem}
...
\begin{theorem}
    ...
    for some $t \in [0, 2\pi)$. [We use $\RR_t$ to denote the rotation with above matrix.]
\end{theorem}
...
\begin{proof}
    ...
    When $\vv = \see_2$, the transitivity of signed angle (recall Theorem \ref{ch::lin_alg::thm::transitivity_of_signed_angle}) gives us $\theta_s(\RR(\see_2), \see_1) = \theta_s(\RR(\see_2), \see_2) + \theta_s(\see_2, \see_1)$. \textbf{by a lemma (which claims that if $\RR$ preserves angle then $\theta_s(\vv, \RR(\vv))$ is a constant)} this is equal to $t + \frac{\pi}{2}$, so we have\footnote{Recall that $\cos(s) = \sin(\pi/2 - s)$ and $\sin(s) = \cos(\pi/2 -s)$.}
