...
\begin{remark}
\label{ch::lin_alg::rmk::direction_vectors}
    (Direction vectors).
    
    Since the length of a unit vector is in some sense uninteresting, unit vectors can be considered to be specifying only direction and not magnitude; they can be considered to be ``direction vectors''.
\end{remark}
...
Signed angle $\theta_s$ is useful for computing the vector that makes a specified signed angle with another vector, since for every vector $\vv \in \R^2$ and angle $t$ there is a unique vector that makes a signed counterclockwise angle of $t$ with $\vv$. (Unsigned angle does not work for this purpose, since for every vector $\vv$ and angle $t$, there are \textit{two} vectors that make an unsigned angle of $t$ with $\vv$). This lays the groundwork for the familiar computations of trigonometry.

Unsigned angle $\theta_u$ is the natural notion of ``the angle between two arbitrary vectors'', since $\theta_u(\vv, \ww) = \theta_u(\ww, \vv)$, which is in accordance to the fact that two arbitrary vectors do not come with an ordering. (Signed angle does not work for this purpose, since $\theta_s(\vv, \ww) \neq \theta_s(\ww, \vv)$).
...
\label{ch::lin_alg::lemma::vector_from_vector_and_angle}
    (Specifying a vector with a vector and an signed angle difference).
    
    For every $\hvv \in \R^2$ and $t \in [0, 2\pi)$ there is a unique $\hww_1 \in \R^2$ such that $\theta_s(\hvv, \hww_1) = t$ and a unique $\hww_2 \in \R^2$ such that $\tttheta_s(\hvv, \hww_2) = t$.
\end{theorem}

\begin{proof}
    We show the theorem for $\theta_s$; the proof for $\tttheta_s$ is similar.
    
    The map $t \mapsto s(\cc, \vv, \cc(t))$ measures the arclength accumulated along the counterclockwise parameterization of the unit circle, and is therefore is strictly increasing, i.e., it is one-to-one, i.e., it is invertible. Since every $\hww_1 \in \R^2$ is of the form $\hww_1 = \cc(t)$ for some $t \in \R$ [come back and ensure consistency of angle intervals], then for all $\hvv \in \R^2$ the map $\hww_1 \mapsto s(\cc, \hvv, \hww_1)$ defined on unit vectors is invertible as well. This means that for every $\hvv \in \R^2$ and $t \in [0, 2\pi)$ there is a unique $\hww_1 \in \R^2$ such that $\theta_s(\vv, \hww_1) = t$.
\end{proof}

\begin{remark}
    (The basis for trigonometry).

    The above theorem makes signed angle $\theta_s$ quite useful for computations involving positions on circles. Specifically, we will later use this theorem to define functions $\cos$ and $\sin$ to be such that for every $t \in [0, 2\pi)$ the vector $(\cos(t), \sin(t))^\top \in \R^2$ is the unique unit vector $\hvv$ such that $\theta_s(\see_1, \hvv) = t$. After this, we can derive the familiar characterization of $\cos$ and $\sin$ as ratios between lengths of sides of right triangles. This familiar characterization is of course where the name \textit{trigonometry} comes from. 
\end{remark}
...
\begin{remark}
    (Unsigned angle is the natural notion of angle between arbitrary vectors).

    Since any two arbitrary vectors $\vv, \ww \in \R^2$ do not come with a natural ordering, a sensible notion of the angle $\theta$ between them cannot depend on the order in which $\vv, \ww$ are specified; we must have $\theta(\vv, \ww) = \theta(\ww, \vv)$. We've just seen that unsigned angle $\theta_u$ has the desired property $\theta_u(\vv, \ww) = \theta_u(\ww, \vv)$. Thus, unsigned angle is the natural notion of the angle between two vectors. 
\end{remark}
...
\begin{defn}
    \label{ch::lin_alg::defn::rotation_in_R2}
    (Rotation in $\R^2$).
...
\begin{theorem}
    A function $\R^2 \rightarrow \R^2$ is a rotation in $\R^2$ iff it is linear, preserves length, and preserves signed angle.
\end{theorem}
...
\label{ch::lin_alg::deriv::cosine_sine}
    ...
    For every $t \in [0, 2\pi)$ we define $\cos(t)$ and $\sin(t)$ to be the coordinates of the unique\footnote{We know such a unique unit vector exists because of Theorem \ref{ch::lin_alg::lemma::vector_from_vector_and_angle}.} unit vector $\hvv$ such that $\theta_s(\see_1, \hvv) = t$.
\end{defn}
...
\begin{theorem}
    \label{ch::lin_alg::defn::standard_matrix_rotation}
    ...
    for some $t \in [0, 2\pi)$.
    
    Since multiplying this matrix by $\see_1$ gives $(\cos(t), \sin(t))^\top = \cc(t)$, the rotation whose standard matrix is the above is called the \textit{counterclockwise rotation by $t$} and is denoted $\RR_t$. Similarly, since $\RR_{2\pi - t}(\see_1) = \widetilde{\cc}(t)$, the rotation $\widetilde{\RR}_t := \RR_{2\pi - t}$ is called the \textit{clockwise rotation by $t$}.

    Clearly, every rotation is both a counterclockwise rotation by some amount and a clockwise rotation by some amount.
\end{theorem}
...
\begin{proof}
    \mbox{} \\
    \begin{enumerate}
        \item Using Theorem \ref{ch::lin_alg::thm::vector_polar_form}, we have $\vv = ||\vv||(\cos(\theta), \sin(\theta))^\top$, where $\theta = \theta_s(\see_1, \vv)$. Thus
        ...
        Then $\theta_s(\vv, \RR_t(\vv)) = s(\xx, \hvv, \widehat{\RR_t(\vv)})$, where $\xx:[0, 2\pi) \rightarrow \R^2$ is defined by $\xx(t) = (\cos(t), \sin(t))^\top$, which is

        \begin{align*}
            s(\xx, \hvv, \widehat{\RR_t(\vv)}) =
            \theta_s
            \left(
        ...
        That is, $\theta_s(\vv, \RR_t(\vv)) = s(\xx, \hvv, \widehat{\RR_t(\vv)}) = t$, as desired.
                
        \item By (1), we have $\theta_s(\hvv, \RR_{\theta_s(\hvv, \hww)}(\hvv)) = \theta_s(\hvv, \hww)$. We know from Theorem \ref{ch::lin_alg::lemma::vector_from_vector_and_angle} that the vector $\huu \in \R^2$ for which $\theta_s(\hvv, \huu) = \theta_s(\hvv, \hww)$ is unique. Therefore $\huu = \RR_{\theta_s(\hvv, \hww)}(\vv) = \hww$, so $\RR_{\theta_s(\hvv, \hww)}$ takes $\hvv$ to $\hww$.        
    \end{enumerate}
\end{proof}
...
\label{ch::lin_alg::defn::vector_proj}
    ...
    Outline:
    \begin{itemize}
        \item Defn. The \textit{unsigned angle between a vector $\vv \in \R^2$ and a line in $\R^2$ through the origin equal to $\spann(\ww)$ for some $\ww \in \R^2$} is defined to be $\min_{\tww \in \spann(\ww)} \theta_u(\vv, \tww)$. Because [...] this is equal to $\min(\theta_u(\vv, \tww), \theta_u(\vv, -\tww))$.
        \item Thm. This notion of unsigned angle is symmetric.
        \item Defn. Two vectors in $\R^2$ are said to be \textit{orthogonal}, or \textit{perpendicular}, if the direction vector of one is as different from the ``direction line''  of the other as possible, i.e. if the unsigned angle between $\vv$ and $\spann(\ww)$ is maximized.
        \begin{itemize}
        ...
        \end{itemize}
        \item If $\vv \in \R^2$ and $\vv_\perp$ is orthogonal to $\vv$ then $\hvv_\perp = \widehat{\vv_\perp}$.
        \item Remark. (Orthogonality is symmetric). Note that $\vv$ is orthogonal to $\ww$ iff $\ww$ is orthogonal to $\vv$.
        \item Thm. For every $\vv \in \R^2$ there exists $\vv_\perp \in \R^2$ that is orthogonal to $\vv$.
        \begin{itemize}
            \item Proof. Let $t$ be such that $\RR_t(\see_1) = \vv$. Then $\RR^{-1}$ is a rotation taking $\see_1$ to $\hvv$ and taking $\RR(\see_2)$ to $\see_2$. Thus $\vv_\perp = \RR(\see_2)$ is orthogonal to $\vv$.
        \end{itemize}
        \item Thm. If $\vv, \vv_\perp \in \R^2$ and $\vv_\perp$ is orthogonal to $\vv$, then $\{\vv, \vv_\perp\}$ is a basis for $\R^2$.
        \begin{itemize}
...
\label{ch::lin_alg::thm::vector_proj_dot_product}
    ...
    \begin{align*}
        \boxed
        {
            \proj(\vv \rightarrow \ww) = (\vv \cdot \hww) \hww
        }
    \end{align*}
    
    In the case $n = 2$, if we didn't know the algebraic characterization of the dot product, this theorem would be a bit of a tautology. However, knowing the algebraic characterization of the dot product allows for easy computation of the expression $\vv \cdot \hww$ in terms of the coordinates of $\vv$ and $\ww$ relative to the standard basis\footnote{Actually, we only need to know the coordinates of $\vv$ and $\ww$ relative to some \textit{orthonormal} basis! Orthonormal bases, also known as \textit{self-dual bases}, are discussed in Section \ref{ch::bilinear_forms_metric_tensors::section::self-duality}.}, and that makes this theorem incredibly useful.
\end{theorem}

\begin{proof}
   Since $\vv \cdot \ww = \sproj(\vv \rightarrow \ww) \spc ||\ww||$, then $\sproj(\vv \rightarrow \ww) = \vv \cdot \frac{\ww}{||\ww||} = \vv \cdot \hww$, and, by the first fact of Theorem \ref{ch::lin_alg::thm::vector_proj_Signed_magnitude}, $\proj(\vv \rightarrow \ww) = \sproj(\vv \rightarrow \ww) \hww = (\vv \cdot \hww) \hww$.
\end{proof}
...
\label{ch::bilinear_forms_metric_tensors::theorem::Gram-Schmidt}
    ...
    Let $V$ be a finite-dimensional inner product space. Given any basis $E = \{\ee_1, ..., \ee_n\}$ for $V$, we can use the following \textit{orthonormalization algorithm} to convert $E$ into an orthonormal basis $\hU = \{\huu_1, ..., \huu_n\}$.
    ...
    This algorithm is most often referred to as the \textit{Gram-Schmidt algorithm}.
\end{theorem}
...
\begin{defn}
	...
	Let $V$ be a vector space with inner product $\langle \cdot, \cdot \rangle$, and consider a vector subspace $W$ of $V$. The \textit{orthogonal complement} $W^\perp$ of $W$ is defined to be the vector space of all vectors in $V$ perpendicular to $W$, $W^\perp := {\{\vv \in V \mid \langle \vv, \ww \rangle = 0 \}}$.
	
	Below, we prove that orthogonal complements are in fact vector spaces. 
\end{defn}
...
\begin{defn}
	...
	Let $V$ be a vector space. An \textit{hyperplane (in $V$)} is a set of the form $\pp + W^\perp$, where $\pp \in V$ and $W$ is a one-dimensional vector subspace of $V$.
\end{defn}

The following characterization of hyperplanes is immediate.

\begin{theorem}
	(Characterization of hyperplanes).
	
	Let $V$ be a vector space over a field $K$. If $W$ is a hyperplane in $V$ then there exists $\widehat{\nn} \in V$ and $c \in K$ such that $W = \{\vv \in V \mid \vv \cdot \widehat{\nn} + c = 0 \}$. 
\end{theorem}
...
% The main reason we need this orthogonal complements section is so that we know that an expression of the form c_1 x_1 + ... + c_n x_n + b = 0 is a hyperplane. The above establishes all we need to know on this in the most elegant way. 
%
% Previously, I thought it would be better to define a hyperplane to be a sum $\pp + W$, where W is an (n - 1)-dimensional subspace, and then derive that hyperplanes can be described by implicit equations such as $c_1 x_1 + ... + c_n x_n + b = 0$. This definition seems to be less natural than the one above (a hyperplane is $\pp + W^\perp$).
%
% Actually, no, it doesn't... But anyway at least this is now all in one place so I can organize it later
\begin{comment}
	\subsection*{Span to implicit}
	...
	\subsection*{Implicit to span: example}
	...
	\subsection*{Implicit to span: general theory}
...
\begin{defn}
    ...
    The difference between ordered bases and ``regular'' bases is that ordered bases are tuples of vectors, and are thus ordered, while ``regular'' bases are sets of vectors, and are thus unordered. For example, if $V$ is a 2-dimensional vector space and has basis $E = \{\ee_1, \ee_2\}$, then $(\ee_1, \ee_2)$ and $(\ee_2, \ee_1)$ are both ordered bases of $V$, and $(\ee_1, \ee_2) \neq (\ee_2, \ee_1)$.
\end{defn}

\begin{defn}
    (Orientation in $\R^2$).
    
    The \textit{orientation} of an ordered basis $(\ee_1, \ee_2)$ of $\R^2$ is said to be \textit{counterclockwise}, or \textit{positive} iff $|\theta_s(\ee_1, \ee_2)| < |\tttheta_s(\ee_1, \ee_2)|$ and \textit{clockwise}, or \textit{negative}, iff $|\tttheta_s(\ee_1, \ee_2)| < |\theta_s(\ee_1, \ee_2)|$.

    Notice, the orientation of $(\ee_1, \ee_2)$ is counterclockwise iff the orientation of $(\ee_2, \ee_1)$ is clockwise.
\end{defn}

We would like to extend this definition to $\R^n$ and to finite-dimensional inner product spaces over $\R$. Unfortunately, this is not possible as of now since we don't have a notion of $n$-dimensional signed angle $\theta_s$. Recall that we earlier defined the signed counterclockwise angle between same-length vectors to involve the length of one of the two possible paths on the circular arc between the vectors, and defined the signed clockwise angle to involve the length of the other of the two arcs. In $n$ dimensions, we have not just two possible arcs on a circle between vectors, but infinitely many arcs on an $n$-sphere. In this situation, there is no obvious way to classify an arbitrary arc between the vectors as being ``counterclockwise'' or ``clockwise''. One might think there is hope for a second, since one way to pick a particular arc between the two vectors is to choose the shortest one, but, unfortunately this arc corresponds to unsigned angle.

The best way to generalize this definition is to search for an equivalent one. We will do this by characterizing what preserves orientation in the next theorem.

\begin{theorem}
    (Preserving orientation in $\R^2$).

    The orientation of an ordered basis in $\R^2$ is preserved by orthonormalization (recall Theorem \ref{ch::bilinear_forms_metric_tensors::theorem::Gram-Schmidt}) and by rotations in $\R^2$.
\end{theorem}
...
The characterization of orientation in $\R^2$ by the above theorem does generalize well to $n$ dimensions. Though, in preparation for the definition of $n$-dimensional orientation, we must first define a notion of rotation in $n$ dimensions.

\begin{defn}
\label{ch::lin_alg::defn::2_dimensional_rotation}
    ($2$-dimensional rotation).

    Let $V$ be an $n$-dimensional inner product space over $\R$ and let $\hU = (\huu_1, ..., \huu_n)$ be an orthonormal ordered basis for $V$. In analogy to the closed form for the standard matrix of a rotation in $\R^2$ (recall Theorem     \ref{ch::lin_alg::defn::standard_matrix_rotation}), we define matrices $\RR_{ij;t}$ and $\widetilde{\RR}_{ij;t}$ to describe the counterclockwise and clockwise rotations by $t$ in the plane $\spann((\huu_i, \huu_j))$:
    ...
    \begin{align*}
        \RR_{ij;t} &:=
            \kbordermatrix{
                ...
                \text{$i$th row} & 1 & \hdots & \cos(t) & 0 & -\sin(t) & 0 \\
                & 0 & & 0 & \vdots & 0 & \vdots \\
                & 0 & & \vdots & 0 & \vdots & \vdots \\
                & \vdots & & 0 & \vdots & 0 & \vdots \\
                \text{$j$th row} & 0 & \hdots & \sin(t) & 0 & \cos(t) & 1 \\
                & \vdots & & \vdots & \vdots & \vdots & \vdots
            }
        \\
        \widetilde{\RR}_{ij;t} &:= \RR_{ij;2\pi - t}.
    \end{align*}

    We define a \textit{$2$-dimensional rotation (in $V$)} to be a linear function $V \rightarrow V$ whose matrix relative to $\hU$ and $\hU$ is of the above form.
\end{defn}

\begin{defn}
    (Orientation).
    
    We define ordered bases $E$ and $F$ finite-dimensional inner product space over $\R$ to have the same \textit{orientation}, and write $E \sim F$, iff either of the following is true:

    \begin{enumerate}
        \item $E = \text{orthonormalize}(F)$ or $F = \text{orthonormalize}(E)$, where for a basis $G$, we use $\text{orthonormalize}(G)$ to denote the orthonormal basis obtained by performing the orthonormalization algorithm of Theorem \ref{ch::bilinear_forms_metric_tensors::theorem::Gram-Schmidt} on $G$.
        \item There exists a composition $\RR = \RR_k \circ ... \circ \RR_1$ of $2$-dimensional rotations such that $F = \RR(E)$.
    \end{enumerate}
...
\begin{theorem}
    ...
    (Permutations preserve orientation).
...
\begin{proof}
    Let $E = (\ee_1, ..., \ee_n)$ and $F = (\ff_1, ..., \ff_n)$. Since $E \sim F$, we have the following cases.

    \fullindent
    {
        (Case: $E = \text{orthonormalize}(F)$ or $F = \text{orthonormalize}(E)$). [TO-DO]
    }

    \fullindent
    {
        (Case: there exists a composition $\RR$ of $2$-dimensional rotations such that $F = \RR(E)$. Then $\ff_i = \RR(\ee_i)$ for all $i$. For all $\sigma \in S_n$ we have $\ff_{\sigma(i)} = \RR_t(\ee_{\sigma(i)})$. This is equivalent to $E^\sigma \sim F^\sigma$, as desired.
    }
\end{proof}

==========================================

[Since orientation is preserved by orthonormalization, we can study the orientation of arbtirary ordered bases by only considering orthonormal ordered bases.]
...
\begin{deriv}
    ...
    Let $\hU = (\huu_1, ..., \huu_n)$ be an orthonormal ordered basis of an $n$-dimensional inner product space. Consider applying a $2$-dimensional counterclockwise rotation by $\pi/2$ to $\hU$. We have

    \begin{align*}
        \RR_{ij;\pi/2}(\hU) = (\RR_{ij;\pi/2}(\huu_1), ..., \RR_{ij;\pi/2}(\huu_i), ..., \RR_{ij;\pi/2}(\huu_j), ..., \RR_{ij;\pi/2}(\huu_n)).
    \end{align*}

    Since the columns of $\RR_{ij,\pi/2}$ other than the $i$th and $j$th columns are the columns of the identity matrix $\II$, then $\RR_{ij,\pi/2}$ acts as the identity on most of the above vectors:

    \begin{align*}
        \RR_{ij;\pi/2}(\hU) = (\huu_1, ..., \RR_{ij;\pi/2}(\huu_i), ..., \RR_{ij;\pi/2}(\huu_j), ..., \huu_n).
    \end{align*}

    It remains to determine $\RR_{ij;\pi/2}(\huu_i)$ and $\RR_{ij;\pi/2}(\huu_j)$. We have
    
    \begin{align*}
        [\RR_{ij;\pi/2}(\huu_i)]_{\hU} = [\RR_{ij;\pi/2}(\hU)]_{\hU} [\huu_i]_{\hU} = [\RR_{ij;\pi/2}(\hU)]_{\hU} \see_i, \\
        [\RR_{ij;\pi/2}(\huu_j)]_{\hU} = [\RR_{ij;\pi/2}(\hU)]_{\hU} [\huu_j]_{\hU} = [\RR_{ij;\pi/2}(\hU)]_{\hU} \see_j.
    \end{align*}

    We see that to determine $\RR_{ij;\pi/2}(\huu_i)$ and $\RR_{ij;\pi/2}(\huu_j)$ we will have to perform some matrix-vector multplications. Of course, to do this, we need to determine the matrix $[\RR_{ij;\pi/2}(\hU)]_{\hU}$ involved in the multiplications. Substituting $\pi/2$ into the matrix from Definition \ref{ch::lin_alg::defn::2_dimensional_rotation}, we have
    ...
    \begin{align*}
        ...
        \kbordermatrix{
            & & & \text{$i$th column} & & \text{$j$th column} \\
            & \vdots & & \vdots & \vdots & \vdots & \vdots \\
            \text{$i$th row} & 1 & \hdots & 0 & 0 & -1 & 0 \\
            & 0 & & 0 & \vdots & 0 & \vdots \\
            & 0 & & \vdots & 0 & \vdots & \vdots \\
            & \vdots & & 0 & \vdots & 0 & \vdots \\
            \text{$j$th row} & 0 & \hdots & 1 & 0 & 0 & 1 \\
            & \vdots & & \vdots & \vdots & \vdots & \vdots
        }
        \text{ for some $i, j$}.
    \end{align*}

    Now we evaluate the matrix-vector multiplications. From above, we know that $[\RR_{ij;\pi/2}(\huu_i)]_{\hU}$ is the result of multiplying this matrix by $\see_i$. Multiplying a matrix by $\see_i$ picks out its $i$th column, so $[\RR_{ij;\pi/2}(\huu_i)]_{\hU}$ is the $i$th column of the above matrix, which is $\see_j$. Thus $[\RR_{ij;\pi/2}(\huu_i)]_{\hU} = \see_j$. 
    
    Similarly, $[\RR_{ij;\pi/2}(\huu_i)]_{\hU}$ is the result of multiplying the above matrix by $\see_j$. Multiplication by $\see_j$ picks out the $j$th column of a matrix, and the $j$th column of the above matrix is $-\see_i$, so $[\RR_{ij;\pi/2}(\huu_j)]_{\hU} = -\see_i$.
    
    Therefore, we have

    \begin{align*}
        [\RR_{ij;\pi/2}(\huu_i)]_{\hU} &= \see_j \\
        [\RR_{ij;\pi/2}(\huu_j)]_{\hU} &= -\see_i.
    \end{align*}

    It immediately follows that $\RR_{ij;\pi/2}(\huu_i) = \huu_j$ and $\RR_{ij;\pi/2}(\huu_j) = -\huu_i$. Ultimately, this tells us

    \begin{align*}
        \RR_{ij;\pi/2}(\hU) = (\huu_1, ..., \huu_j, ..., -\huu_i, ..., \huu_n).
    \end{align*}

    We can go through a similar computation to see that if we apply a clockwise (rather than counterclockwise) $2$-dimensional rotation by $\pi/2$ to $\hU$, we have

     \begin{align*}
        \widetilde{\RR}_{ij;\pi/2}(\hU) = (\huu_1, ..., -\huu_j, ..., \huu_i, ..., \huu_n).
    \end{align*}

    Since $\hU$ has the same orientation as do the results of applying these $2$-dimensional rotations to i, we have $\hU \sim \RR_{ij;\pi/2}(\hU) \sim \widetilde{\RR}_{ij;\pi/2}(\hU)$. More explicitly,
    ...
    We have arrived at an extremely fundamental result: when working with an orthonormal ordered basis $\hU$, we can swap two of the vectors in $\hU$ and then negate one of the swapped vectors to obtain an orthonormal ordered basis that has the same orientation as $\hU$.
\end{deriv}

We can expand the result of the above derivation from orthonormal ordered bases to arbitrary ordered bases.

\begin{theorem}
    (Antisymmetry of ordered bases).

    If $(\ee_1, ..., \ee_n)$ is an ordered basis of an $n$-dimensional inner product space, then
    ...
    \begin{align*}
        ...
        (\ee_1, ..., -\ee_j, ..., \ee_i, ..., \ee_n)
        \sim
        (\ee_1, ..., \ee_j, ..., -\ee_i, ..., \ee_n) \text{ for all $i, j \in \{1, ..., n\}$}.
    \end{align*}

    That is working with an ordered basis $E$, we can swap two of the vectors in $E$ and then negate one of the swapped vectors to obtain an orthonormal ordered basis that has the same orientation as $E$.
\end{theorem}

\begin{proof}
    Set $E := (\ee_1, ..., \ee_n)$ for convenience. The above derivation proved the theorem for the case when $E$ is an orthonormal ordered basis. For the case when $E$ isn't necessarily orthonormal, we rely on the following claim: if $\RR$ is a composition of $2$-dimensional rotations and $E$ is an ordered basis, then $\text{orthonormalize}(\RR(E)) = \RR(\text{orthonormalize}(E))$.

    If the claim is true, then we have $\RR(E) \sim \text{orthonormalize}(\RR(E)) = \RR(\text{orthonormalize}(E)) \sim \text{orthonormalize}(E) \sim E$. Substituting $\RR_{ij;\pi/2}$ and $\widetilde{\RR}_{ij;\pi/2}$ in for $\RR$ then result in matrix-vector products of the kind seen in the previous derivation, which give the result of the above theorem.

    So it remains to prove the claim, that if $\RR$ is a composition of $2$-dimensional rotations and $E$ is an ordered basis, then $\text{orthonormalize}(\RR(E)) = \RR(\text{orthonormalize}(E))$.
...
\begin{theorem}
    ...
    \begin{align*}
        (s_1 \ee_1, ..., s_n \ee_n) \sim
        \begin{cases}
            (\ee_1, ..., \ee_n), & \text{the number of $s_i$ equal to $-1$ is $-1$ is even} \\
            (-\ee_1, \ee_2, ..., \ee_n) \text{ for any $j \in \{1, ..., n\}$}, & \text{the number of $s_i$ equal to $-1$ is $-1$ is odd} 
        \end{cases}.
    \end{align*}
\end{theorem}
...
\begin{proof}
    ...
    (Case: the number of $s_i$ equal to $-1$ is even). Since the number of $s_i$ equal to $-1$ is even, we can pair every $s_i \ee_i$ with $s_i = -1$ to some $s_j \ee_j$ with $s_j = -1$, so that all $s_i \ee_i$ with $s_i = -1$ appear as a member of only one pair. If we swap the vectors in each pair twice, then, as illustrated by the example $(-\ee_i, -\ee_j) \sim (\ee_j, -\ee_i) \sim (\ee_i, \ee_j)$, the coefficients on these vectors in the new basis with the same orientation will be $1$, with the order of the vectors being the same as in the original pair. Thus $(s_1 \ee_1, ..., s_n \ee_n)$ has the same orientation as $(\ee_1, ..., \ee_n)$.

    (Case: the number of $s_i$ equal to $-1$ is odd). Since the number of $s_i$ equal to $-1$ is odd, we can pair every $s_i \ee_i$ with $s_i = -1$ to some $s_j \ee_j$ with $s_j = -1$, so that all but one $s_i \ee_i$ with $s_i = -1$ appear as a member of only one pair. Let $s_k \ee_k$ be the vector with $s_k = -1$ that is not a member of a pair. Applying the same reasoning as in the previous case, we see that $(s_1 \ee_1, ..., s_n \ee_n)$ has the same orientation as $(\ee_1, ..., \ee_{k - 1}, -\ee_k, \ee_{k + 1}, ... \ee_n)$. Making use of the example $(-\ee_k, \ee_\ell) \sim (\ee_\ell, \ee_k) \sim (\ee_k, -\ee_\ell)$, we see that this ordered basis has the same orientation as $(\ee_1, \ee_2, ..., \ee_{\ell - 1}, -\ee_\ell, \ee_{\ell + 1}, ..., \ee_n)$ for any $\ell \in \{1, ..., n\}$. Thus, when the number of $s_i$ equal to $-1$ is odd, the ordered basis $(s_1 \ee_1, ..., s_n \ee_n)$ has the same orientation as $(\ee_1, ..., \ee_{\ell - 1}, -\ee_\ell, \ee_{\ell + 1}, ..., \ee_n)$ for any $\ell \in \{1, ..., n\}$. In particular it has the same orientation as $(-\ee_1, \ee_2, ..., \ee_n)$, as desired.
\end{proof}

\begin{lemma}
    Let $E$ be an ordered basis of an $n$-dimensional inner product space. We have $\sgn(\sigma) = 1 \implies E^\sigma \sim E$ for all $\sigma \in S_n$.
\end{lemma}

\begin{proof}
    Let $E = (\ee_1, ..., \ee_n)$. The sign of a permutation is $1$ iff the permutation consists of an even number of swaps. So, an equivalent statement to the theorem is: for all $n$, if $\sigma$ consists of $2n$ transpositions, then $E^\sigma \sim E$. We prove this statement by induction on $n$.
    
    (Base case). Consider $E^\sigma$, where $\sigma = \tau \circ \pi$, and where $\pi, \tau$ are transpositions. 
    
    The permutation $\pi$ swaps some $i, j$. Using the example $(\ee_j, \ee_i) \sim (-\ee_i, \ee_j)$, we see \\ $E^\pi \sim (\ee_1, ..., -\ee_i, ..., \ee_j, ..., \ee_n)$. Since this ordered basis has only one basis vector with a coefficient of $-1$, it follows from the previous theorem that it has the same orientation as $F :=  (-\ee_1, \ee_2, ..., \ee_n)$.

    So far, we've shown $E^\pi \sim F$. Since permutations preserve orientation (see Theorem \ref{ch::lin_alg::thm::permutations_preserve_rotational_equivalence}), we may apply the permutation $\tau$ to each side of the equivalence $E^\pi \sim F$ to obtain $(E^\pi)^\tau = E^{\tau \circ \pi}  = E^\sigma \sim F^\tau$.

    We now compute the right side of this equivalence, $F^\tau$.

    \fullindent
    {
        (Case: $\tau$ swaps $1$ with some $\ell > 1$). By studying the example $(\ee_\ell, -\ee_1) \sim (\ee_1, \ee_\ell)$, we see $F^\tau = (-\ee_1, \ee_2, ..., \ee_n)^\tau \sim (\ee_1, ..., \ee_n) = E$.
    }
    \fullindent
    {
        (Case: $\tau$ swaps $k \neq 1$ with some $\ell > k$). By studying the example $(\ee_\ell, \ee_k) \sim (-\ee_k, \ee_\ell)$, we see $F^\tau = (-\ee_1, ..., \ee_n)^\tau \sim (-\ee_1, ..., \ee_{k - 1}, -\ee_k, \ee_{k + 1}, ..., \ee_n)$. In this last ordered basis, the number of basis vectors with a coefficient of $-1$ is even. It follows from the previous theorem that this last ordered basis has the same orientation as $E$. Therefore $F^\tau \sim E$.
    }
    \fullindent
    {
        (Case: $\tau$ swaps $k \neq 1$ with some $\ell < k$). Performing a similar argument as in the previous case, we see that $F^\tau \sim E$.
    }

    We've shown that $F^\tau = E$ for all $\tau \in S_n$. Therefore, we have $E^\sigma \sim F^\tau \sim E$, as desired.
    
    (Inductive case). Assume as the inductive hypothesis if $\sigma$ consists of $2n$ transpositions, then $E^\sigma \sim E$. We need to prove that if $\sigma$ consists of $2(n + 1)$ transpositions, then $E^\sigma \sim E$.

    So, assume $\sigma$ consists of $2(n + 1) = 2n + 2$ transpositions. Then $\sigma = \tau \circ \pi$, where $\pi$ consists of $2n$ transpositions and $\tau$ consists of $2$ transpositions. By the inductive hypothesis, $E^\pi \sim E$. Since permutations preserve orientation (see Theorem \ref{ch::lin_alg::thm::permutations_preserve_rotational_equivalence}), then $(E^\pi)^\tau = E^{\tau \circ \pi} = E^\sigma \sim E^\tau$. Since $\tau$ consists of $2$ transpositions, then from the base case it follows that $E^\tau \sim E$. Thus $E^\sigma \sim E^\tau \sim E$, as desired.
\end{proof}

\begin{theorem}
    For any ordered basis $E$ of a finite-dimensional inner product space over $\R$, we have
...
\begin{proof}
    ...
    \begin{enumerate}
        \item If $\sgn(\pi) = 1$, then $\sgn(\sigma) = 1 \implies E^\sigma \sim E^\pi$ for all $\sigma \in S_n$.
        \item If $\sgn(\pi) = -1$, then $\sgn(\sigma) = -1 \implies E^\sigma \sim E^\pi$ for all $\sigma \in S_n$.
    \end{enumerate}
    ...
    \begin{itemize}
        \item $\sgn(\sigma) = 1 \implies E^\sigma \sim E$ for all $\sigma \in S_n$.
    \end{itemize}

    Now we prove (1) and (2).

    \begin{enumerate}
        \item If $\sgn(\sigma) = \sgn(\pi) = 1$, then $E^\sigma \sim E \sim E^\pi$ by the above bullet.
        \item Let $\pi$ be any odd permutation. If $\sigma$ is an even permutation, then applying the above bullet yields $(E^\pi)^\sigma \sim E^\pi$, i.e. $E^{\sigma \circ \pi} \sim E^\pi$. Thus $\sgn(\sigma) = 1 \implies E^{\sigma \circ \pi} = E^\pi$. In general, every odd permutation is of the form $\sigma \circ \pi$, where $\sigma$ is some even permutation. Thus, as $\sigma$ varies over the even permutations, the permutation $\tau := \sigma \circ \pi$ varies over the odd permutations. This makes the statement $\sgn(\sigma) = 1 \implies E^{\sigma \circ \pi} \sim E^\pi$ equivalent to the statement $\sgn(\tau) = -1 \implies E^\tau \sim E^\pi$. Since the former statement is true, so is the later, as desired.
    \end{enumerate}
   ...
   ($\implies$).
    ...
    \begin{enumerate}
        \item[3.] If $\sgn(\pi) = 1$, then $E^\sigma \sim E^\pi \implies \sgn(\sigma) = 1$ for all $\sigma \in S_n$.
        \item[4.] If $\sgn(\pi) = -1$, then $E^\sigma \sim E^\pi \implies \sgn(\sigma) = -1$ for all $\sigma \in S_n$.
    \end{enumerate}

    Before we can prove (3) and (4), we need to prove the following bullet point:

    \begin{itemize}
        \item $E \sim E^\sigma \implies \sgn(\sigma) = 1$ for all $\sigma \in S_n$.

        We show the contrapositive, $\sgn(\sigma) = -1 \implies E \nsim E^\sigma$ for all $\sigma \in S_n$. Because of (2), it suffices to show that $E \nsim E^\sigma$ for \textit{any} $\sigma$ with $\sgn(\sigma) = -1$. So, we will show that $E \nsim E^{(1 \spc 2)}$.

        It's quick to check that, in general, if $F = (\ff_1, ..., \ff_n)$ and $G = (\gg_1, ..., \gg_n)$ are ordered bases then $F \sim G$ iff $(\ff_1, ..., \ff_i) = (\gg_1, ..., \gg_i)$ for all $i \in [1, n]$. Applying this principle, we see that $E:= (\ee_1, \ee_2, \ee_3, ..., \ee_n) \sim E^{(1 \spc 2)} = (\ee_2, \ee_1, \ee_3, ..., \ee_n)$ iff $(\ee_1, \ee_2) \sim (\ee_2, \ee_1)$. So, it suffices to show $(\ee_1, \ee_2) \sim (\ee_2, \ee_1)$

        [TO-DO: show that the orthonormalizations are not equal and that there is no composition of $2$-rotations from one to the other. note: since in a $2$-dim inner product space, any composition of $2$-dim rotations is just one $2$-dim rotation]
    \end{itemize}

    Now we prove (3) and (4).
    
    \begin{enumerate}
        \item[3.] Let $\pi$ be any even permutation. If $\sigma$ is an even permutation, then applying the above bullet yields $(E^\pi)^\sigma \sim E^\pi \implies \sgn(\sigma) = 1$, i.e., $E^{\sigma \circ \pi} \sim E^\pi \implies \sgn(\sigma) = 1$. If we allow $\pi$ to vary over the even permutations, then since $\sigma$ varies over the even permutations, $\tau := \sigma \circ \pi$ varies over the even permutations, and we obtain the statement $E^\tau \sim E^\pi \implies \sgn(\tau) = 1$ for all $\tau \in S_n$, as desired.
...
\begin{theorem}
    ...
    By the previous theorem, a permutation $E^\sigma$ of an ordered basis $E$ for a finite-dimensional inner product space over $\R$ has one of two\footnotemark orientations: if $\sgn(\sigma) > 0$, then $E^\sigma = E$, and if $\sgn(\sigma) < 0$, then $E^\sigma \sim E^{(1 \spc 2)}$.    
    
    \footnotetext{This might be relatively suprising! Intuition could be that there is something like $2^n$ or $n!$ equivalence classes in $n$ dimensions.}
\end{theorem}

\begin{defn}
    (Positive and negative orientation).

    We say that $E^\sigma$ is \textit{counterclockwise oriented (relative to $E$)} or \textit{positively oriented (relative to $E$)} iff $\sgn(\sigma) > 0$ and that $E^\sigma$ is \textit{clockwise oriented (relative to $E$)} or \textit{negatively oriented (relative to $E$)} iff $\sgn(\sigma) < 0$.
\end{defn}

\subsection*{The determinant and orientation}

\begin{lemma}
    (In three dimensions, there is a composition of $2$-dimensional rotations taking any nonzero vector to any other vector).

    Let $V$ be a $3$-dimensional inner product space. If $\vv \neq \mathbf{0}$, then for all $\ww \in V$ with $||\ww|| = ||\vv||$ there exists a composition of $2$-dimensional rotations $\RR$ such that $\RR(\vv) = \ww$.
\end{lemma}

\begin{proof}
    Let $\hU = (\huu_1, \huu_2, \huu_3)$ be an orthornormal ordered basis of $V$. If we can find angles $\alpha_1, \beta_1$ that rotate $\vv$ to align with $\huu_3$, $(\RR_{2, \beta_1} \circ \RR_{1, \alpha_1})(\vv) = ||\vv||\huu_3$, and angles $\alpha_2, \beta_2$ that rotate $\huu_3$ to align with $\ww$, $(\RR_{2, \beta_2} \circ \RR_{1, \alpha_2})(\huu_3) = \hww$, then the composition of $2$-dimensional rotations $\RR := \RR_{2, \beta_2} \circ \RR_{1, \alpha_2} \circ \RR_{2, \beta_1} \circ \RR_{1, \alpha_1}$ satisfies $\RR(\vv) = \ww$, and the lemma is proven.
...
\begin{lemma}
    (There is a composition of $2$-dimensional rotations taking any nonzero vector to any other vector).
...
\label{ch::lin_alg::thm::n_rot_acts_on_orthonormal_basis}
     (Any orthonormal ordered basis has the same orientation as some permutation $\hU^\sigma$ of a given orthonormal ordered basis $\hU$).
     
     Let $V$ be an finite-dimensional inner product space. If $\hW$ and $\hU$ are orthonormal ordered bases of subspaces of $V$, then there exists $\sigma \in S_k$ such that $\hW \sim \hU^\sigma$. 
\end{theorem}

\begin{proof}
    Let $\hU := (\huu_1, ..., \huu_k)$ and $\hW := (\hww_1, ..., \hww_k)$. 
    
    By the previous lemma, we know there is a composition of $2$-dimensional rotations $\RR$ that satisfies $\RR(\hww_1) = \huu_1$. 
...
\label{ch::lin_alg::thm::det_tracks_orientation}
    ...
    Let $V$ be an $n$-dimensional inner product space with an orientation given by an ordered basis $E$. The orientation of an ordered basis $F = (\ff_1, ..., \ff_n)$ relative to $E$ is $\sgn(\det([\FF]_E))$, where $\FF$ is the matrix whose $i$th column is $\ff_i$.
\end{theorem}

\begin{proof} \mbox{} \\
    \fullindent
    {
        (Case: $V = \R^n$, $F = E$, $E = \sE$). We need to show that the orientation of $E$ is $\sgn(\det(\EE))$, where $\EE$ is the matrix whose $i$th column is $\ee_i$.

        [in each step, show that orientation is preserved and that det is preserved]
    }
...
\begin{theorem}
    in $\R^2$, $\theta_s(\vv, \ww) = \orn((\vv, \ww)) \theta_u(\vv, \ww)$
\end{theorem}
...
\begin{deriv}
    ...
    If a function preserves length and signed angle, then it must preserve orientation. Orientation is the sign of signed angle, so if orientation is not preserved, then the sign of signed angle is not preserved, and thus signed angle is not preserved.
...
As a last sidenote, the following theorem gives some retroactive justification for involving compositions of $2$-dimensional rotations in the definition of orientation.

\begin{theorem} 
    \begin{align*}
        \{\text{compositions of $2$-dimensional rotations in $\R^n$}\} = \{\text{rotations in $\R^n$}\}
    \end{align*}
\end{theorem}
...
\begin{proof}
    ...
    \indent ($\subseteq$). Because their actions are given by matrix-vector products, $2$-dimensional rotations are linear functions. A quick computation shows that $\det(\RR_{ij;t}) = 1$ for all $i, j$. Thus $2$-dimensional rotations are linear functions preserving length and orientation; they are rotations in $\R^n$.
