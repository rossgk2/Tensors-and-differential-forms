...
\begin{theorem}
    (Signedness of signed angle).
    
    We have $\theta_s(\vv, \ww) = -\theta_s(\ww, \vv)$ and $\tttheta_s(\vv, \ww) = -\tttheta_s(\ww, \vv)$ for all $\vv, \ww \in \R^2$.
\end{theorem}

\begin{proof}
    We prove the theorem for $\theta_s$; the proof for $\tttheta_s$ is similar.

    Let $t_1, t_2 \in [0, 2\pi)$ be such that $\cc(t_1) = \hvv$ and $\cc(t_2) = \hww$. We have $\theta_s(\vv, \ww) = s(\cc, \hvv, \hww) = \int_{t_1}^{t_2} ||\frac{d\cc(t)}{dt}|| dt = -\int_{t_2}^{t_1} ||\frac{d\cc(t)}{dt}|| dt = -s(\cc, \hww, \hvv) = -\theta_s(\hww, \hvv)$.
\end{proof}
...
\label{ch::lin_alg::thm::negation_of_angle}
    ...
    We have $-\theta_s(\vv, \ww) \bmod 2\pi = \tttheta_s(\vv, \ww)$ for all $\vv, \ww \in \R^2$ and $-\tttheta_s(\vv, \ww) \bmod 2\pi = \theta_s(\vv, \ww)$ for all $\vv, \ww \in \R^2$.
\end{theorem}
...
\begin{deriv}
    ...
    Knowing this, then for every $t \in [0, 2\pi)$ we can define $\cos(t)$ and $\sin(t)$ to be the coordinates of the unique unit vector $\hvv$ such that $\theta_s(\see_1, \hvv) = t$.
\end{deriv}
...
\begin{theorem}
    ...
    \begin{align*}
        ...
        \begin{pmatrix}
            \cos(\theta_s(\see_1, \vv)) \\
            \sin(\theta_s(\see_1, \vv))
        \end{pmatrix} \text{ for all $\vv \in \R^2$}.
    \end{align*}
\end{theorem}

\begin{proof}
    Let $\vv \in \R^2$. We have $\theta_s(\see_1, \vv) = t$ for some $t \in [0, 2\pi)$. By the previous derivation, we have $\hvv = \vv/||\vv|| = (\cos(t), \sin(t))^\top = (\cos(\theta_s(\see_1, \vv)), \sin(\theta_s(\see_1, \vv)))^\top$. Multiply both sides of this equation by $||\vv||$ to obtain the desired result.
\end{proof}
...
\begin{deriv}
    ...
    consider an arbitrary right triangle $T$ with non-hypotenuse sides $\ss_1$ and $\ss_2$. the hypotenuse $\hh$ is $\hh = \ss_1 \pm \ss_2$. designate either $\theta_u(\ss_1, \hh)$ or $\theta_u(\ss_2, \hh)$ as the ``angle of importance''. for concreteness, let's say the former is the angle of importance.

    since $\ss_1$ is involved in the angle of importance, we call it the \textit{adjacent side}; since $\ss_2$ is not, we call it the \textit{opposite side}.

    let $\ff$ be an length-preserving and signed-angle-preserving function that takes the arbitrary right triangle $T$ to a congruent right triangle $\ff(T) = \tT \subseteq Q_1$ in the first quadrant with hypotenuse $\thh = \ff(\hh)$, a horizontal side $\ss_h = \ff(\ss_1)$ that lies on the $x$-axis, and a vertical side $\ss_v = \ff(\ss_2)$. 

    note, since $\ff$ preserves angles, the angle of importance $\theta_u(\ss_2, \hh)$ is equal to $\theta_u(\ff(\ss_2), \ff(\hh)) = \theta_u(\ss_h, \thh)$, and\footnote{since $\ss_h$ is involved in the angle of importance $\theta_u(\ss_h, \thh)$, it is the adjacent side; since $\ss_v$ is not, it is the opposite side} $\ff$ maps adjacent side to adjacent side and opposite side to opposite side.

    \textbf{since $\ff$ is linear (justify why)} then since $\hh = \ss_1 \pm \ss_2$ we have $\ff(\hh) = \ff(\ss_1) \pm \ff(\ss_2) \iff \thh = \ss_h \pm \ss_v$. since $\thh \in Q_1$, the $\pm$ sign must be $+$, and we have $\thh = \ss_h + \ss_v = ||\ss_h||\see_1 + ||\ss_v|| \see_2 = (||\ss_h||, ||\ss_v||)^\top$. also, by the previous theorem we have $\thh = ||\thh||(\cos(t), \sin(t))^\top = (||\thh||\cos(t), ||\thh||\sin(t))^\top$, where $t = \theta_s(\see_1, \thh) = \theta_s(\ss_h, \thh)$.

    comparing the results of the previous two sentences we see $\ss_h = (||\thh||\cos(t), 0) = ||\thh||\cos(t)\see_1$ and $\ss_v = (0, ||\thh||\sin(t)) = ||\thh||\sin(t)\see_2$.

    with this information, we can obtain expressions for $\cos(t)$ and $\sin(t)$ as functions of the side lengths of $\tT \cong T$:

    \begin{align*}
        \frac{||\ss_1||}{||\hh||} = \frac{||\ff(\ss_1)||}{||\ff(\hh)||} = \frac{||\ss_h||}{||\thh|| \cos(t)} = \frac{||\thh||}{||\thh||} = \cos(t) \\
        \frac{||\ss_2||}{||\hh||} = \frac{||\ff(\ss_2)||}{||\ff(\hh)||} = \frac{||\ss_v||}{||\thh|| \sin(t)} = \frac{||\thh||}{||\thh||} = \sin(t)
    \end{align*}
    
    [make sure to define for $Q_2, Q_3, Q_4$]
\end{deriv}
...
\begin{defn}
    ...
    \textbf{so the abs comes before the mod, meaning it isn't the abs of the mod; it isn't $|\theta_s(v, w)|$. edit intro to reflect this.}
\end{defn}
...
\begin{defn}
    ...
    An isometry $\ff:\R^2 \rightarrow \R^2$ is called the \textit{counterclockwise rotation (by $t$)} iff there exists $t \in [0, 2\pi)$ such that $\theta_s(\vv, \ff(\vv)) = t$ for all $\vv \in \R^2$.
\end{defn}
