...
\begin{theorem}
    ...
    We have $-\theta_s(\vv, \ww) \bmod 2\pi =  \tttheta_s(\vv, \ww)$ for all $\vv, \ww \in \R^2$.
\end{theorem}
...
\begin{proof}
    ...
    Rearranging, we have $\tttheta_s(\vv, \ww) = 2\pi - \theta_s(\vv, \ww)$; applying the map $\theta \mapsto \theta \bmod 2\pi$ to each side of this equation and using the general fact that modulus distributes over addition\footnote{See \ref{ch::appendix::thm::modulus_distributes_over_addition}.}, we have $\tttheta_s(\vv, \ww) \bmod 2\pi = 2\pi \bmod 2\pi - \theta_s(\vv, \ww) \bmod 2\pi = -\theta_s(\vv, \ww) \bmod 2\pi$ for all $\vv, \ww \in \R^2$. So $\tttheta_s(\vv, \ww) \bmod 2\pi = -\theta_s(\vv, \ww) \bmod 2\pi$ for all $\vv, \ww \in \R^2$. Since $\tttheta_s(\vv, \ww) \in [0, 2\pi)$, then $\tttheta_s(\vv, \ww) \bmod 2\pi = \tttheta_s(\vv, \ww)$. Therefore $\tttheta_s(\vv, \ww) = -\theta_s(\vv, \ww) \bmod 2\pi$, as desired. 
\end{proof}
...
\begin{deriv}
    ...
    Knowing this, then for every $t \in [0, 2\pi)$ we can define $\cos(t)$ and $\sin(t)$ to be the coordinates of the unique unit vector $(\cos(t), \sin(t))^\top$ such that
