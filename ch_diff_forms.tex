...
\begin{proof}
   ...
   \begin{align*}
    ...
    Since the pushforward on the top exterior power is multiplication by the determinant (Theorem \ref{ch::antisymmetry::rmk::top_pullback_det}), the above is equal to
...
\label{ch::diff_forms::rmk::pullback_star_notation}
    ...
    In Remark \ref{ch::antisymmetry::rmk::star_notation_pushforward_pullback}, we introduced the practice of using $\FF^*$ to denote a pullback. From now on, we denote the pullback $\Omega^k \FF^*:T_{\FF(\pp)}(N) \rightarrow T_\pp(M)$ by $\FF^*:T_{\FF(\pp)}(N) \rightarrow T_\pp(M)$.
\end{remark}
