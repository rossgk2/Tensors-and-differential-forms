...
\label{ch::exterior_pwrs::rmk::exterior_product}
    ...
    The wedge product is sometimes called the \textit{exterior product}.

    In Remark \ref{ch::motivated_intro::rmk::outer_products}, we explained how there is no relationship between the concepts of ``inner product'' and ``outer product'' that is as fundamental as is suggested by the concepts' names. The same is true when we add the concept ``exterior product'' into the mix: there is no real fundamental relationship between the concepts of ``inner products'', ``outer products'', and ``exterior products''. (Well, perhaps ``outer products'' and ``exterior products'' are somewhat related, since the exterior product involves a tensor product of vectors, which can be identified with an outer product of vectors. This still seems pretty tenuous to me, though). It's moreso that these concepts coincidentally happen to have names that are suggestive of a fundamental relationship.
\end{remark}
