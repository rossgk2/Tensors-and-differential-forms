...
\begin{defn}
    ...
    Let $V$ be an $n$-dimensional inner product space and let $\hU = (\huu_1, ..., \huu_n)$ be an orthonormal ordered basis for $V$. A \textit{rotation by the Euler angles $\theta_1, ..., \theta_k \in [0, 2\pi)$} is a function $\RR_{\theta_1, ..., \theta_k}:V \rightarrow V$ of the form ${\RR_{\theta_1, ..., \theta_k} = \RR_{k, \theta_k} \circ ... \circ \RR_{1, \theta_1}}$, where, for each $i$, $\RR_{i, \theta_i}$ is the $2$-rotation by $\theta_i \in [0, 2\pi)$ on the subspace $\spann(\hU - \{\huu_i\})$. The angles $\theta_1, ..., \theta_k$ are called \textit{Euler angles}.
\end{defn}

\begin{theorem}
    All rotations by Euler angles are orthogonal linear functions.
\end{theorem}
...
%     ``To do so, we first choose $\alpha, \beta, \gamma$ such that $\RR(\tuu_1) = \huu_1$.''    
% \end{theorem}
...
\begin{lemma}
    ...
    Let $\hU$ be an orthonormal ordered basis of a $3$-dimensional inner product space $V$. If $\vv \neq \mathbf{0}$, then for all $\ww \in V$ with $||\ww|| = ||\vv||$ there exists an Euler rotation $\RR$ such that $\RR(\vv) = \ww$.
\end{lemma}

\begin{proof}
    If we can find angles $\alpha_1, \beta_1$ that rotate $\vv$ to align with $\huu_3$,
    ...
    then the Euler rotation $\RR := \RR_{2, \beta_2} \circ \RR_{1, \alpha_2} \circ \RR_{2, \beta_1} \circ \RR_{1, \alpha_1}$ satisfies $\RR(\vv) = \ww$, and we are done.

    ============================

    Let $[\vv]_{\hU} = (v_1, v_2, v_3)^\top$. 
    ...
    That is, $\alpha_1 = \arctan2(v_2, v_3)$. Since $\cos(\alpha_1) = v_3/\sqrt{v_2^2 + v_3^2}$ and $\sin(\alpha_1) = v_2/\sqrt{v_2^2 + v_3^2}$, the above is
    ...
    $\beta_1$ is such that the first component of the above is zero,

    \begin{align*}
        v_1 \cos(\beta_1) - \sqrt{v_2^2 + v_3^2} \sin(\beta_1) = 0.
    \end{align*}
    
    That is, $\beta_1 = \arctan2(v_1, \sqrt{v_2^2 + v_3^2})$. Since $\cos(\beta_1) = \sqrt{v_2^2 + v_3^2}$ and $\sin(\beta_1) = v_1$, we have

    \begin{align*}
        [\RR_{2, \beta_1}(\RR_{1, \alpha_1}(\vv))]_{\hU} = 
        \begin{pmatrix}
            v_1 \sqrt{v_2^2 + v_3^2} -(\sqrt{v_2^2 + v_3^2}) v_1 \\
            0 \\
            v_1^2 + \sqrt{v_2^2 + v_3^2}^2
        \end{pmatrix}
        =
        \begin{pmatrix}
            0 \\
    ...
    Now do similar thing for $\alpha_2, \beta_2$
\end{proof}

\begin{lemma}
    Previous lemma is true for $n$ dimensions?
\end{lemma}
...
\label{ch::exterior_pwrs::thm::3_rot_acts_on_subspaces}
    ...
    Let $V$ be a inner product space with $\dim(V) \geq 3$, and consider $3$-dimensional subspaces $\widetilde{W}$ and $W$ of $V$. If $\tU = ( \tuu_1, \tuu_2 )$ and $\hU = ( \huu_1, \huu_2 )$ are orthonormal ordered bases of $\widetilde{W}$ and $W$, respectively, there exists an Euler rotation that takes $\tU$ to $\hU^\sigma$ for some $\sigma \in S_2$.
\end{lemma}

\begin{proof}
    By the lemma, consider an Euler rotation $\RR_{\alpha, \beta, \gamma}$ such that $\RR_{\alpha, \beta, \gamma}(\tuu_1) = \huu_1$.

    Since $\RR_{\alpha, \beta, \gamma}$ is an orthogonal linear function, then $\RR(\tuu_2)$ must have length $1$ and be orthogonal to $\RR(\tuu_1) = \huu_1$. Thus $\RR(\tuu_2)$ is either $\huu_2$ or $-\huu_2$, and $\RR(\tU) = (\huu_1, \pm \huu_2 )$. More formally, $\RR(\tU) =\hU^\sigma$ for some $\sigma \in S_2$.
\end{proof}
