...
\begin{proof}
    \mbox{} \\ \indent
    \begin{enumerate}
        \item Suppose $x_0$ appears in two different cycles. Then the lengths $k$ and $\ell$, $k \neq \ell$, of these cycles are the minimal $r$ and $s$ such that $\sigma^{r - 1}(x_0) = x_0$ and $\sigma^{s - 1}(x_0) = x_0$. The minimal $t$ for which $\sigma^{t - 1}(x_0) = x_0$ is unique, so we have $k = t = \ell$, which contradicts $k \neq \ell$. Thus the cycles must be the same.
        ...
        So, $x_1 = \pi^{-(j - 1)}(x_j)$. By definition of the cycle of $\pi$ about $x_1$, we also have $x_1 = \pi^{p - 1}(x_1)$, where $p = \text{argmin}_q \pi^{q - 1}(x_1) = x_1$. Substituting $x_1 = \pi^{-(j - 1)}(x_j)$ into $x_1 = \pi^{p - 1}(x_1)$, we obtain
        
        \begin{align*}
            x_1 = \pi^{(p - 1) - (j - 1)}(x_j). \tag{2}
        \end{align*}

        Equation (2) maps $x_j$ to $x_1$, and equation (1) maps $x_1$ to $x_j$. Combining these two equations, we can map $x_j$ to $x_j$:
        
        \begin{align*}
            x_j = \pi^{j - 1}(\pi^{(p - 1) - (j - 1)}(x_j)) = \pi^{p - 1}(x_j).
        \end{align*}
        
        Since $\pi^{p - 1}(x_j) = x_j$, then $\pi$ is the cycle of $\pi$ about $x_j$.
        
        \item We can use (2) to obtain an equivalent tuple that has $x_0$ as its first element. This tuple represents the cycle about $x_0$.
    \end{enumerate}
\end{proof}
...
\begin{theorem}
    ...
    Every permutation $\sigma$ on a finite set can be uniquely decomposed as $\sigma = \sigma_k \circ ... \circ \sigma_1$, where $\sigma_1, ..., \sigma_k$ are cycles.    
\end{theorem}
...
\begin{proof}
    ...
    \begin{itemize}
        \item Consider the sequence $(x_0, \sigma(x_0), ..., \sigma^i(x_0), ...)$. Since $X$ is a finite set, then the pigeonhole principle implies that for some $j > i \geq |X|$ we have $\sigma^i(x_0) = \sigma^j(x_0)$. Applying $(\sigma^{-1})^i$ to each side of this equation, we have $\sigma^{j - i}(x_0) = x_0$. So, we see that there exists $k - 1$ such that $\sigma^{k - 1}(x_0) = x_0$. (We have $k - 1 = j - i \iff k = j - i + 1$).
    ...
    To prove this, let $x \in X$. By the previous lemma, there is a unique cycle $\pi_x \in C$ of $\sigma$ that has $x$ in its image. Since no other cycles from $C$ have $x$ in their image, then when they act on $x$, they return $x$, unchanged, as output. Thus $P(x) = \pi_x(x)$. Our definition of $\sigma_{x_0}$ ensures that $\sigma_{x_0}(x) = \sigma(x)$ for any $x$ in the image of $\sigma_{x_0}$; i.e., that $\pi(x) = \sigma(x)$ for all $\pi \in C$. So we have $P(x) = \pi_x(x) = \sigma(x)$ for all $x \in X$, and $\sigma = P = \prod_{\pi \in C} \pi$, as desired.
    ...
    % \begin{itemize}
    %     \item if a cycle has $x$ in its image then it is about $x$
    %     \item cycles are either disjoint or equal
    %     \item cycles are disjoint <=> all elements of one are disjoint with the entire other
    % \end{itemize}
