...
\begin{lemma}
    ...
    \begin{align*}
        \hU \sim \hU^\sigma \implies \sgn(\sigma) = 1 \text{ for all $\sigma \in S_n$}.
    \end{align*}
\end{lemma}

\begin{proof}
    If $\hU \sim \hU^\sigma$, then $\hU^\sigma = \RR_\theta(\hU)$ for some $\theta \in [0, 2\pi)$.
    
    (Case: $\theta \notin \{\frac{\pi}{2}, \frac{3\pi}{2}\}$). The matrix $[\RR_\theta(\hU)]_{\hU}$ has one column containing the entries $\cos(\theta), \sin(\theta)$, and one column containing the entries $-\sin(\theta), \cos(\theta)$. Since $\theta \notin \{\frac{\pi}{2}, \frac{3\pi}{2}\}$, then these columns contain entries in $[0, 1] - \{0, 1\} = (0, 1)$. Because of this, each of these columns is not equal to $\see_i$ for any $i$. It follows that each of the vectors in $\RR_\theta(\hU)$ corresponding to these columns is not equal to $\huu_i$ for any $i$. So, $\RR_\theta(\hU)$ is not a permutation of $\hU$. This contradicts the assumption $\hU \sim \hU^\sigma$, making this case impossible.

    (Case: $\theta \in \{\frac{\pi}{2}, \frac{3\pi}{2}\}$). As was alluded to in Theorem \ref{ch::exterior_pwrs::thm::antisymmetry_ordered_bases_general}, when $\theta \in \{\frac{\pi}{2}, \frac{3\pi}{2}\}$, then \\ $\RR_\theta(\hU) \in \{(\huu_1, ..., -\huu_{i + 1}, \huu_i, ..., \huu_n), (\huu_1, ...,\huu_{i + 1}, -\huu_i, ..., \huu_n)\}$. 
\end{proof}
