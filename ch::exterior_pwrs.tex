...
\subsection*{Basic notions of antisymmetry}

First, we formalize the notion of swapping elements in a list and in a tensor.

\begin{defn}
    (Transposition).

    A \textit{transposition} is a function $\{1, ..., n\} \rightarrow \{1, ..., n\}$ that swaps two elements and leaves all of the others fixed. That is, a transposition is a function $\sigma:\{1, ..., n\} \rightarrow \{1, ..., n\}$ for which there exist $i, j \in \{1, ..., n\}$, $i \neq j$, such that $\sigma(i) = j$, $\sigma(j) = i$, and $\sigma(k) = k$ for all $k \notin \{i, j\}$.
\end{defn}

\begin{defn}
    (Transposition of a tensor).
    
    Let $V_1, ..., V_k$ be vector spaces over the same field. Given a transposition $\sigma$ and a tensor $\TT \in V_1 \otimes ... \otimes V_k$, we define the map $(\cdot)^\sigma:V_1 \otimes ... \otimes V_k \rightarrow V_1 \otimes ... \otimes V_k$ sending $\TT \mapsto \TT^\sigma$ by specifying its action on elementary tensors and extending linearly. We define
    
    \begin{align*}
        (\vv_1 \otimes ... \otimes \vv_i \otimes ... \otimes \vv_j \otimes ... \otimes \vv_k)^\sigma &:= \vv_{\sigma(1)} \otimes ... \otimes \vv_{\sigma(i)} \otimes ... \otimes \vv_{\sigma(j)} \otimes ... \otimes \vv_{\sigma(k)} \\
        &= \vv_1 \otimes ... \otimes \vv_j \otimes ... \otimes \vv_i \otimes ... \otimes \vv_k.
    \end{align*}

    So, for example, if $\TT = \vv_1 \otimes \vv_2 \otimes \vv_3 + \vv_3 \otimes \vv_1 \otimes \vv_2$ and $\sigma$ is the transposition swapping $1$ and $2$, $\sigma(1) = 2$ and $\sigma(2) = 1$, then $\TT^\sigma = \vv_2 \otimes \vv_1 \otimes \vv_3 + \vv_3 \otimes \vv_2 \otimes \vv_1$.
\end{defn}

Now that we have notation for applying a transposition to every element in a tensor's decomposition, we can define antisymmetric tensors.

\begin{defn}
    (Antisymmetry in terms of transpositions).
    
    Let $V_1, ..., V_k$ be vector spaces over the same field. A tensor $\TT \in V_1 \otimes ... \otimes V_k$ is \textit{antisymmetric} iff  $\TT^\sigma = -\TT$ for all transpositions $\sigma$.
\end{defn}

The above is a characterization of antisymmetry in terms of a transposition. One can imagine that since any reordering of $1, ..., k$ can be achieved by a sequence of transpositions\footnote{This is an intuitive, yet unproven, claim. We formally prove this in Theorem \ref{ch::exterior_pwrs::thm::permutations_decomposition_into_transpositions}.}, antisymmetry can be characterized in terms of what happens when a more general reordering like this is applied.

Such reorderings are called \textit{permutations}. A basic understanding of permutations is crucial for further understanding of antisymmetry, so we briefly pause to study permutations now.
...
Now, we prove an intuitive theorem which we alluded to earlier.

\begin{theorem}
\label{ch::exterior_pwrs::thm::permutations_decomposition_into_transpositions}
...
\begin{lemma}
    ...
    If $\sigma$ and $\pi$ are transpositions in $S_n$ with $\sigma \neq \pi$, then there exist transpositions $\gamma$ and $\delta$ in $S_n$ and $i \in \{1, ..., n\}$ such that $\pi \circ \sigma = \delta \circ \gamma$, where $\gamma$ moves $i$ and $\delta$ fixes $i$.
\end{lemma}

\begin{proof}
    Since $\pi \neq \sigma$, there are two possible cases.
    
    (Case: $\pi$ is disjoint with $\sigma$). If $\pi$ and $\sigma$ are disjoint, then they commute, $\pi \circ \sigma = \sigma \circ \pi$. Set $\delta := \sigma$, $\gamma := \pi$, and let $i \in \gamma$. Then $\gamma$ moves $i$, $\delta$ fixes $i$, and we have $\pi \circ \sigma = \delta \circ \gamma$.
    
    (Case: $\pi$ is not disjoint with $\sigma$).
    
    \indent \indent (Case: $\pi \circ \sigma = \transp{i}{j} \circ \transp{i}{r}$ for some $r \neq j$). We have $\pi \circ \sigma = \transp{i}{j} \circ \transp{i}{r} = \begin{pmatrix} i & r & j \end{pmatrix} = \begin{pmatrix} j & i & r \end{pmatrix} = \transp{j}{r} \circ \transp{i}{j}$. The transposition $\gamma := \transp{i}{j}$ moves $i$ and the transposition $\delta := \transp{j}{r}$ fixes $i$, as desired.
    
    \indent \indent (Case: $\pi \circ \sigma = \transp{i}{j} \circ \transp{j}{s}$ for some $s \neq i$). A similar argument produces $\gamma := \transp{j}{i}$, which moves $j$, and $\delta := \transp{i}{s}$, which fixes $j$, as desired.
\end{proof}

\begin{theorem}
    (The parity of a permutation is unique).

    A permutation in $S_n$ cannot be both even and odd.
\end{theorem}
...
\begin{proof}
    ...
    We now prove by induction on $s \in \{r, r - 1, ..., 2\}$ that there exist transpositions $\tttau_1, ..., \tttau_s$ and $i \in \{1, ..., n\}$ such that $\tau_r \circ ... \circ \tau_s = \tttau_r \circ ... \circ \tttau_s$, where all of the transpositions on the right side of the equation, except for the rightmost one, fix $i$. (That is, $\tttau_r, \tttau_{r - 1} ..., \tttau_{s + 1}$ fix $i$ and $\tttau_s$ moves $i$). After we know this, then in particular, when $s = 2$, we have that there exists an $i \in \{1, ..., n\}$ such that $\id = \tttau_r \circ ... \circ \tttau_2 \circ \tttau_1$, where $\tttau_r, ..., \tttau_2$ do not move $i$ and where $\tttau_1$ moves $i$. Thus, $\id(i) = \tttau_1(i) \neq i$, which is a contradiction!

    \vspace{.25cm}
    
    It remains to prove the statement on $s$ by induction.
    
    (Base case: $s = r - 1$). We have to show that there exist transpositions $\tttau_r, \tttau_{r - 1}$ and $i \in \{1, ..., n\}$ such that $\tau_r \circ \tau_{r - 1} = \tttau_r \circ \tttau_{r - 1}$, where $\tttau_{r - 1}$ moves $i$ and $\tau_r$ fixes $i$.
    
    Note that $\tau_r \neq \tau_{r - 1}$; otherwise, $\tau_r \circ \tau_{r - 1}$ can be written as a composition of zero transpositions, making $r$ not minimal. Since $\tau_r \neq \tau_{r - 1}$, the previous lemma, which gives us exactly what we desire, can be applied to $\tau_r \circ \tau_{r - 1}$.
    
    (Inductive case: $s < r - 1$). By the inductive hypothesis, we have $\tau_r \circ ... \circ \tau_s = \tttau_r \circ ... \circ \tttau_s$, where all of the transpositions on the right side of the equation except for the rightmost one fix $i$.

    Similarly as to in the base case, we cannot have $\tttau_s = \tau_{s - 1}$; if we did, then $r$ would not be minimal. Thus $\tttau_s \neq \tau_{s - 1}$, and the previous lemma can be applied. The lemma says that there exist $i \in \{1, ..., n\}$, a 
    transposition $\gamma$ fixing $i$, and a transposition  $\delta$ moving $i$ such that $\tttau_s \circ \tau_{s - 1} = \gamma \circ \delta$. Thus, $\tttau_r \circ ... \circ \tttau_s \circ \tau_{s - 1} = \tttau_r \circ ... \circ \tttau_{s - 2} \circ \gamma \circ \delta$, so that all of the transpositions on the right side of the equation except for the rightmost one fix $i$, as desired.
\end{proof}
...
As promised, we can now characterize antisymmetric tensors in terms of arbitrary permutations, not just transpositions.
...
\label{ch::exterior_pwrs::defn::antisymmetric_tensor}
    ...
    Let $V_1, ..., V_k$ be vector spaces over the same field. A tensor $\TT \in V_1 \otimes ... \otimes V_k$ is \textit{antisymmetric} iff $\TT^\sigma = \sgn(\sigma) \TT$ for all permutations $\sigma \in S_k$.
\end{theorem}

\begin{proof}
    We must prove that $\TT^\pi = -\TT$ for all transpositions $\pi \in S_k$ $\iff$ $\TT^\sigma = \sgn(\sigma) \TT$ for all permutations $\sigma \in S_k$.

    ($\implies$). Decompose $\sigma$ into a product of transpositions, $\sigma = \sigma_k \circ ... \circ \sigma_1$. We have $\TT^\sigma = ((\TT^{\sigma_1})^{\sigma_2}...)^{\sigma_k} = (-1)^k \TT = \sgn(\sigma) \TT$.

    ($\impliedby$). If $\pi$ is a transposition, then $\sgn(\pi) = -1$ and we have $\TT^\pi = \sgn(\pi)\TT = -\TT$.
\end{proof}

Now, we provide a way to construct antisymmetric tensors (from arbitrary tensors). This is important, as we haven't shown that antisymmetric tensors actually exist.

\begin{defn}
    (Antisymmetrization of elements of tensor product spaces).
    
    Let $V_1, ..., V_k$ be vector spaces over the same field. We define ${\alt:V_1 \otimes ... \otimes V_k \rightarrow V_1 \otimes ... \otimes V_k}$ on elementary tensors and extend linearly; for an elementary tensor $\TT \in V_1 \otimes ... \otimes V_k$, we define
...
Unsurprisingly, the tensor product of two antisymmetric tensors is not guaranteed to be antisymmetric.

\begin{theorem}
    (The tensor product of antisymmetric tensors is not always antisymmetric).
\end{theorem}
...
Therefore, if we want an operation $\wedge$ such that $\TT \wedge \SS$ is an antisymmetric tensor whenever $\TT$ and $\SS$ are antisymmetric tensors, we must do some additional work.
...
\begin{defn}
\label{ch::exterior_pwrs::defn::wedge_product_space}
    (Wedge product space). 
