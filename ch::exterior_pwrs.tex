...
\begin{defn}
    ...
    We use $S_n$ to denote the \textit{set of permutations on $\{1, ..., n\}$}. Formally, $S_n := \{ \text{functions } \text{$\{1, ..., n\} \rightarrow \{1, ... n \}$} \}$.
\end{defn}

\begin{remark}
    (Sign of a permutation).

    A permutation $\sigma$ is called a \textit{swap}, or \textit{transposition}, iff there exist $j, k$ such that $\sigma(j) = k$, $\sigma(k) = j$, and $\sigma(i) = i$ for all $i \notin \{j, k\}$. Every permutation can be decomposed into a composition of swaps.
    ...
    \begin{align*}
        \sgn(\sigma) := 
        \begin{cases}
            1 & \text{there are an even number of swaps in any decomposition of $\sigma$} \\
            -1 & \text{there are an odd number of swaps in any decomposition of $\sigma$}
        \end{cases}.
    \end{align*}
    
    Surprisingly, $\sgn$ is a well-defined function.: the parity (the ``evenness'' or ``oddness'') of the number of swaps in a permutation's decomposition into only swaps is always the same. (We do not prove this).
...
\begin{proof}
    Let $\hU = (\huu_1, ..., \huu_n)$. The sign of a permutation is $1$ iff the permutation consists of an even number of swaps. So, an equivalent statement to the theorem is: for all $n$, if $\sigma$ consists of $2n$ swaps, then $\hU^\sigma \sim \hU$. We prove this statement by induction on $n$.
