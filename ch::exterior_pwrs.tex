...
First, we formalize the notion of applying $2$-dimensional rotations to vectors of dimension greater than $2$. We do this by ``extending'' $2$-dimensional rotations so that they can be applied to elements of $n$-dimensional inner product spaces. 
...
\begin{defn}
    ...
    We define orthonormal ordered bases $\hU$ and $\hW$ of a finite-dimensional inner product space to be \textit{equivalent under rotation} iff there exists a $2$-dimensional rotation $\RR_\theta$ for which $\hW = \RR_\theta(\hU)$. (Recall Definition \ref{ch::lin_alg::defn::linear_fn_acts_on_vectors} for the meaning of $\RR_\theta(\hU)$).
\end{defn}
...
Similarly to what was done in the previous derivation, we use $\theta \in \{\frac{\pi}{2}, \frac{3\pi}{2}\}$ in the matrix relative to bases of a $2$-dimensional rotation defined on $\spann((\huu_i, \huu_j))$ to obtain the formal characterization of the antisymmetry of ordered bases.
...
\begin{proof}
    ...
    (Case: the number of $s_i$ equal to $-1$ is even). Since the number of $s_i$ equal to $-1$ is even, we can pair every $s_i \huu_i$ with $s_i = -1$ to some $s_j \huu_j$ with $s_j = -1$, so that all $s_i \huu_i$ with $s_i = -1$ appear as a member of only one pair. If we swap the vectors in each pair twice, then, as illustrated by the example $(-\huu_i, -\huu_j) \sim (\huu_j, -\huu_i) \sim (\huu_i, \huu_j)$, the coefficients on these vectors in the rotationally equivalent ortonormal ordered basis will be $1$, with the order of the vectors being the same as in the original pair. Thus $(s_1 \huu_1, ..., s_n \huu_n)$ is rotationally equivalent to $(\huu_1, ..., \huu_n)$.
...
\begin{lemma}
    Let $\hU = (\huu_1, ..., \huu_n)$ be an orthonormal ordered basis of an $n$-dimensional inner product space.  We have

    \begin{align*}
        \hU^\sigma \sim \hU \text{ for all $\sigma \in S_n$ with $\sgn(\sigma) = 1$}.
    \end{align*}
\end{lemma}

\begin{proof}
    The sign of a permutation is $1$ iff it consists of an even number of swaps. So, an equivalent statement to the theorem is: for all $n$, if $\sigma$ consists of $2n$ swaps, then $\hU^\sigma \sim \hU$. We prove this statement by induction on $n$.
    
    (Base case). Consider $\hU^\sigma$, where $\sigma = \tau \circ \pi$, and where $\pi, \tau$ are swaps. 
    ...
    (Inductive case). Assume as the inductive hypothesis if $\sigma$ consists of $2n$ swaps, then $\hU^\sigma \sim \hU$. We need to prove that if $\sigma$ consists of $2(n + 1)$ swaps, then $\hU^\sigma \sim \hU$.

    So, assume $\sigma$ consists of $2(n + 1) = 2n + 2$ swaps. Then $\sigma = \tau \circ \pi$, where $\pi$ consists of $2n$ swaps and $\tau$ consists of $2$ swaps. We have $\hU^\sigma = (\hU^\pi)^\tau$. By the inductive hypothesis, $\hU^\pi \sim \hU$. Since permutations preserve rotational equivalence (see Theorem \ref{ch::exterior_pwrs::thm::permutations_preserve_rotational_equivalence}), then $(\hU^\pi)^\tau = \hU^{\tau \circ \pi} = \hU^\sigma \sim \hU^\tau$. \textbf{apply base case to $\hU^\tau$}
\end{proof}
...
\begin{proof}
    ...
    ($\implies$). If $\hU^\sigma \sim \hU^\pi$, then $\hU^\pi = \RR_\theta(\hU^\sigma)$ for some $\theta \in [0, 2\pi)$.
    
    (Case: $\theta \notin \{\frac{\pi}{2}, \frac{3\pi}{2}\}$). The matrix $[\RR_\theta(\hU^\sigma)]_{\hU^\sigma}$ has one column containing the entries $\cos(\theta), \sin(\theta)$, and one column containing the entries $-\sin(\theta), \cos(\theta)$. Since $\theta \notin \{\frac{\pi}{2}, \frac{3\pi}{2}\}$, then these columns contain entries in $[0, 1] - \{0, 1\} = (0, 1)$. Because of this, each of these columns is not equal to $\see_i$ for any $i$. It follows that each of the vectors in $\RR_\theta(\hU^\sigma)$ corresponding to these columns is not equal to $\huu_i$ for any $i$. So, $\RR_\theta(\hU^\sigma)$ is not a permutation of $\hU$. This contradicts the assumption $\hU^\sigma \sim \hU^\pi$, making this case impossible.

    (Case: $\theta \in \{\frac{\pi}{2}, \frac{3\pi}{2}\}$). As we saw in Theorem \ref{ch::exterior_pwrs::thm::antisymmetry_ordered_bases_general}
...
\subsection*{Completing the definition of orientation for finite-dimensional inner product spaces}
