...
Now that we have notation for applying a transposition to every element in a tensor's decomposition, we can define antisymmetry.

\begin{defn}
    (Antisymmetry in terms of transpositions).
    
    Let $V_1, ..., V_k$ be vector spaces over the same field. We say a tensor $\TT \in V_1 \otimes ... \otimes V_k$ is \textit{antisymmetric} iff  $\TT^\sigma = -\TT$ for all transpositions $\sigma$.
\end{defn}
...
The definition of permutation allows for a more general definition of permutation.
...
\label{ch::exterior_pwrs::thm::permutations_decomposition_into_transpositions}
    ...
    Every permutation $\sigma$ on a finite set can be decomposed as $\sigma = \sigma_k \circ ... \circ \sigma_1$, where $\sigma_1, ..., \sigma_k$ are transpositions. Note, the set of transpositions in the decomposition does not uniquely correspond to $\sigma$.
\end{theorem}
...
Lastly, we present the following definition and theorem- the main reason for our excursion into permutations.

\begin{defn}
    (Parity of a permutation).

    A permutation is said to have \textit{even parity}, or to be \textit{even}, iff it is equal to the composition of an even number of transpositions. A permutation is said to have \textit{odd parity}, or to be \textit{odd}, iff it is equal to the composition of an odd number of transpositions.
\end{defn}
...
Unsuprisingly, the tensor product of two antisymmetric tensors is not guarenteed to be antisymmetric.

\begin{theorem}
    
\end{theorem}

Therefore, if we want an operation $\wedge$ such that if $\TT$ and $\SS$ are antisymmetric tensors, then $\TT \wedge \SS$ is another antisymmetric tensor, we must do some additional work.
