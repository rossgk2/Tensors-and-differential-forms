...
%\begin{proof}
    ...
    %Let $\tW_1$ and $\tW_2$ be additional positively oriented orthonormal bases for $V$ and $W$.
...
\begin{defn}
    ...
    We define orthonormal ordered bases $\hU = (\huu_1, \huu_2)$ and $\tW = (\hww_1, \hww_2)$ of a 2-dimensional inner product space\footnote{Note, this is the first time we have required that the vector space under consideration be an inner product space. We need this constraint so that we can speak of orthonormal bases.} to be \textit{equivalent under rotation} iff there exists a $2$-dimensional rotation $\RR_\theta$ for which $\tW = \RR_\theta(\hU)$. (Recall Definition \ref{ch::lin_alg::defn::linear_fn_acts_on_vectors} for the meaning of $\RR_\theta(\hU)$).
\end{defn}
...
\begin{defn}
    ...
    It's helpful to spell out a further detail. If $\RR_\theta$ is a $2$-dimensional on a finite-dimensional inner product space and $\hU$ is an orthonormal basis of the space, then the matrix of $\RR_\theta$ relative to $\hU$ and $\hU$ is    
...
Equipped with $2$-rotations that can be applied on finite-dimensional inner product spaces, we can now generalize our ideas about orientation to $n$ dimensions.
...
\begin{theorem}
    ...
    (Antisymmetry of ordered bases for an $n$-dimensional inner product space).
     ...
     Similarly to what was done in the previous derivation, we use $\theta = -\frac{\pi}{2}$ and $\theta = \frac{\pi}{2}$ in the matrix relative to bases of a $2$-dimensional rotation defined on $\spann((\huu_i, \huu_j))$ to obtain the formal statement of the antisymmetry of ordered bases: for any orthonormal ordered basis $\hU = (\huu_1, ..., \huu_n)$ of $V$, we have
...
\begin{theorem}
    ...
    Let $\hU = (\huu_1, ..., \huu_n)$ be an orthonormal ordered basis of an inner product space. If $s_1, ..., s_n \in \{-1, 1\}$, we have

    \begin{align*}
        (s_1 \huu_1, ..., s_n \huu_n) \sim
        \begin{cases}
            (\huu_1, ..., \huu_n), & \text{the number of $s_i$ equal to $-1$ is $-1$ is even} \\
            (\huu_1, ..., \huu_{i - 1}, -\huu_i, \huu_{i + 1}, ..., \huu_n) \text{ for any $i \in \{1, ..., n\}$}, & \text{the number of $s_i$ equal to $-1$ is $-1$ is odd} 
        \end{cases}.
    \end{align*}
\end{theorem}
