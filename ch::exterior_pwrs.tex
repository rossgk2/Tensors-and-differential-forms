...
\begin{defn}
    ...
    We say that a permutation $\sigma$ is a \textit{cycle} iff there exists an $x \in X$ such that $\sigma(X) = \cyc_\sigma(x)$. 
\end{defn}

\begin{defn}
    (Cycle notation).

    We use the tuple-without-commas $\begin{pmatrix} x_0 & x_1 & \hdots & x_k \end{pmatrix}$ to denote the permutation that sends $x_i \mapsto x_{i + 1}$ for $i \in \{0, 1, ..., k - 1\}$ and $x_k \mapsto x_0$. Clearly, this permutation is a cycle.

    In a slight abuse of notation, we write ``$x \in \sigma$'' to mean that $x$ is one of the elements appearing in the tuple-without-commas notation for $\sigma$.
\end{defn}
...
\begin{lemma}
    ...
    \begin{enumerate}
        \item If $\sigma$ is a cycle, then the $x \in X$ for which $\sigma(X) = \cyc_\sigma(x)$ is unique.
        \item If $x \in X$ is in a cycle, then that cycle is the cycle about $x$.
        \item $\begin{pmatrix} x_0 & x_1 & \hdots & x_k \end{pmatrix}
        =
        \begin{pmatrix} x_k & x_0 & x_1 & \hdots & x_{k - 1} \end{pmatrix}
        =
        ...
        =
        \begin{pmatrix} x_1 & & \hdots & x_k & x_0 \end{pmatrix}$
        \end{enumerate}
        \item The cycle of $\sigma$ about $x$ is equal to the cycle consisting of $\begin{pmatrix} x & \sigma(x) & \hdots & \sigma^{k - 1}(x) \end{pmatrix}$, where $k = \text{argmin}_\ell \sigma^{\ell - 1}(x) = x$.
\end{lemma}
...
\begin{proof}
    ...
    \begin{enumerate}
        \item 
        \item Suppose $x \in \sigma$, where $\sigma = \cyc_\pi(y)$ for some permutation $\pi$ and some $y \in X$. So, $x \in \cyc_\pi(y)$.
        
        We need to show that $\cyc_\pi(y) = \cyc_\pi(x)$. Since $x \in \cyc_\pi(y)$, we have $x = \pi^i(y) \iff y = \pi^{-i}(x)$ for some $i$. Since $y \in \cyc_\pi(y)$, we also have $y = \pi^j(y)$ for some $j$. Without loss of generality, we can assume $j - i > 0 \iff j > i$. (We show that there exists $k$ such that $y = \pi^k(y)$, where $k > i$: if $j \ngtr i$, then there exists an  $n \geq 0$ such that $nj > i$; applying $\pi^{nj}$ to both sides $y = \pi^j(y)$, we obtain $y = \pi^{nj + 1}(y)$, where $nj + 1 > nj > i$. Thus, $k := nj + 1 > i$ is such that $y = \pi^k(y)$). Combining these two equations, we have $y = \pi^j(\pi^{-i}(x)) = \pi^{j - i}(x)$. So, we know $y = \pi^{j - i}(x)$, where $j - i > 0$, i.e., $y \in \cyc_\pi(x)$.

        Overall, we've shown that, for all $x,y \in X$, if $x \in \cyc_\pi(y)$ then $y \in \cyc_\pi(x)$.
