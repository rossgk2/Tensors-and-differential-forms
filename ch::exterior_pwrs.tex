\chapter{Antisymmetry}
\label{ch::exterior_pwrs}

This chapter focuses on \textit{antisymmetric} mathematical objects. Loosely speaking, a mathematical object is antisymmetric if swapping two of its components is equivalent to negating one of the components.
...
\label{ch::exterior_pwrs::defn::antisymmetric_tensor}
    ...
    Let $V_1, ..., V_k$ be vector spaces over the same field. We say that a tensor $\vv_1 \otimes ... \otimes \vv_k \in V_1 \otimes ... \otimes V_k$ is \textit{antisymmetric} iff for all $i, j$ with $i < j$, swapping $\vv_i$ and $\vv_j$ is equivalent to negating the tensor:

    \begin{align*}
       \vv_1 \otimes ... \otimes \vv_i \otimes ... \otimes \vv_j \otimes ... \otimes \vv_k =
...
\begin{theorem}
    (Alternate antisymmetric tensor characterization).

     Let $V_1, ..., V_k$ be vector spaces over the same field. The tensor $\vv_1 \otimes ... \otimes \vv_k \in V_1 \otimes ... \otimes V_k$ is antisymmetric iff, when we perform a sequence of swaps such that for some bijection $\sigma: \{1, .., n\} \rightarrow \{1, ..., n\}$, the $i$th position in the tensor is occupied by $\vv_{\sigma(i)}$, then

    \begin{align*}
        \vv_1 \otimes ... \vv_k =
        s \spc \vv_{\sigma(1)} \otimes ... \otimes \vv_{\sigma(k)},
    \end{align*}

    where $s = 1$ if the ordering given by $\sigma$ was obtained with an even number of swaps were performed and $s = -1$ if the ordering given by $\sigma$ was obtained with an odd number of swaps.    
\end{theorem}

As we can see above, using bijections $\sigma:\{1, .., n\} \rightarrow \{1, ..., n\}$ to represent the rearrangement of indices is quite helpful when discussing antisymmetry. We formalize this idea in the following two definitions.

\begin{defn}
    (Permutation).
    
    Let $X$ be a finite set. A \textit{permutation on $X$} is a bijection $X \rightarrow X$. Intuitively, a permutation on $X$ is thought of as redistributing the names of the elements of $X$. We use $S_k$ to denote the set of permutations on $\{1, ..., n\}$. (So, $S_k = \{ \text{functions } \text{$\{1, ..., n\} \rightarrow \{1, ... n \}$} \}$).
\end{defn}

\begin{defn}
    (Permutations of tensors).

    Given a permutation $\sigma \in S_k$ and a tensor $\TT \in V_1 \otimes ... \otimes V_k$, we define the map $(\cdot)^\sigma:V_1 \otimes ... \otimes V_k \rightarrow V_1 \otimes ... \otimes V_k$ sending $\TT \mapsto \TT^\sigma$ by specifying its action on elementary tensors and extending linearly. We define
...
The previous definition allows us to succinctly restate the definition of an antisymmetric tensor.

\begin{theorem}
\label{ch::exterior_pwrs::defn::antisymmetric_tensor}
    (Succinct antisymmetric tensor characterization). 
    
    Let $V_1, ..., V_k$ be vector spaces over the same field. A tensor $\TT \in V_1 \otimes ... \otimes V_k$ is antisymmetric iff $\TT = s \TT^\sigma$ for all $\sigma \in S_k$, where $s = 1$ if $\sigma$ is a composition of an even number of swaps and $s = -1$ if $\sigma$ is a composition of an odd number of swaps.
\end{theorem}

Now we must address an important point that has been skirted.

\begin{deriv}
    (Uniqueness of the parity of a permutation).

    Let $V_1, ..., V_k$ be vector spaces over the same field, and let $\vv_1 \otimes ... \otimes \vv_k \in V_1 \otimes ... \otimes V_k$ be an antisymmetric tensor. What if it were possible for $\sigma$ to be both a composition of an even number of swaps and a composition of an odd number of swaps? By the definition of antisymmetric tensors, we would have $\vv_1 \otimes ... \otimes \vv_k = (\vv_1 \otimes ... \otimes \vv_k)^\sigma$ and $\vv_1 \otimes ... \otimes \vv_k = -(\vv_1 \otimes ... \otimes \vv_k)^\sigma$; adding these equations and dividing by $2$ gives $\vv_1 \otimes ... \otimes \vv_k = \mathbf{0}$. Since $\vv_1 \otimes ... \otimes \vv_k$ was arbitrary, it follows that all antisymmetric tensors are the zero tensor. This sounds ridiculous, and, indeed, the following lemma shows that this cannot be true.
    
    Thus, $\sigma$ can only be achieved by \textit{either} an even number of swaps or an odd number of swaps, but not both.
\end{deriv}

\begin{lemma}
    There exists a nonzero order-$k$ antisymmetric tensor for every $k \geq 2$.
\end{lemma}

\begin{proof}
    We prove the lemma by induction.

    (Case: $k = 2$). Define $\TT := \vv_1 \otimes \vv_2 - \vv_2 \otimes \vv_1$. We have $\TT^\sigma = (\vv_1 \otimes \vv_2)^\sigma - (\vv_2 \otimes \vv_1)^\sigma$ for all $\sigma \in S_2$. When $\sigma$ is the identity, we clearly have $\TT^\sigma = \TT$. When $\sigma$ is the permutation that swaps $1$ and $2$, then we have $\TT^\sigma = \vv_2 \otimes \vv_1 - \vv_1 \otimes \vv_2 = -(\vv_1 \otimes \vv_2 - \vv_2 \otimes \vv_1) = -\TT$. So, $\TT = s\TT^\sigma$,  , for all $\sigma \in S_2$.

    (Case: $k > 2$). Assume as the inductive hypothesis that $\vv_1 \otimes ... \otimes \vv_k$ is an antisymmetric tensor. We need to show that there exists an antisymmetric tensor of order $k$.
    
    Now let $\vv \in V_k$. Let $\TT_i$ be the elementary order-$(k + 1)$ tensor that has $\vv$ in the $i$th position, with all other positions being occupied by $\vv_1, ..., \vv_k$ such that the order of $\vv_1, ..., \vv_k$ is preserved. We claim that $\sum_{i = 1}^{k + 1} (-1)^{i - 1} \TT_i$ is antisymmetric.
    ...
    \begin{align*}
       \sum_{i = 1}^{k + 1} (-1)^{i - 1} \TT_i = \sum_{i = 1}^{k + 1} (-1)^{i - 1} \ww_{i1} \otimes ... \otimes \ww_{ik}.
    \end{align*}
    
    \textbf{TO-DO}
\end{proof}

The above justifications allow for the following terminology.

\begin{defn}
    (Parity of a permutation).

    A permutation is said have \textit{even parity}, or to be \textit{even}, iff it is a composition of an even number of swaps. A permutation is said have \textit{odd parity}, or to be \textit{odd}, iff it is a composition of an even number of swaps.

    Recall that the discussion of the previous derivation, together with the previous lemma, show that a permutation must either be even or odd.
\end{defn}
...
We now repeat an earlier definition for convenience.
