...
\begin{theorem}
    ...
    Let $V$ be a finite-dimensional vector space. The set of antisymmetric tensors in $V^{\otimes k}$ is a vector subspace of $V^{\otimes k}$.
\end{theorem}
...
\begin{defn}
    ...
    In the below, we give proof that $\alt(\TT)$  $\TT \in V^{\otimes k}$.
...
\begin{remark}
    (Antisymmetric tensors exist).

    Since $\alt(\TT)$ is antisymmetric for all tensors $\TT$, we can conclude that antisymmetric tensors exist.
\end{remark}
...
[The properties of the antisymmetrization function can be restated in the following significant way.] 
