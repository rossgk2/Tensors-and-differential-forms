...
\begin{defn}
    ...
    Let $V$ be a $n$-dimensional inner product space over $\R$. The \textit{orientation} for $V$ induced by an ordered basis $E = (\ee_1, ..., \ee_n)$ for $V$ is the vector in $\{-1, 1\}^{n - 1}$ whose $i$th component is $\sgn(\sproj(\ee_{i + 1} \rightarrow \ee_i))$.

    In general, an \textit{orientation for $V$} is an element of $\{-1, 1\}^{\times(n - 1)}$.

    We define the \textit{standard orientation for $V$} to be the orientation of the standard basis, which is $(1, ..., 1) \in \{-1, 1\}^{\times(n - 1)}$.
\end{defn}
